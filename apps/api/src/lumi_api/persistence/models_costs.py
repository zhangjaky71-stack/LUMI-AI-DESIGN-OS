...
class CostBudgetLimitModel(
    ...
    __table_args__ = (
        CheckConstraint(
            "scope_type IN "
            "('organization','project','agent_run','task','operation')",
            name="scope_type",
        ),
        ...
        CheckConstraint(
            "enforcement_mode IN ('hard','approval')",
            name="mode",
        ),
    )


class CostReservationModel(
    Base,
    UUIDPrimaryKeyMixin,
    TenantMixin,
    MutableMixin,
    ...
    pricing_snapshot_id: Mapped[str | None] = mapped_column(
        String(128),
        nullable=True,
    )
...
class QuotaLimitModel(
    ...
    metric: Mapped[str] = mapped_column(String(100), nullable=False)
    period_key: Mapped[str] = mapped_column(String(32), nullable=False)
    quantity_limit: Mapped[Decimal] = mapped_column(
        Numeric(30, 10),
        nullable=False,
    )
    unit: Mapped[str] = mapped_column(String(64), nullable=False)
...
class CostBudgetChangeAuditModel(
    Base,
    UUIDPrimaryKeyMixin,
    TenantMixin,
    CreatedAtMixin,
    ...
    __table_args__ = (
        CheckConstraint(
            "action IN ('create','update','disable','enable')",
            name="action",
        ),
    )
