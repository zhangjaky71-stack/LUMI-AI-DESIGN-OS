...
class AutoRepairJobModel(Base):
    ...
    working_artifact_version_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("artifact_versions.id", ondelete="RESTRICT"),
        nullable=False,
    )
    current_quality_result_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("artifact_quality_results.quality_result_id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('PLANNED','RUNNING','READY','REVIEW_REQUIRED','FAILED',"
            "'BUDGET_EXHAUSTED','STALE_CONFLICT','CANCELLED')",
...
class AutoRepairAttemptModel(Base):
    ...
    repair_job_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("auto_repair_jobs.repair_job_id", ondelete="CASCADE"),
        primary_key=True,
    )
...
class RepairLearningSignalModel(Base):
    ...
    candidate_quality_result_id: Mapped[UUID | None] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("artifact_quality_results.quality_result_id", ondelete="SET NULL"),
        nullable=True,
    )
    repair_kind: Mapped[str] = mapped_column(String(40), nullable=False)
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            name="fk_repair_learning_attempt",
            ondelete="CASCADE",
        ),
        CheckConstraint(
