...
class CostLedgerModel(Base, UUIDPrimaryKeyMixin, TenantMixin, CreatedAtMixin):
    ...
    generation_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("generations.id", ondelete="SET NULL"), nullable=True)
    related_entry_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("cost_ledger.id", ondelete="RESTRICT"), nullable=True)
    ...
    provider_request_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("provider_requests.id", ondelete="SET NULL"), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint("(entry_type = 'charge' AND related_entry_id IS NULL) OR (entry_type IN ('reversal','adjustment') AND related_entry_id IS NOT NULL)", name="related_entry_semantics"),
    )
...
class UsageCounterModel(Base, UUIDPrimaryKeyMixin, TenantMixin, MutableMixin):
    ...
    quantity: Mapped[Decimal] = mapped_column(Numeric(30, 10), nullable=False, server_default="0")
    unit: Mapped[str] = mapped_column(String(80), nullable=False)
    __table_args__ = (UniqueConstraint("organization_id", "period_key", "metric_key", name="uq_usage_counter_scope"),)
