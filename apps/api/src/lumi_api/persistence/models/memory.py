...
_SCOPE_CHECK = "scope_type IN ('SESSION','USER','PROJECT','BRAND','AGENT','ORGANIZATION')"
_KIND_CHECK = (
    "kind IN ('PREFERENCE','FACT','DECISION','CONSTRAINT_PREFERENCE',"
...
class MemoryRecordModel(IdMixin, MutableTimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint(_SCOPE_CHECK, name="scope"),
        CheckConstraint(_KIND_CHECK, name="kind"),
        CheckConstraint(
            "status IN ('ACTIVE','PENDING_CONFIRMATION','SUPERSEDED',"
        ...
        Index(
            "ix_memory_records_org_scope",
            "organization_id",
            "scope_type",
            "scope_id",
        ),
        Index(
            "ix_memory_records_semantic_key",
            "organization_id",
            "scope_type",
            "scope_id",
            "kind",
    ...
    content_structured: Mapped[dict[str, Any]] = mapped_column(
        JSONB,
        nullable=False,
    )
...
class MemoryCandidateModel(IdMixin, MutableTimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "outcome IN ('WRITE','DEDUPLICATE_CONFIRM','REQUIRE_CONFIRMATION',"
            "'BRAND_RULE_PROPOSAL')",
            name="outcome",
        ),
        CheckConstraint(
        ...
        Index(
            "ix_memory_candidates_org_outcome",
            "organization_id",
            "outcome",
            "created_at",
        ),
        Index(
            "ix_memory_candidates_scope_key",
            "organization_id",
            "scope_type",
            "scope_id",
            "semantic_key",
    ...
    created_by_type: Mapped[str] = mapped_column(String(32), nullable=False)
    created_by_id: Mapped[str] = mapped_column(String(512), nullable=False)
    explicit_remember: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
        server_default="false",
    )
    temporal_coexistence: Mapped[bool] = mapped_column(
        Boolean,
        nullable=False,
        default=False,
        server_default="false",
