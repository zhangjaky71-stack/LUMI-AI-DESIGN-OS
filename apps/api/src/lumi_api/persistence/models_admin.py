...
class PlatformAdminPrincipalModel(Base):
    ...
    granted_by_user_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"))
...
class PlatformAdminAuditEventModel(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_platform_admin_audit_resource", "resource_type", "resource_id", "created_at"),
    )
    ...
    target_organization_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), ForeignKey("organizations.id", ondelete="SET NULL"))
    reason: Mapped[str | None] = mapped_column(Text)
    metadata_json: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False, server_default=JSON_OBJECT_DEFAULT)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
...
class PlatformFeatureFlagModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("flag_key", "scope", "target_id", name="uq_platform_feature_flag_scope"),
        CheckConstraint("scope IN ('global','organization','user')", name="ck_platform_feature_flag_scope"),
        CheckConstraint("scope='global' OR target_id IS NOT NULL", name="ck_platform_feature_flag_target"),
    ...
    value_json: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False, server_default=JSON_OBJECT_DEFAULT)
    ...
    updated_by_user_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), ForeignKey("users.id", ondelete="RESTRICT"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=func.now())
    version: Mapped[int] = mapped_column(Integer, nullable=False, server_default="1")
