...
class GovernanceRetentionPolicyModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "retention_class",
            "policy_version",
            name="uq_governance_retention_policy",
        ),
        CheckConstraint(
...
class GovernanceLegalHoldModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "organization_id", "hold_key", name="uq_governance_legal_hold_key"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "length(btrim(reason)) >= 8", name="ck_governance_legal_hold_reason"
        ),
        CheckConstraint(
...
Index(
    ...
    GovernanceLegalHoldModel.organization_id,
    GovernanceLegalHoldModel.scope_type,
    GovernanceLegalHoldModel.scope_id,
    postgresql_where=GovernanceLegalHoldModel.released_at.is_(None),
)
...
class GovernanceDeletionRequestModel(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_governance_deletion_subject",
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "length(btrim(reason)) >= 8", name="ck_governance_deletion_reason"
        ),
        Index(
            "ix_governance_deletion_org_status",
            "organization_id",
            "status",
            "requested_at",
        ),
    ...
    status: Mapped[str] = mapped_column(
        String(32), nullable=False, server_default="IDENTIFIED"
    )
    ...
    hold_blockers_json: Mapped[list[Any]] = mapped_column(
        JSONB, nullable=False, server_default=JSON_ARRAY_DEFAULT
    )
    object_gc_status: Mapped[str] = mapped_column(
        String(24), nullable=False, server_default="PENDING"
    )
    search_gc_status: Mapped[str] = mapped_column(
        String(24), nullable=False, server_default="PENDING"
    )
...
class GovernanceAuditExportModel(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_governance_audit_export_format",
        ),
        CheckConstraint(
        ...
        Index(
            "ix_governance_audit_export_org_requested",
            "organization_id",
            "requested_at",
        ),
    )
