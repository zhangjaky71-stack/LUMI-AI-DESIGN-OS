...
from lumi_video_generation.gateway_contract import (
...
def _capability(shot: CompiledShot) -> Capability:
    value = video_capability(shot)
    if value == VIDEO_IMAGE_TO_VIDEO:
        return Capability.VIDEO_IMAGE_TO_VIDEO
    if value == VIDEO_TEXT_TO_VIDEO:
        return Capability.VIDEO_TEXT_TO_VIDEO
    raise ValueError("VIDEO_CAPABILITY_INVALID")
...
class VideoFeatureRegistry:
    ...
    def excluded_providers(self, required: frozenset[str]) -> tuple[str, ...]:
        ...
        return tuple(
            sorted(
                provider
                for provider in self.provider_features
...
def to_model_request(
    ...
    *,
    ...
    required_features = required_video_features(shot)
    ...
    return ModelRequest(
        ...
        agent_run_id=(
            _stable_uuid(spec.agent_run_id)
            if spec.agent_run_id is not None
            else None
        ),
...
class ModelGatewayVideoAdapter:
    ...
    async def estimate(
        ...
        *,
        ...
        if self.feature_registry is not None:
            reasons += (
                f"VIDEO_FEATURE_REGISTRY:{self.feature_registry.snapshot_id}",
            )
    ...
    async def poll(
        ...
        *,
        ...
        if (
            result.provider != pending.result.provider
            or result.model != pending.result.model
        ):
