...
def test_operation_guard_execute_commits_snapshot_once() -> None:
...
def test_operation_guard_replay_never_invokes_graph() -> None:
    async def scenario() -> None:
        snapshot = _snapshot()
        client = _FakeClient(
            [
...
def test_unknown_execution_failure_is_marked_ambiguous() -> None:
    async def scenario() -> None:
        snapshot = _snapshot()
        client = _FakeClient(
            [
...
def test_policy_denial_is_marked_final() -> None:
    async def scenario() -> None:
        snapshot = _snapshot()
        client = _FakeClient(
            [
...
def test_event_retry_reuses_exact_payload() -> None:
    async def scenario() -> None:
        snapshot = _snapshot()
        client = _FakeClient(
            [
...
def test_approval_reader_preserves_canonical_scope() -> None:
    async def scenario() -> None:
        approval_id = uuid4()
        organization_id = uuid4()
        project_id = uuid4()
