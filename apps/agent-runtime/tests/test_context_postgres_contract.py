...
def class_fields(path: Path, class_name: str) -> set[str]:
    ...
    for node in tree.body:
        if isinstance(node, ast.ClassDef) and node.name == class_name:
            return {child.target.id for child in node.body if isinstance(child, ast.AnnAssign) and isinstance(child.target, ast.Name)}
...
class ContextPostgresContractTests(unittest.TestCase):
    def test_context_adapter_is_sdk_neutral_and_read_only(self) -> None:
        ...
        for forbidden in ("import asyncpg","import sqlalchemy","from sqlalchemy","import psycopg","INSERT INTO","UPDATE ","DELETE FROM","openai","anthropic"):
            self.assertNotIn(forbidden, text)

    def test_project_task_asset_queries_are_tenant_scoped(self) -> None:
        text = SOURCE.read_text(encoding="utf-8")
        for marker in ("organization_id","project_id","WHERE id=$1 AND organization_id=$2","a.organization_id=$1 AND a.project_id=$2","WHERE organization_id=$1 AND project_id=$2"):
            self.assertIn(marker, text)

    def test_adapter_reads_canonical_sources(self) -> None:
        text = SOURCE.read_text(encoding="utf-8")
        for marker in ("FROM projects","brief_json","brief_version","profile_json","tone_json","brand_rules","FROM tasks","task_dependencies","FROM assets","asset_metadata","FROM artifacts","asset_embeddings"):
            self.assertIn(marker, text)
        for forbidden in ("project_summaries","summary/source_digest","chat_history","conversation_messages","raw_chat"):
            self.assertNotIn(forbidden, text)

    def test_embedding_query_uses_actual_fields(self) -> None:
        text = SOURCE.read_text(encoding="utf-8")
        fields = class_fields(ASSET_MODEL, "AssetEmbedding")
        self.assertIn("embedding", fields); self.assertIn("dimensions", fields)
        self.assertIn("e.embedding", text); self.assertIn("e.dimensions", text); self.assertIn("query_embedding", text); self.assertNotIn("e.dims", text)
