...
class TaskGraphRuntimeTests(unittest.TestCase):
    ...
    def test_retry_reuses_stable_logical_operation_key(self) -> None:
        ...
        self.assertEqual(
            attempts[0].logical_operation_key,
            attempts[1].logical_operation_key,
        )

    def test_lease_expiry_requires_reconciliation(self) -> None:
        task = claim_ready_tasks(
            self.store,
            self.bundle.graph.graph_id,
        ...
        self.assertEqual(reclaimed, (task.task_id,))
        failed = self.store.task(task.task_id)
        self.assertEqual(failed.status, TaskState.FAILED_RETRYABLE)
        self.assertTrue(failed.error["provider_reconciliation_required"])

    def test_restart_reuses_persisted_store_state(self) -> None:
        first = claim_ready_tasks(
            self.store,
            self.bundle.graph.graph_id,
        ...
        self.assertEqual(ready, ["finalize"])
