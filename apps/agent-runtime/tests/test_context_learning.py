...
class Writer:
    ...
    async def submit_correction(self, proposal):
...
class ContextLearningTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_project_change_invalidates_project_cache(self):
        invalidator = ContextCacheInvalidator(InMemoryContextCache())
    ...
    def test_irrelevant_event_is_noop(self):
        invalidator = ContextCacheInvalidator(InMemoryContextCache())
        event = ContextInvalidationEvent(
            event_name="unrelated.event", project_id=str(PROJECT), source_version=None
        )
        self.assertEqual(invalidator.apply(event), 0)
