...
class InMemoryContextCache:
    """Process-local cache only; durable project state remains in source systems."""
    ...
    def get(self, key: str, *, source_versions: tuple[str, ...]) -> ContextManifest | None:
    ...
    def put(self, key: str, *, project_id: str, manifest: ContextManifest) -> None:
    ...
    def invalidate_project(self, project_id: str) -> int:
        keys = [key for key, entry in self._entries.items() if entry.project_id == project_id]
        for key in keys:
