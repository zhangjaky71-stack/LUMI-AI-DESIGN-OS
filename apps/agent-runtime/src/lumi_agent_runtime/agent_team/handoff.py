...
def validate_result_for_agent(
    ...
    if (
        result.status == TeamTaskStatus.WAITING_EXTERNAL
        and not profile.supports_waiting_external
    ):
        raise ValueError("AGENT_TEAM_WAITING_EXTERNAL_NOT_SUPPORTED")
    if (
        result.status == TeamTaskStatus.WAITING_APPROVAL
        and not profile.approval_gated_actions
    ):
