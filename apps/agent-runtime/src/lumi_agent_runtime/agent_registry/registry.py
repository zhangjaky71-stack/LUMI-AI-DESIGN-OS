...
class AgentRegistry:
    ...
    def resolve(self, requested_ref: str) -> ResolvedAgent:
        ...
        if definition is None:
            raise AgentDefinitionNotFoundError(
                f"Agent definition not found: {agent_id}@{exact_version}"
            )
    ...
    def _resolve_version(
        ...
        if selector in aliases:
            ...
            if selector == "production" and release.status != AgentReleaseStatus.PRODUCTION:
                raise AgentReleaseError(
                    f"production alias does not target PRODUCTION: {agent_id}@{version}"
                )
            if release.status == AgentReleaseStatus.DISABLED:
                raise AgentReleaseError(
                    f"Agent alias targets disabled release: {agent_id}@{version}"
                )
    ...
    def _release(self, agent_id: str, version: str) -> AgentReleaseRecord:
        row = next(
            (
                item
                for item in self.release_manifest.releases
    ...
    def _validate_release_manifest(self) -> None:
        ...
        if definition_keys != release_keys:
            raise AgentReleaseError(
                ...
                f"missing_release={sorted(definition_keys - release_keys)}, "
                f"missing_definition={sorted(release_keys - definition_keys)}"
            )
