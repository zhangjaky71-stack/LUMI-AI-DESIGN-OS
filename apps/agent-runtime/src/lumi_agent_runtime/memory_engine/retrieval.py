    ...
    async def search(self, query: MemorySearchQuery) -> tuple[MemorySearchResult, ...]:
        ...
        for record in records:
            ...
            if query.kind is not None and record.kind != query.kind:
                continue
            if not can_read_scope(record.scope_type, record.scope_id, query.access):
                continue
            lexical = _lexical_score(query.text, f"{record.semantic_key} {record.summary}")
            semantic = _semantic_score(query.query_embedding, record.embedding)
            scope_score = _SCOPE_WEIGHT[record.scope_type]
            confidence = record.confidence
            freshness = _freshness(record.last_confirmed_at or record.created_at, now)
            score = min(
                1.0,
                0.30 * lexical
                + 0.28 * semantic
...
def _semantic_score(query: tuple[float, ...] | None, record: tuple[float, ...] | None) -> float:
    ...
    if left == 0 or right == 0:
        return 0.0
    return max(0.0, min(1.0, (dot / (left * right) + 1.0) / 2.0))
