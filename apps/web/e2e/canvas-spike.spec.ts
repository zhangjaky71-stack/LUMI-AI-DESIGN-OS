...
test.describe("NODE-08 Canvas Technology Spike", () => {
  ...
  test("loads Pixi, supports precise UI interactions, and emits benchmark evidence", async ({ page }) => {
    await page.goto("/canvas-spike");
    await page.waitForFunction(() => window.__LUMI_CANVAS_SPIKE__?.snapshot().ready === true, null, {
      timeout: 30_000,
    });
    ...
    await expect(page.getByTestId("selected-reference")).toContainText("asset://product-reference-v1");
    await expect(page.getByTestId("selection-box")).toBeVisible();
    await expect(page.getByRole("button", { name: "resize southeast" })).toBeVisible();
    ...
    expect(report.metrics.find((metric) => metric.name === "simple-2k")?.nodeCount).toBe(2_000);
    expect(report.metrics.find((metric) => metric.name === "simple-10k")?.nodeCount).toBe(10_000);
    expect(report.metrics.find((metric) => metric.name === "images-1k")?.nodeCount).toBe(1_000);
    expect(report.metrics.find((metric) => metric.name === "text-1k-rich-100")?.nodeCount).toBe(1_100);
    expect(report.metrics.find((metric) => metric.name === "selected-500-drag")?.nodeCount).toBe(500);
