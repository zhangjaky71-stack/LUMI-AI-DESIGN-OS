...
async function startRun(page: Page) {
  ...
  await canvas
    .getByRole("button", { name: "Hero Product", exact: true })
    .click();
  await canvas
...
test.describe("NODE-54 AI Design Workspace", () => {
  test("chat and Canvas share one project workspace with explicit selection context", async ({
    page,
  }) => {
    await page.goto(workspace);
    await expect(
      page.getByRole("heading", { name: "夏季新品发布" }),
    ).toBeVisible();
    const canvas = page.getByLabel("Canvas preview");
    ...
    await canvas
      .getByRole("button", { name: "Headline", exact: true })
      .click({ modifiers: ["Shift"] });
    await expect(agent.getByText("2 selected", { exact: true })).toBeVisible();
    await expect(agent.getByText(/Document v7/).first()).toBeVisible();
    await expect(
      agent.getByText(/Hero Product · locked identity/),
    ).toBeVisible();
  });

  test("streamed run deduplicates events and ends on an actionable approval", async ({
    page,
  }) => {
    await page.goto(workspace);
    await startRun(page);
  ...
  test("stale approval is visible but cannot submit an old decision", async ({
    page,
  }) => {
    await page.goto(workspace);
    const stale = page
      .getByRole("heading", { name: "旧方向确认" })
  ...
  test("pause, resume and stop use versioned run controls", async ({
    page,
  }) => {
    await page.goto(workspace);
    await startRun(page);
  ...
  test("artifact placement binds exact artifact and document versions", async ({
    page,
  }) => {
    await page.goto(workspace);
    await startRun(page);
  ...
  test("provider fallback warning is explicit without exposing internal reasoning", async ({
    page,
  }) => {
    await page.goto(workspace);
    await expect(
      page.getByText(/主图像 Provider 当前处于降级状态/),
    ).toBeVisible();
    const inspector = page.getByLabel("Layers 与 Inspector");
    await inspector
      .getByRole("button", { name: "context", exact: true })
      .click();
    await expect(
      inspector.getByText(/private chain-of-thought/i),
    ).toBeVisible();
    await expect(page.getByText(/reasoning trace/i)).toHaveCount(0);
  });

  test("mobile uses focused panels instead of squeezing the desktop three-column layout", async ({
    page,
  }) => {
