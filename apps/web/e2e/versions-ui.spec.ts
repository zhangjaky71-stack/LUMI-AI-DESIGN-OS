...
test.describe("NODE-59 Versions UI", () => {
  test("shows immutable ArtifactVersion timeline with approval and head state", async ({
    page,
  }) => {
    await page.goto(versions);
    await expect(
      page.getByRole("heading", { name: /夏季新品发布 · Versions/ }),
    ).toBeVisible();
    await expect(page.getByLabel("Artifact")).toHaveValue(
      "artifact-campaign-canvas",
    );
  ...
  test("compares exact Design IR versions and exposes structured semantic property changes", async ({
    page,
  }) => {
    await page.goto(versions);
    ...
    await expect(
      compare.getByText("Offer Badge · x", { exact: true }),
    ).toBeVisible();
    await expect(
      compare.getByText("Headline · text", { exact: true }),
    ).toBeVisible();
    await expect(compare.getByText(/920 → 944/)).toBeVisible();
    await expect(
      compare.getByText(/SUMMER DROP → SUMMER FLAVOR DROP/),
    ).toBeVisible();
  });

  test("restores v2 by appending DRAFT v5 while preserving approved v2 and later v4", async ({
    page,
  }) => {
    await page.goto(versions);
  ...
  test("forks an exact historical version into a named branch without creating a merge", async ({
    page,
  }) => {
    await page.goto(versions);
    ...
    await expect(
      page.getByText(/dark-direction ← design-v3 · head design-v3/),
    ).toBeVisible();
  });

  test("notifies about a concurrent newer head without changing the selected compare pair", async ({
    page,
  }) => {
    await page.goto(versions);
    await page.getByLabel("Compare before").selectOption("design-v2");
    ...
    await expect(
      page.getByLabel("Version timeline").getByText("v5", { exact: true }),
    ).toBeVisible();
  });

  test("supports raster side-by-side and wipe compare modes", async ({
    page,
  }) => {
    await page.goto(versions);
    await page.getByLabel("Artifact").selectOption("artifact-hero-raster");
    await expect(page.getByLabel("Artifact")).toHaveValue(
      "artifact-hero-raster",
    );
  ...
  test("shows safe provenance identity without rendering raw private execution content", async ({
    page,
  }) => {
    await page.goto(versions);
    const timeline = page.getByLabel("Version timeline");
    await timeline
      .locator("article", { hasText: "v4" })
      .getByRole("button", { name: "Provenance" })
      .click();
    const provenance = page.getByLabel("Version provenance");
    await expect(
      provenance.getByText("run-summer-21", { exact: true }),
    ).toBeVisible();
    await expect(
      provenance.getByText("backup-image-provider", { exact: true }),
    ).toBeVisible();
    await expect(
      provenance.getByText("d".repeat(64), { exact: true }),
    ).toBeVisible();
  ...
  test("honors provenance permission restrictions", async ({ page }) => {
    ...
    const provenance = page.getByLabel("Version provenance");
    await expect(
      provenance.getByText(/Provenance access is restricted/),
    ).toBeVisible();
