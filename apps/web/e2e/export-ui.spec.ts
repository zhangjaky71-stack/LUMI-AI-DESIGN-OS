...
test.describe("NODE-60 Export Product UX", () => {
  test("locks an exact version and only shows supported format capabilities", async ({
    page,
  }) => {
    await page.goto(route);
    await expect(page.getByTestId("exact-version-lock")).toContainText(
      "artifact-summer-launch-design-v4",
    );
    await expect(page.getByTestId("exact-version-lock")).toContainText(
      "design-summer-launch-v4",
    );
  ...
  test("hides vector and batch-only options for a raster single-frame source", async ({
    page,
  }) => {
    await page.goto(route);
    ...
    await expect(page.getByTestId("format-options")).not.toContainText(
      "ZIP Batch",
    );
  });

  test("separates Crop/Scale from AI Adapt when the aspect ratio changes", async ({
    page,
  }) => {
    await page.goto(route);
    ...
    await expect(adapt).toHaveAttribute(
      "href",
      /adaptFromDesignVersion=design-summer-launch-v4/,
    );
  });

  test("creates a lifecycle-shaped job and only downloads after READY", async ({
    page,
  }) => {
    await page.goto(route);
    await page.getByTestId("create-export").click();
    await expect(page.getByText("Exact source frozen:")).toBeVisible();
    await expect(page.getByLabel("Service reported progress 100")).toBeVisible({
      timeout: 4_000,
    });
    await page.getByRole("button", { name: "Get fresh download" }).click();
    await expect(page.getByTestId("signed-download")).toContainText(
      "Signed download ready",
    );
  });

  test("refreshes an expired-style signed link without creating a new job", async ({
    page,
  }) => {
    await page.goto(route);
    await page.getByTestId("create-export").click();
  ...
  test("shows zero AI export charge and a safe job-level failure boundary", async ({
    page,
  }) => {
    await page.goto(route);
  ...
  test("batch source exposes only formats with real multi-frame support", async ({
    page,
  }) => {
    await page.goto(route);
    ...
    await expect(page.getByTestId("format-options")).toContainText(
  ...
  test("remains usable on a mobile viewport", async ({ page }) => {
    ...
    await page.goto(route);
    await expect(
      page.getByRole("heading", { name: "Export Center" }),
    ).toBeVisible();
