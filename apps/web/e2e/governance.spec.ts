...
async function openGovernance(page: import("@playwright/test").Page) {
  ...
  await expect(
    page.getByRole("heading", { name: "Audit, Retention & Data Governance" }),
  ).toBeVisible();
  await expect(page.getByText("Tenant scoped")).toBeVisible();
}

test("audit search is organization-scoped and exposes safe identity fields", async ({
  page,
}) => {
...
test("retention view lists seven classes and publishes the next immutable version", async ({
  page,
}) => {
  await openGovernance(page);
  ...
  await page
    .getByLabel("Policy note")
    .fill("Enterprise content policy revision");
...
test("legal hold blocks deletion until release and retained evidence stays counted", async ({
  page,
}) => {
  await openGovernance(page);
  ...
  await page.getByRole("button", { name: "Legal Holds" }).click();
  await page
    .getByRole("button", { name: "Release with reason + ticket" })
    .click();
  ...
  await expect(
    page.getByText("deleted 1 · anonymized 1 · retained 1"),
  ).toBeVisible();
});

test("audit export returns a fresh signed lease without creating a second render job", async ({
  page,
}) => {
  await openGovernance(page);
  ...
  await page
    .getByRole("button", { name: "Get fresh download" })
    .first()
    .click();
  const firstHref = await page
    .getByRole("link", { name: /Open fresh signed download/ })
    .getAttribute("href");
  await page
    .getByRole("button", { name: "Get fresh download" })
    .first()
