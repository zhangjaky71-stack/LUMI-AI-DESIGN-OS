...
test.describe("NODE-58 Brand Kit UI", () => {
  test("edits versioned palette tokens and warns on duplicate values", async ({
    page,
  }) => {
  ...
  test("uploads logo through governed asset lifecycle and exposes variant constraints", async ({
    page,
  }) => {
    await page.goto(brandKit);
    await page.getByRole("button", { name: "Assets & Type" }).click();
    const input = page
      .locator("label", { hasText: "Upload logo" })
    ...
    const row = page
      .getByText("campaign-secondary.svg")
  ...
  test("unknown font rights are visible and block publishing while font is active", async ({
    page,
  }) => {
    await page.goto(brandKit);
    await page.getByRole("button", { name: "Assets & Type" }).click();
    ...
    const input = page
      .locator("label", { hasText: "Upload font" })
      .locator("input[type=file]");
    await input.setInputFiles({
  ...
  test("Brand Guide import creates cited proposals that require complete human review", async ({
    page,
  }) => {
    await page.goto(brandKit);
    await page.getByRole("button", { name: "Brand Guide" }).click();
    const input = page
      .locator("label", { hasText: "Choose PDF" })
      .locator("input[type=file]");
    await input.setInputFiles({
  ...
  test("checks exact ArtifactVersion and deep-links the violation to the exact Canvas node", async ({
    page,
  }) => {
    await page.goto(brandKit);
    await page.getByRole("button", { name: "Projects & Compliance" }).click();
    ...
    await expect(
      canvas.getByRole("button", { name: "Offer Badge", exact: true }),
    ).toHaveAttribute("aria-pressed", "true");
    await expect(
      page.getByText("Compliance source v1.0.0", { exact: true }),
    ).toBeVisible();
  });

  test("rejects stale compliance versions instead of silently using the latest", async ({
    page,
  }) => {
    await page.goto(brandKit);
    await page.getByRole("button", { name: "Projects & Compliance" }).click();
  ...
  test("freezes the resolved BrandRuleSet version into a newly started Agent Run", async ({
    ...
    await expect(
      page.getByText("Brand v1.0.0 · next Run", { exact: true }),
    ).toBeVisible();
    await page
      .getByLabel("给 LUMI Agent 的指令")
      .fill("按当前 Brand Kit 创建一个方向");
    await page.getByRole("button", { name: "Send", exact: true }).click();
    await expect(
      page.getByText("Brand v1.0.0 · frozen", { exact: true }).first(),
    ).toBeVisible();
  });

  test("remains usable as a focused mobile settings surface", async ({
    page,
  }) => {
    ...
    await page.getByRole("button", { name: "Assets & Type" }).click();
    await expect(
      page.getByRole("heading", { name: "Fonts & rights" }),
    ).toBeVisible();
  });
});
