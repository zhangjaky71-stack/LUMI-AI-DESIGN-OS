...
test.describe("NODE-56 Layers / Inspector UI", () => {
  test("Layers selection is the same selection used by Canvas and Agent context", async ({
    page,
  }) => {
    await page.goto(workspace);
    await selectLayer(page, "node-headline");
    await expect(
      agent(page).getByText("1 selected", { exact: true }),
    ).toBeVisible();
    await expect(
      page
        .getByLabel("Canvas preview")
        .getByRole("button", { name: "Headline", exact: true }),
    ).toHaveAttribute("aria-pressed", "true");
    await expect(layerRow(page, "node-headline")).toHaveAttribute(
      "data-layer-selected",
      "true",
    );
  });

  test("visibility and lock edits persist through the NODE-55 autosave channel", async ({
    page,
  }) => {
    await page.goto(workspace);
    const panel = inspector(page);
    await panel
      .getByRole("button", { name: "Headline 隐藏", exact: true })
      .click();
    await expect(
      page
        .getByLabel("Canvas preview")
    ...
    await expect(
      page.getByLabel("Canvas preview").getByText("Server v8", { exact: true }),
    ).toBeVisible({ timeout: 5_000 });
    await panel
      .getByRole("button", { name: "Headline 显示", exact: true })
      .click();
    await expect(
      page
        .getByLabel("Canvas preview")
    ...
    await panel
      .getByRole("button", { name: "Headline 锁定", exact: true })
      .click();
    await expect(
      panel.getByRole("button", { name: "Headline 解锁", exact: true }),
    ).toBeVisible();
  });

  test("Design inspector edits transform and typography as semantic DesignOperations", async ({
    page,
  }) => {
    await page.goto(workspace);
    const panel = inspector(page);
    ...
    await expect(
      page
        .getByLabel("Canvas preview")
        .getByText("LUMI SUMMER", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByLabel("Canvas preview").getByText(/Server v\d+/),
    ).toBeVisible({ timeout: 5_000 });
    await expect(
      page.getByLabel("Canvas preview").getByText(/^SAVED/),
    ).toBeVisible({ timeout: 5_000 });
  });

  test("Group and Ungroup preserve editable hierarchy instead of creating UI-only grouping", async ({
    page,
  }) => {
    await page.goto(workspace);
    const panel = inspector(page);
    ...
    await expect(
      agent(page).getByText("2 selected", { exact: true }),
    ).toBeVisible();
  ...
  test("inline layer rename and z-order controls update the same document", async ({
    page,
  }) => {
    await page.goto(workspace);
    const panel = inspector(page);
    await layerRow(page, "node-headline")
    ...
    await expect(
      layerRow(page, "node-headline").getByText("Campaign Title", {
        exact: true,
      }),
    ).toBeVisible();
    await selectLayer(page, "node-headline");
  ...
  test("Context tab preserves Brand / reference selection and safe-context disclosure", async ({
    page,
  }) => {
    await page.goto(workspace);
    const panel = inspector(page);
    ...
    await expect(
      panel.getByText("hero-product.png", { exact: true }),
    ).toBeVisible();
    await expect(
      panel.getByText(
        /never exposes system prompts or private chain-of-thought/i,
      ),
    ).toBeVisible();
  });

  test("mobile Inspector tab remains usable without compressing the desktop tree", async ({
    page,
  }) => {
