...
test.describe("NODE-61 Collaboration", () => {
  test("shows same-project presence and distinguishable AI actor", async ({
    page,
  }) => {
  ...
  test("posts a mention on the exact current version", async ({ page }) => {
    ...
    await page
      ...
      .fill("@LUMI please review the protected headline treatment.");
    await page.getByTestId("mention-select").selectOption("agent-lumi");
    await page.getByTestId("post-comment").click();
    await expect(page.getByTestId("review-threads")).toContainText(
      "please review the protected headline",
    );
    await expect(page.getByTestId("review-threads")).toContainText(
  ...
  test("keeps old-version comments visible after their node is gone", async ({
    page,
  }) => {
    await page.goto(route);
  ...
  test("commits different-property edit to a new canonical version", async ({
    page,
  }) => {
    await page.goto(route);
    await expect(page.getByTestId("canonical-version")).toContainText(
      "design-summer-launch-v4",
    );
    await page.getByTestId("safe-edit").click();
    await expect(page.getByTestId("canonical-version")).toContainText(
      "design-summer-launch-v5",
    );
    await expect(page.getByTestId("notice")).toContainText(
      "canonical Design Operation API",
    );
  });

  test("reconnect conflict preserves local edit instead of silent overwrite", async ({
    page,
  }) => {
    await page.goto(route);
