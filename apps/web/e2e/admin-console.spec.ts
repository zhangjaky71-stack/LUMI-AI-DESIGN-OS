...
async function confirmAction(page: import("@playwright/test").Page) {
  ...
  await expect(
    page.getByRole("dialog", { name: "Confirm sensitive admin action" }),
  ).toBeHidden();
}
...
test("platform admin is separate, overview is service-backed and PII starts masked", async ({
  page,
}) => {
...
test("PII reveal requires reason and ticket while View-as is readonly", async ({
  page,
}) => {
  await page.getByRole("button", { name: "Users & Orgs" }).click();
...
test("provider disable and queue requeue use sensitive confirmation", async ({
  page,
}) => {
  await fillSupportContext(page);
...
test("registry and billing writes use the same guarded action panel", async ({
  page,
}) => {
  await fillSupportContext(page);
  await page.getByRole("button", { name: "Registry" }).click();
  await page
    .getByRole("button", { name: "Disable via registry service" })
    .first()
    .click();
  await confirmAction(page);
  ...
  await page
    .getByRole("button", { name: "Grant +100 credits to org-lumi" })
    .click();
