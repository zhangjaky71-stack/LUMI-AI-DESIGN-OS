...
export async function POST(request: Request): Promise<Response> {
  ...
  if (origin && origin !== requestOrigin) {
    return Response.json(
      { detail: "cross-origin telemetry forbidden" },
      { status: 403 },
    );
  }
  ...
  if (fetchSite && fetchSite !== "same-origin") {
    return Response.json(
      { detail: "cross-site telemetry forbidden" },
      { status: 403 },
    );
  }
  ...
  if (!event) {
    return Response.json(
      { detail: "invalid telemetry event" },
      { status: 422 },
    );
  }
  ...
  const logRecord = {
    ...
    level:
      event.kind.endsWith("error") || event.kind === "api_failure"
        ? "WARN"
        : "INFO",
    service: "web",
