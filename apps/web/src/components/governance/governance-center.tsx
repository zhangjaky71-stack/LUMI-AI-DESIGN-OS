...
export function GovernanceCenter({
  bootstrap,
}: Readonly<{ bootstrap: GovernanceBootstrap }>) {
  ...
  const [workspace, setWorkspace] = useState<GovernanceWorkspace | null>(
    bootstrap.workspace,
  );
  ...
  async function search(reset = true): Promise<void> {
    ...
    try {
      const page = await gateway.searchAudit(
        reset
          ? filters
          : { ...filters, cursor: workspace.audit.next_cursor ?? undefined },
      );
      setWorkspace({
        ...workspace,
        audit: reset
          ? page
          : {
              items: [...workspace.audit.items, ...page.items],
  ...
  async function createHold(): Promise<void> {
    ...
    await run(() =>
      gateway.createHold("USER", scopeId.trim(), reason.trim(), ticket.trim()),
    );
    setScopeId("");
  }
    ...
    <section className={styles.shell} aria-label="Audit and Governance Center">
      <header className={styles.header}>
        <div>
          ...
          <p>
            Append-only 审计、版本化保留策略、Legal
            Hold、数据删除和审计导出。业务日志不等于 Audit truth。
          </p>
        </div>
            ...
            {workspace.audit.items.map((event) => (
              <article className={styles.card} key={event.event_id}>
                <div className={styles.row}>
                  <strong>{auditEventLabel(event.action)}</strong>
                  <span className={styles.state}>{event.result}</span>
                </div>
                <p>
                ...
                <p>
                  ...
                  {event.resource_version ? ` · ${event.resource_version}` : ""}
                </p>
                <div className={styles.meta}>
                  <span>{event.reason_code}</span>
                  <span>{new Date(event.occurred_at).toLocaleString()}</span>
                  <code>
                    {event.trace_id ?? event.request_id ?? "no trace"}
                  </code>
                </div>
                <small>
                  Retention {event.retention_class} · policy v
                  {event.retention_policy_version} · hash{" "}
                  {event.event_hash.slice(0, 12)}…
                </small>
              </article>
            ))}
          </div>
          {workspace.audit.next_cursor ? (
            <button
              disabled={busy}
              type="button"
        ...
        <div className={styles.columns}>
          <article className={styles.card}>
            ...
            <p className={styles.notice}>
              这些天数是工程默认值，不代表任何地区的法律结论；生产上线前必须经过适用法域的法律/合规审查。
            </p>
            {latestPolicies.map((policy) => (
              <div className={styles.row} key={policy.retention_class}>
                <span>
                  {policy.retention_class} · v{policy.version}
                </span>
                <strong>{policy.retention_days} days</strong>
              </div>
            ))}
            {workspace.capabilities.can_manage_retention ? (
              <div className={styles.item}>
                ...
                <label>
                  ...
                  <select
                    value={policyClass}
                    onChange={(event) =>
                      setPolicyClass(event.target.value as RetentionClass)
                    }
                  >
                    {latestPolicies.map((policy) => (
                      <option
                        key={policy.retention_class}
                        value={policy.retention_class}
                      >
                ...
                <label>
                  ...
                  <input
                    ...
                    placeholder="Reason / policy reference"
                  />
                </label>
                <span>
                  Next exact version: v{(selectedPolicy?.version ?? 0) + 1}
                </span>
                <button
                  disabled={busy || !policyNote.trim()}
                  type="button"
                  onClick={() => void publishRetention()}
                >
          ...
          <article className={styles.card}>
            ...
            {workspace.retention_candidates.length ? (
              workspace.retention_candidates.map((candidate) => (
                <div
                  className={styles.item}
                  key={`${candidate.resource.resource_type}:${candidate.resource.resource_id}`}
                >
                  <strong>
                  ...
                  <span>
                    {candidate.resource.retention_class} · policy v
                    {candidate.policy_version}
                  </span>
                  <span>
                    Eligible{" "}
                    {new Date(candidate.eligible_at).toLocaleDateString()}
                  </span>
                </div>
              ))
            ) : (
              <p>No currently eligible resources.</p>
            )}
          </article>
        </div>
      ) : null}
      ...
      {tab === "holds" ? (
        <div className={styles.columns}>
          <article className={styles.card}>
            <h2>Active holds</h2>
            {workspace.legal_holds.length ? (
              workspace.legal_holds.map((hold) => (
                <div className={styles.item} key={hold.hold_id}>
                  <div className={styles.row}>
                    <strong>
                      {hold.hold_type} · {hold.scope_type}:{hold.scope_id}
                    </strong>
                    <span>{hold.ticket_ref}</span>
                  </div>
                  <span>{hold.reason_code}</span>
                  {workspace.capabilities.can_manage_holds ? (
                    <button
                      disabled={busy || !reason.trim() || !ticket.trim()}
                      type="button"
                      onClick={() =>
                        void run(() =>
                          gateway.releaseHold(hold.hold_id, reason, ticket),
                        )
                      }
                    >
                      Release with reason + ticket
                    </button>
                  ) : null}
                </div>
              ))
          ...
          <article className={styles.card}>
            ...
            <p>
              高权限动作。Hold 只暂停受影响资源的删除/GC，不修改原 Audit Event。
            </p>
            <label>
              User scope ID
              <input
            ...
            <button
              disabled={busy || !workspace.capabilities.can_manage_holds}
              type="button"
      ...
      {tab === "deletion" ? (
        <div className={styles.columns}>
          <article className={styles.card}>
            ...
            <p>
              流程：scope → hold check → deactivate → delete/anonymize → object
              GC → search/vector removal → completion record。
            </p>
            <label>
              Subject user ID
              <input
            ...
            <button
              disabled={busy || !workspace.capabilities.can_manage_deletion}
              type="button"
          ...
          <article className={styles.card}>
            ...
            {workspace.deletions.length ? (
              workspace.deletions.map((item) => (
                <div className={styles.item} key={item.request_id}>
                  <div className={styles.row}>
                    <strong>{item.request_id}</strong>
                    <span className={styles.state}>{item.status}</span>
                  </div>
                  <span>
                    {item.subject_user_id} · resources{" "}
                    {item.resource_refs.length}
                  </span>
                  {item.blocked_hold_ids.length ? (
                    <span className={styles.warning}>
                      Blocked by {item.blocked_hold_ids.join(", ")}
                    </span>
                  ) : null}
                  <span>
                    deleted {item.deleted_count} · anonymized{" "}
                    {item.anonymized_count} · retained {item.retained_count}
                  </span>
                  {workspace.capabilities.can_manage_deletion &&
                  !["COMPLETED", "DELETING"].includes(item.status) ? (
                    <button
                      disabled={busy}
                      type="button"
                      onClick={() =>
                        void run(() => gateway.executeDeletion(item.request_id))
                      }
                    >
                      Execute workflow
                    </button>
      ...
      {tab === "exports" ? (
        <div className={styles.columns}>
          <article className={styles.card}>
            ...
            <p>
              大型导出由异步 job 生成。持久化的是 object ref / checksum /
              size；signed URL 只在下载响应中短时返回。
            </p>
            <div className={styles.actions}>
              <button
                disabled={busy || !workspace.capabilities.can_export_audit}
            ...
            {downloadUrl && lease ? (
              <a
                ...
                rel="noreferrer"
              >
                Open fresh signed download · expires{" "}
          ...
          <article className={styles.card}>
            ...
            {workspace.exports.length ? (
              workspace.exports.map((job) => (
                <div className={styles.item} key={job.job_id}>
                  <div className={styles.row}>
                    <strong>
                      {job.export_format} · {job.job_id}
                    </strong>
                    <span className={styles.state}>{job.status}</span>
                  </div>
                  <span>{job.file_name ?? "Preparing file"}</span>
                  {job.checksum_sha256 ? (
                    <code>sha256 {job.checksum_sha256.slice(0, 16)}…</code>
                  ) : null}
                  {job.size_bytes !== null ? (
                    <span>{job.size_bytes.toLocaleString()} bytes</span>
                  ) : null}
                  {canDownload(job) ? (
                    <button
                      disabled={busy}
                      type="button"
                      onClick={() => void freshDownload(job.job_id)}
                    >
                      Get fresh download
                    </button>
                  ) : null}
      ...
      <footer className={styles.guardrail}>
        ...
        <span>
          Audit is append-only. Correction creates a new event. Signed download
          URLs are never canonical state.
        </span>
        <span>
          Deletion does not erase retained Audit/Billing evidence while
          retention or hold policy requires it.
        </span>
      </footer>
    </section>
  );
