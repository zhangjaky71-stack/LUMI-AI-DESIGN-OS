    ...
    <div className="agent-timeline" aria-label="Agent run timeline">
      ...
      <div className="agent-timeline-items" aria-live="polite" aria-relevant="additions text">
        {filtered.length ? filtered.map((item) => (
          <TimelineCard key={item.id} item={item} onOpenArtifact={onOpenArtifact} />
        )) : (
          <div className="timeline-history-empty">
            {items.length ? "No activity matches this filter." : "Durable safe events will appear here as the run progresses."}
          </div>
        )}
      </div>
    ...
    <article className={`timeline-card timeline-${item.type} status-${item.status}${current ? " is-current" : ""}`}>
      ...
      <div className="timeline-card-body">
        <div className="timeline-card-title-row">
          <div>
            <span className="timeline-type-label">{timelineTypeLabel(item.type)}</span>
            <h3>{item.label}</h3>
          </div>
          <span className={`timeline-status status-${item.status}`}>{statusLabel(item.status)}</span>
        </div>
        {summary ? <p>{summary}</p> : null}
        {item.progress ? (
          <div className="timeline-count-progress" aria-label={`${item.progress.current} of ${item.progress.total}`}>
            <div><span>Completed</span><strong>{item.progress.current}/{item.progress.total}</strong></div>
            <progress value={item.progress.current} max={item.progress.total} />
          </div>
        ) : null}
        ...
        {item.artifact ? (
          <button className="timeline-artifact-link" type="button" onClick={() => onOpenArtifact(item.artifact!)}>
            Open exact artifact {item.artifact.versionNumber ? `v${item.artifact.versionNumber}` : shortId(item.artifact.artifactVersionId)} in Canvas
          </button>
        ) : null}
...
function filterLabel(filter: AgentTimelineFilter): string {
  if (filter === "agent") return "Agent";
  if (filter === "artifact") return "Artifacts";
  if (filter === "attention") return "Attention";
  return "All";
}

function timelineGlyph(type: TimelineItemType): string {
  if (type === "approval") return "!";
  if (type === "artifact") return "A";
...
function timelineTypeLabel(type: TimelineItemType): string {
  if (type === "approval") return "Approval";
  if (type === "artifact") return "Artifact";
  if (type === "error") return "Issue";
...
function statusLabel(status: WorkspaceTimelineItem["status"]): string {
