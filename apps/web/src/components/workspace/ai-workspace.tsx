...
import { connectRunEventStream, type WorkspaceConnectionState } from "@/lib/workspace/stream";
import type { AgentRunResource, CanvasSelectionContext, ExactArtifactRef } from "@/lib/workspace/types";
...
export function AiWorkspace({ organizationId, project, initialRunId }: {
  ...
  const [runtime, setRuntime] = useState<WorkspaceRuntimeState>(() => initialWorkspaceRuntimeState());
  ...
  const refreshCanonical = useCallback(async () => {
    ...
    if (controlResult.status === "fulfilled") {
      ...
      setNotice(null);
    } else if (controlResult.reason instanceof ApiError && controlResult.reason.status === 503) {
      setNotice("Live control/event replay is not composed in this deployment. Durable run status is still refreshed from Project Core.");
    }
  ...
  useEffect(() => {
    ...
    void connectRunEventStream({
      ...
      onStreamEnd: async () => { await refreshCanonical(); },
    });
  ...
  useEffect(() => {
    if (!selectedArtifact && runtime.artifacts.length > 0) setSelectedArtifact(runtime.artifacts[runtime.artifacts.length - 1] ?? null);
  ...
  async function submitGoal(event: React.FormEvent<HTMLFormElement>) {
    ...
    setNotice(null);
    try {
      const created = await createAgentRun(organizationId, project.id, { goal: command, selection: canvasSelection }, crypto.randomUUID());
      setLastSubmittedGoal(command);
      setGoal("");
      setRun(created);
      setRunId(created.id);
      setRuntime(initialWorkspaceRuntimeState());
      router.replace(`/workspace?project=${encodeURIComponent(project.id)}&run=${encodeURIComponent(created.id)}`);
    } catch (error) {
  ...
  async function stopRun() {
    ...
    try {
      ...
      setNotice("Cancellation requested. Provider work already accepted externally may still require reconciliation.");
    } catch (error) {
  ...
  return (
    <div className="ai-workspace" data-run-status={status}>
      <header className="workspace-header">
        <div><p className="eyebrow">AI Workspace</p><h1>{project.name}</h1></div>
        <div className="workspace-run-meta" aria-live="polite">
          <ConnectionBadge state={connection} />
          <span className="workspace-status-pill">{statusLabel(status)}</span>
          {selectedArtifact ? <span className={`workspace-status-pill canvas-context-${canvasSaveState}`}>Canvas {canvasSaveState}</span> : null}
          {canStop ? <button className="workspace-ghost-button" type="button" onClick={stopRun} disabled={pending !== null}>{pending === "stop" ? "Stopping…" : "Stop"}</button> : null}
        </div>
      </header>
      ...
      <div className="workspace-grid">
        <section className="workspace-chat-panel" aria-label="Agent conversation and progress">
          <div className="workspace-panel-heading">
            <div><p className="eyebrow">Agent</p><h2>Run timeline</h2></div>
            {runId ? <code className="workspace-run-id">{shortId(runId)}</code> : null}
          </div>

          <div className="workspace-timeline-shell">
            {!runId && runtime.timeline.length === 0 ? <WorkspaceWelcome project={project} /> : null}
            {lastSubmittedGoal ? <article className="workspace-message workspace-message-user"><span>You</span><p>{lastSubmittedGoal}</p></article> : null}
            {runId ? (
              <AgentTimeline
          ...
          <form className="workspace-composer" onSubmit={submitGoal}>
            {canvasSelection && canvasSelection.nodeIds.length ? (
              <div className="workspace-context-chip">{canvasSelection.nodeIds.length} selected · document v{canvasSelection.documentVersion}</div>
            ) : selectedArtifact && canvasSaveState !== "saved" ? (
              <div className="workspace-context-chip workspace-context-chip-warning">Save or resolve Canvas {canvasSaveState} before using selection in AI</div>
            ) : (
              <div className="workspace-context-chip workspace-context-chip-muted">No canvas selection</div>
            )}
            <label htmlFor="workspace-command" className="sr-only">Tell LUMI what to design</label>
            <textarea id="workspace-command" value={goal} onChange={(event) => setGoal(event.target.value)} placeholder="Describe what you want to create or change…" rows={4} maxLength={20_000} disabled={pending !== null || canStop} />
            <div className="workspace-composer-footer">
              <span>{canStop ? "Stop the current run before starting another command." : !canvasContextReady ? "Canvas changes must be saved before the selection can enter an Agent command." : "Project and saved Canvas context are attached automatically."}</span>
              <button className="primary-button" type="submit" disabled={!goal.trim() || pending !== null || canStop || !canvasContextReady}>{pending === "send" ? "Starting…" : "Run agent"}</button>
            </div>
          </form>
        </section>
        ...
        <section className="workspace-canvas-panel" aria-label="Artifact canvas">
          <div className="workspace-panel-heading">
            <div><p className="eyebrow">Canvas</p><h2>{selectedArtifact ? selectedArtifact.label ?? "Artifact" : "Working surface"}</h2></div>
            {selectedArtifact ? <span className="workspace-version-badge">{selectedArtifact.versionNumber ? `artifact v${selectedArtifact.versionNumber}` : "exact artifact"}</span> : null}
          </div>
          <div className="workspace-stage">
            {selectedArtifact ? (
              <InfiniteCanvas organizationId={organizationId} artifactVersionId={selectedArtifact.artifactVersionId} onSelectionChange={setCanvasSelection} onSaveStateChange={setCanvasSaveState} />
            ) : (
              <div className="workspace-stage-empty"><span className="workspace-artifact-mark">+</span><h3>Artifacts will appear here.</h3><p>Only `artifact.created` events with an exact artifact version are admitted to the editable stage.</p></div>
            )}
          </div>
          {runtime.artifacts.length ? (
            <div className="workspace-artifact-strip" aria-label="Run artifacts">
              {runtime.artifacts.map((artifact) => (
                <button type="button" key={artifact.artifactVersionId} className={selectedArtifact?.artifactVersionId === artifact.artifactVersionId ? "is-selected" : ""} onClick={() => setSelectedArtifact(artifact)}>
                  <span>{artifact.label ?? "Artifact"}</span><small>{artifact.versionNumber ? `v${artifact.versionNumber}` : shortId(artifact.artifactVersionId)}</small>
                </button>
              ))}
            </div>
          ) : null}
        ...
        <aside className="workspace-inspector" aria-label="Project and run context">
          <section><p className="eyebrow">Project context</p><h2>Brief</h2><p>{project.objective || "No explicit objective recorded."}</p></section>
          ...
          <section className="workspace-run-details">
            ...
            <dl>
              ...
              <div><dt>Task</dt><dd>{runtime.control?.taskId ? shortId(runtime.control.taskId) : "—"}</dd></div>
          ...
          {selectedArtifact ? (
            <ExportPanel
              key={`export-${selectedArtifact.artifactVersionId}`}
              organizationId={organizationId}
...
function WorkspaceWelcome({ project }: { project: WorkspaceProject }) {
  return (
    <div className="workspace-welcome">
      ...
      <p>Give one clear instruction. LUMI will stream safe task/domain events here; private reasoning is never displayed.</p>
...
function InspectorList({ title, values }: { title: string; values: readonly string[] }) {
  return <section><p className="eyebrow">{title}</p>{values.length ? <ul className="workspace-inspector-list">{values.map((value, index) => <li key={`${index}-${value}`}>{value}</li>)}</ul> : <p className="workspace-muted">None recorded.</p>}</section>;
}
function ConnectionBadge({ state }: { state: WorkspaceConnectionState }) { return <span className={`workspace-connection connection-${state}`}>{state}</span>; }
function statusLabel(value: string): string { return value.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()); }
