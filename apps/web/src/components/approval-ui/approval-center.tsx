...
import {
  ...
  policyLabel,
  safeApprovalError,
} from "@/lib/approval-ui/contracts";
import type {
  ApprovalBootstrap,
  ApprovalDecision,
  ApprovalRecord,
  ApprovalWorkspace,
} from "@/lib/approval-ui/types";
...
export function ApprovalCenter({
  ...
  projectId: string;
  bootstrap: ApprovalBootstrap;
}) {
  const gateway = useMemo(() => createApprovalGateway(bootstrap), [bootstrap]);
  const [workspace, setWorkspace] = useState<ApprovalWorkspace | null>(
    bootstrap.workspace,
  );
  ...
  if (!workspace) {
    return (
      <main className={styles.page}>
        <div className={styles.loading}>Loading canonical approvals…</div>
      </main>
    );
  }
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.lede}>
            Decisions are locked to an exact subject version. A newer
            ArtifactVersion never inherits an older approval.
          </p>
        </div>
      ...
      <section className={styles.truth} data-testid="approval-truth-boundary">
        ...
        <span>
          Server permission + exact version + status + expiry + run-resume
          checks execute atomically before Graph resume.
        </span>
      </section>
      ...
      <section className={styles.list}>
        {items.length === 0 && (
          <div className={styles.empty}>No approvals in this view.</div>
        )}
        {items.map((approval) => {
          const waiting = approval.status === "PENDING";
          const actionDisabled =
            !workspace.can_decide || busy === approval.approval_id;
          return (
            <article
            ...
            >
              <div className={styles.cardTop}>
                <div>
                  <span className={styles.type}>
                    {approval.approval_type.replaceAll("_", " ")}
                  </span>
                  <h2>{approval.payload_summary}</h2>
                </div>
                <span
                  className={`${styles.status} ${styles[`status${approval.status}`]}`}
                >
              ...
              <div className={styles.subject} data-testid="exact-subject">
                ...
                <code>
                  {approval.subject.subject_type} /{" "}
                  {approval.subject.subject_id}
                </code>
              ...
              <dl className={styles.meta}>
                <div>
                  <dt>Policy</dt>
                  <dd>
                    {policyLabel(approval)} · policy v{approval.policy.version}
                  </dd>
                </div>
                <div>
                  <dt>Permission</dt>
                  <dd>{approval.policy.required_permission}</dd>
                </div>
                <div>
                  <dt>Requested by</dt>
              ...
              {approval.feedback && (
                <div className={styles.feedback}>
                  ...
                  <p>{approval.feedback.comment}</p>
                  {approval.feedback.requested_changes.map((item) => (
              ...
              {waiting && (
                <div className={styles.actions}>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setChangesFor(
                        changesFor === approval.approval_id
                          ? null
              ...
              {changesFor === approval.approval_id && waiting && (
                <div className={styles.changeBox}>
                  <label htmlFor={`changes-${approval.approval_id}`}>
                    Structured feedback
                  </label>
                  <textarea
                    id={`changes-${approval.approval_id}`}
                    value={feedback}
                    onChange={(event) => setFeedback(event.target.value)}
                    placeholder="Describe the change. Node/region references are preserved by the API when supplied from Canvas."
                  />
                  <button
                    disabled={actionDisabled || !feedback.trim()}
                    onClick={() => void decide(approval, "REQUEST_CHANGES")}
                    className={styles.primary}
                  >
                    Send changes to workflow
                  </button>
                </div>
              )}
              ...
              <footer className={styles.footer}>
                <span>
                  {approval.decisions.length} decision record
                  {approval.decisions.length === 1 ? "" : "s"}
                </span>
