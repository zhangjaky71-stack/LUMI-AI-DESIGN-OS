...
export function ProjectDetail({
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await queryCache.fetchQuery(
        ...
        (signal) =>
          gateway.getProject(activeOrganization.id, projectId, signal),
        10_000,
      );
      setDetail(result);
    } catch (loadError) {
  ...
  const saveBrief = async () => {
    ...
    try {
      ...
      setDraft(null);
      setNotice(
        `Structured Brief 已保存为 v${result.brief_version}。历史版本仍可追溯。`,
      );
    } catch (saveError) {
    ...
    <div className={styles.projectDetailPage}>
        ...
        <div>
          <div className={styles.detailStatusRow}>
            <span
              ...
              data-status={detail.summary.status}
            >
          ...
          <p>
            {detail.summary.brand?.name ?? "未绑定 Brand"} ·{" "}
            {detail.summary.artifact_count} Artifacts ·{" "}
            {detail.summary.active_run_count} Active Runs
          </p>
        </div>
      ...
      <div className={styles.detailGrid}>
        <section
          ...
          aria-labelledby="structured-brief-title"
        >
          ...
          {editing && draft ? (
            ...
            <dl className={styles.briefDefinition}>
              <div>
                <dt>Objective</dt>
                <dd>{detail.brief.objective || "—"}</dd>
              </div>
              <div>
                <dt>Audience</dt>
                <dd>{detail.brief.audience || "—"}</dd>
              </div>
              <div>
                <dt>Deliverables</dt>
                <dd>
                  {detail.brief.deliverables.length
                    ? detail.brief.deliverables.join(" · ")
                    : "待确认"}
                </dd>
              </div>
              <div>
                <dt>Constraints</dt>
                <dd>
                  {detail.brief.constraints.length
                    ? detail.brief.constraints.join(" · ")
                    : "暂无"}
                </dd>
              </div>
              <div>
                <dt>Assumptions</dt>
                <dd>
                  {detail.brief.assumptions.length
                    ? detail.brief.assumptions.join(" · ")
                    : "暂无"}
                </dd>
              </div>
              <div>
                <dt>Locale</dt>
                <dd>{detail.brief.locale}</dd>
              </div>
            </dl>
          )}
        </section>
          ...
          >
            ...
            {detail.references.length === 0 ? (
              <p className={styles.mutedText}>
                还没有项目参考素材。后续可从 Project Dashboard 或 Workspace
                继续添加。
              </p>
            ) : (
                ...
                {detail.references.map((reference) => (
                  <div key={reference.id} className={styles.detailReferenceRow}>
                    ...
                    <span data-status={reference.scan_status}>
                      {reference.scan_status === "REJECTED"
                        ? `不可用 · ${reference.failure_code ?? "REJECTED"}`
                        : reference.scan_status}
                    </span>
                  </div>
                ))}
              </div>
