...
} from "@/lib/agent-timeline/projector";
...
export function AgentTimeline({
  ...
  const renderItem = (item: TimelineItem) => {
    ...
    const actionable = approval
      ? isApprovalActionable(approval, snapshot.run)
      : false;
    const stale = Boolean(
      approval &&
        !actionable &&
    ...
    return (
      ...
      >
        ...
        <div className={styles.itemBody}>
          ...
          {item.safe_summary ? (
            <p className={styles.summary}>{item.safe_summary}</p>
          ) : null}

          {item.progress ? (
            <div
              className={styles.progress}
              aria-label={`${item.label} progress ${item.progress.label}`}
            >
              <div>
                <span
                  style={{
                    width: `${Math.round((item.progress.completed / item.progress.total) * 100)}%`,
                  }}
                />
              </div>
          ...
          {item.error ? (
            <div className={styles.errorBox}>
              <div>
                ...
                <span>{item.error.safe_message}</span>
              </div>
              {item.error.provider_fallback ? (
                <p>Fallback: {item.error.provider_fallback}</p>
              ) : null}
              {item.error.request_id ? (
                <p>Request {item.error.request_id}</p>
              ) : null}
              {item.error.retrying ? <p>正在自动重试。</p> : null}
              {item.task_id &&
              snapshot.run?.tasks.find((task) => task.task_id === item.task_id)
                ?.retryable ? (
                <button
                  type="button"
                  onClick={() => onRetryTask(item.task_id!)}
                  disabled={busy}
                >
                  Retry {item.label}
                </button>
              ...
              <div>
                ...
                <div className={styles.actions}>
                  <button
                    type="button"
                    onClick={() =>
                      onPlaceArtifact(artifact.artifact_id, artifact.version_id)
                    }
                    disabled={busy}
                  >
                    放到 Canvas
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      onToggleArtifactReference(artifact.version_id)
                    }
                    aria-pressed={artifactReferenceIds.includes(
                      artifact.version_id,
                    )}
                  >
                    作为参考
                  ...
                  <button
                    type="button"
                    onClick={() => onJumpToCanvas(artifact.version_id)}
                  >
                    查看 Canvas
                  </button>
                </div>
          ...
          {approval ? (
            <div className={styles.approvalCard} data-actionable={actionable}>
              <div className={styles.approvalHeading}>
                <div>
                  <span className={styles.micro}>APPROVAL</span>
              ...
              {approval.impact ? (
                <p className={styles.muted}>{approval.impact}</p>
              ) : null}
              {money(approval.estimated_cost_microusd) ? (
                <p>预计增量成本 {money(approval.estimated_cost_microusd)}</p>
              ) : null}
              {stale ? (
                <p className={styles.stale}>
                  旧审批不会被提交；请以当前 Run 的 canonical state 为准。
                </p>
              ) : null}
                ...
                <>
                  ...
                  <div className={styles.actions}>
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        onDecideApproval(approval, "REQUEST_CHANGES")
                      }
                      disabled={!actionable || busy}
                    >
                      Request Changes
          ...
          {item.cost && item.type !== "APPROVAL" ? (
            <details className={styles.itemCost}>
              ...
              <span>
                estimated {money(item.cost.estimated_microusd) ?? "—"}
              </span>
              <span>actual {money(item.cost.actual_microusd) ?? "—"}</span>
              {item.cost.credits ? (
                <span>{item.cost.credits} credits</span>
              ) : null}
            </details>
          ) : null}
        </div>
    ...
    <div className={styles.timeline} aria-label="Agent Timeline">
      <div className={styles.timelineHeader}>
        <div>
          ...
          <strong>
            {projection.run_id ? `Run ${projection.run_status}` : "Ready"}
          </strong>
        </div>
        {projection.cost ? (
          <button
          ...
          >
            Cost{" "}
            {money(projection.cost.actual_microusd) ??
              money(projection.cost.estimated_microusd) ??
              "—"}
          </button>
        ) : null}
      </div>
      ...
      {costOpen && projection.cost ? (
        <div className={styles.costPanel}>
          <span>
            Estimated{" "}
            <strong>{money(projection.cost.estimated_microusd) ?? "—"}</strong>
          </span>
          <span>
            Actual{" "}
            <strong>{money(projection.cost.actual_microusd) ?? "—"}</strong>
          </span>
          {projection.cost.credits ? (
            <span>
              Credits <strong>{projection.cost.credits}</strong>
            </span>
          ) : null}
          {projection.cost.budget_warning ? (
            <strong className={styles.budgetWarning}>Budget warning</strong>
          ) : null}
        </div>
      ) : null}
      ...
      <div className={styles.items} aria-live="polite">
        {normal.length ? (
          ...
          <p className={styles.empty}>当前筛选下没有 Timeline 事件。</p>
        )}
      </div>

      <p className={styles.safety}>
        Timeline 只展示 TaskGraph / Run / safe tool summaries；不展示 system
        prompt、内部 chain-of-thought、secret tool payload 或 stack trace。
      </p>
    </div>
  );
}
