...
import {
  CanvasAutosaveBuffer,
  rebaseOperationsVersion,
} from "@/lib/infinite-canvas/autosave";
...
function isConflict(error: unknown): boolean {
  return (
    error instanceof LumiApiError &&
    error.problem.code === "DOCUMENT_VERSION_CONFLICT"
  );
}
...
export function InfiniteCanvasProduct({
  ...
  const gateway = useMemo(
    () => getInfiniteCanvasGateway(api, bootstrap),
    [api, bootstrap],
  );
  ...
  const emitEditorState = useCallback(
    (state: CanvasSyncState) => {
      ...
      if (!snapshot) return;
      onEditorStateChange(
        buildCanvasEditorState(snapshot, serverVersionRef.current, state),
      );
    },
    [onEditorStateChange],
  );
  ...
  const flushPending = useCallback(async () => {
    ...
    try {
      ...
      const nextState: CanvasSyncState = autosaveRef.current.size
        ? "DIRTY"
        : "SAVED";
      ...
      const nextState: CanvasSyncState = isConflict(error)
        ? "CONFLICT"
        : online
          ? "DIRTY"
          : "OFFLINE";
  ...
  const applyLocalOperations = useCallback(
    (label: string, operations: readonly DesignOperation[]) => {
      ...
      const prepared = rebaseOperationsVersion(
        operations,
        getDocumentVersion(before),
        `local-${label}`,
      );
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      void gateway
        ...
        .catch((error) => {
          if (!cancelled)
            setNotice(`Canvas load failed: ${errorMessage(error)}`);
        })
        .finally(() => {
  ...
  useEffect(() => {
    const handleOnline = () => {
      setOnline(true);
      const state: CanvasSyncState = autosaveRef.current.size
        ? "DIRTY"
        : "SAVED";
      setSyncState(state);
      emitContext(state);
  ...
  const beginNodeDrag = (
    ...
    const controller = controllerRef.current;
    if (!controller) return;
    if (!controller.selection.snapshot().ids.includes(node.id))
      controller.selection.set([node.id], node.id);
  ...
  const moveNodeDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    drag.transform.previewMove(
      dx / controller.snapshot().camera.zoom,
      dy / controller.snapshot().camera.zoom,
    );
  };
  ...
  const beginPan = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const background =
      ...
      target.dataset.canvasWorld === "true";
    if (!background) return;
    if (tool === "select" && !spaceHeldRef.current && event.button === 0)
      updateSelection([]);
  ...
  const arrange = (nodeId: string, direction: "forward" | "back") => {
    ...
    const next =
      direction === "forward"
        ? Math.min(siblings.length - 1, current + 1)
  ...
  const duplicate = (ids?: readonly string[]) => {
    ...
    applyLocalOperations(
      ...
      duplicateOperations(
        ...
        getDocumentVersion(snapshot.document),
      ),
    );
    setContextMenu(null);
  };
  ...
  const removeSelection = () => {
    ...
    if (!controller) return;
    const snapshot = controller.snapshot();
    const ids = snapshot.selection.ids.filter(
      (id) => !snapshot.scene.nodes.get(id)?.locked,
    );
  ...
  const setProperty = useCallback(
    (
      ...
      return applyLocalOperations(label, [
        propertyOperation(document, nodeIds, path, value, label),
      ]);
    },
    [applyLocalOperations],
  );
  ...
  const renameNode = (nodeId: string, name: string): boolean => {
    ...
    return applyLocalOperations("inspector-rename", [
      renameOperation(document, nodeId, name),
    ]);
  };
  ...
  const setTransform = (
    ...
    return applyLocalOperations(
      "inspector-transform",
      transformOperations(document, nodeId, patch),
    );
  };
  ...
  const setText = (nodeId: string, patch: InspectorTextPatch): boolean => {
    ...
    return applyLocalOperations(
      "inspector-text",
      textOperations(document, nodeId, patch),
    );
  };
  ...
  const groupSelection = (): boolean => {
    ...
    if (!result || !applyLocalOperations("layers-group", result.operations))
      return false;
  ...
  const ungroupSelection = (): boolean => {
    ...
    if (!result || !applyLocalOperations("layers-ungroup", result.operations))
      return false;
  ...
  const editorApi: CanvasEditorApi = {
    select: (ids, primaryId) =>
      updateSelection(ids, primaryId ?? ids[0] ?? null),
    renameNode,
    setVisibility: (nodeIds, visible) =>
      setProperty("layers-visible", nodeIds, "visible", visible),
    setLocked: (nodeIds, locked) =>
      setProperty("layers-lock", nodeIds, "locked", locked),
    setOpacity: (nodeIds, opacity) =>
      setProperty(
        "inspector-opacity",
        nodeIds,
        "opacity",
    ...
    setBlendMode: (nodeIds, blendMode) =>
      setProperty("inspector-blend", nodeIds, "blend_mode", blendMode),
    setFill: (nodeId, fill) =>
      setProperty("inspector-fill", [nodeId], "metadata.fill", fill),
    setTransform,
    setText,
    moveLayer,
    groupSelection,
  ...
  const undo = () => {
    ...
    const operations = rebaseOperationsVersion(
      entry.inverse,
  ...
  const rebasePending = async () => {
    ...
    if (!pending) return;
    try {
      const canonical = await gateway.getDocument(
        activeOrganization.id,
        projectId,
      );
      ...
      if (!replay.ok) {
        setNotice(
          `Rebase failed: ${replay.failures[0]?.message ?? "operation rejected"}`,
        );
        return;
      }
      autosaveRef.current.clear();
      ...
      setSyncState("DIRTY");
      setNotice(
        "Local commands 已基于最新 canonical version 重放，准备重新保存。",
      );
      syncRuntime();
      emitContext("DIRTY");
      scheduleFlush();
  ...
  const dropPayload = (event: ReactDragEvent<HTMLDivElement>) => {
    ...
    if (event.dataTransfer.files.length) {
      setNotice(
        `${event.dataTransfer.files.length} 个系统文件必须先进入 Asset lifecycle；未伪造为已上传资源。`,
      );
      return;
    }
    const rect = viewport.getBoundingClientRect();
    ...
    const artifactRaw = event.dataTransfer.getData(
    ...
    try {
      if (assetRaw) {
        const payload = JSON.parse(assetRaw) as {
          asset_id?: string;
          file_name?: string;
        };
        ...
        const payload = JSON.parse(artifactRaw) as {
  ...
  if (loading || !runtime)
    return (
      <div className={styles.loading}>正在启动 Infinite Canvas runtime…</div>
    );
  ...
  const primary = runtime.selection.primary_id
    ? (runtime.scene.nodes.get(runtime.selection.primary_id) ?? null)
    : null;
    ...
    <div className={styles.productCanvas}>
      <div className={styles.topToolbar}>
        ...
        <div className={styles.toolGroup}>
          ...
          <button
            ...
            disabled={!runtime.selection.ids.length}
          >
            Fit selection
          </button>
          <button
            type="button"
            onClick={() => {
              controllerRef.current?.fitAll(72);
              syncRuntime();
            }}
          >
            Fit all
        ...
        <div className={styles.toolGroup}>
          <button
            ...
            onClick={() => {
              ...
              if (c) {
                ...
                c.setCamera({
                  ...s.camera,
                  zoom: Math.max(0.05, s.camera.zoom / 1.2),
                });
                syncRuntime();
              }
            }}
          ...
          <button
            ...
            onClick={() => {
              ...
              if (c) {
                ...
                c.setCamera({
                  ...s.camera,
                  zoom: Math.min(8, s.camera.zoom * 1.2),
                });
                syncRuntime();
              }
            }}
          ...
          <span className={styles.syncBadge} data-state={syncState}>
      ...
      {primary ? (
        <div className={styles.contextToolbar}>
          <strong>
            {runtime.document.nodes[primary.id]?.name ?? primary.id}
          </strong>
          <span>X {Math.round(primary.world_bounds.x)}</span>
          <span>Y {Math.round(primary.world_bounds.y)}</span>
          <span>W {Math.round(primary.world_bounds.width)}</span>
          <span>H {Math.round(primary.world_bounds.height)}</span>
          <button
            type="button"
            onClick={() => setLock(primary.id, !primary.locked)}
          >
            {primary.locked ? "Unlock" : "Lock"}
          </button>
          <button
            type="button"
            onClick={() => arrange(primary.id, "back")}
            disabled={!primary.parent_id}
          >
            Back
          </button>
          <button
            type="button"
            onClick={() => arrange(primary.id, "forward")}
            disabled={!primary.parent_id}
          >
            Forward
          </button>
          <button type="button" onClick={() => onAIEdit(runtime.selection.ids)}>
            AI Edit
          </button>
        </div>
      ) : null}
      ...
      {syncState === "CONFLICT" ? (
        <div className={styles.conflictBar} role="alert">
          <span>
            Document version conflict：未提交 commands
            保留在内存中，不会覆盖服务器版本。
          </span>
          <button type="button" onClick={() => void rebasePending()}>
            Rebase local commands
          </button>
          <button type="button" onClick={() => void reloadCanonical()}>
            Reload canonical
          </button>
        </div>
      ) : null}
      ...
      <div className={styles.canvasBody}>
          ...
          <aside className={styles.assetTray} aria-label="Canvas drag sources">
            ...
            {references.map((reference) => (
              ...
              >
                ...
                <small>{reference.scan_status}</small>
              </div>
            ))}
            {artifacts.map((artifact) => (
              ...
              >
                ...
                <small>{artifact.version_id}</small>
              </div>
            ))}
          </aside>
        ) : null}
        ...
        >
          <div
            ...
            style={{
              transform: `translate(${-runtime.camera.x * runtime.camera.zoom}px, ${-runtime.camera.y * runtime.camera.zoom}px) scale(${runtime.camera.zoom})`,
            }}
          >
            {visible.map((node) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    mixBlendMode: (documentNode?.blend_mode ??
                      "normal") as React.CSSProperties["mixBlendMode"],
                    ...fillStyle,
                    ...(dragging
                      ? {
                          transform: `translate(${dragDelta.x / runtime.camera.zoom}px, ${dragDelta.y / runtime.camera.zoom}px)`,
                        }
                      : {}),
                  }}
                  onClick={(event) => {
                ...
                >
                  ...
                  {node.kind === "IMAGE" ? (
                    <span className={styles.imageNode}>
                      <b>{node.asset_id ? "ASSET" : "ARTIFACT"}</b>
                      <small>{documentNode?.name ?? node.id}</small>
                    </span>
                  ) : null}
                  {node.kind === "SHAPE" && label ? (
                    <span className={styles.shapeLabel}>{label}</span>
                  ) : null}
                  {node.locked ? (
                    <span className={styles.lockBadge}>LOCK</span>
                  ) : null}
                </div>
              );
            })}
          ...
          {contextMenu ? (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => duplicate([contextMenu.nodeId])}
              >
                Duplicate
              </button>
              <button
                ...
                onClick={() =>
                  setLock(
                    contextMenu.nodeId,
                    !Boolean(
                      runtime.scene.nodes.get(contextMenu.nodeId)?.locked,
                    ),
                  )
                }
              >
              ...
              <button
                type="button"
                onClick={() => arrange(contextMenu.nodeId, "forward")}
              >
                Bring forward
              </button>
              <button
                type="button"
                onClick={() => arrange(contextMenu.nodeId, "back")}
              >
                Send backward
              </button>
              <button
              ...
              <button
                ...
                onClick={() => {
                  updateSelection([contextMenu.nodeId]);
                  removeSelection();
                  setContextMenu(null);
                }}
              >
          ...
          <div className={styles.frameNavigator}>
            {runtime.scene.frame_ids.map((frameId) => (
              <button
                ...
                type="button"
                onClick={() => {
                  controllerRef.current?.fitFrame(frameId, 72);
                  syncRuntime();
                }}
              >
          ...
          <div className={styles.versionHud}>
            ...
            <span>
              {visible.length}/{sceneNodes.length} visible
            </span>
          </div>
        </div>
      </div>
