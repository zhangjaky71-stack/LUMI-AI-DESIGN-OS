...
export function subscriptionKeepsEntitlements(
  subscription: BillingSubscription | null,
): boolean {
  return (
    subscription !== null &&
    ["TRIALING", "ACTIVE", "CANCEL_AT_PERIOD_END"].includes(subscription.state)
  );
}
...
export function entitlementRows(
  ...
  return Object.entries(workspace.entitlements).map(
...
export function safeBillingError(error: unknown): string {
  ...
  switch (code) {
    case "BILLING_FORBIDDEN":
      return "You do not have permission to manage billing for this organization.";
    case "BILLING_INSUFFICIENT_CREDITS":
      return "Not enough credits are available for this operation.";
    case "BILLING_WEBHOOK_SIGNATURE_INVALID":
      return "The payment event could not be verified.";
    case "BILLING_PLAN_VERSION_NOT_AVAILABLE":
      return "That plan version is no longer available for new checkout sessions.";
    default:
      return "Billing could not complete the request. Retry or contact an organization owner.";
  }
}
