  ...
  return {
    ...
    nodes: {
      ...
      text: {
        ...
        children: [],
        content: "Hello",
        transform: { x: 20, y: 300, width: 300, height: 80 },
...
describe("Layers / Inspector DesignOperation builders", () => {
  ...
  it("ungroups a zero-rotation group and restores child coordinates", () => {
    ...
    expect(ungrouped.document.nodes.a?.transform).toMatchObject({
      x: 40,
      y: 50,
    });
    expect(ungrouped.document.nodes.b?.transform).toMatchObject({
      x: 200,
      y: 100,
    });
  });
  ...
  it("builds transform and typography edits as versioned semantic operations", () => {
    ...
    const ops = [
      ...transformOperations(source, "a", {
        x: 88,
        width: 140,
        rotation_deg: 15,
      }),
      ...textOperations(source, "text", {
        content: "Summer",
        font_size: 42,
        text_align: "center",
        fill: "#222222",
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.document.nodes.a?.transform).toMatchObject({
      x: 88,
      width: 140,
      rotation_deg: 15,
    });
    expect(result.document.nodes.text?.content).toBe("Summer");
    expect(result.document.nodes.text?.metadata).toMatchObject({
      font_size: 42,
      text_align: "center",
      fill: "#222222",
    });
