...
export function assertExactVersionId(value: string, field: string): void {
  if (!value.trim() || FLOATING_VERSION.test(value.trim()))
    throw exportProblem(`EXPORT_${field.toUpperCase()}_MUST_BE_EXACT`);
}
...
export function capabilitiesForSource(
  ...
  return capabilities.filter((capability) => {
    ...
    if (source.frame_ids.length > 1 && !capability.supports_multi_frame)
      return false;
    if (capability.format === "ZIP" && source.frame_ids.length < 2)
      return false;
    return true;
  });
}

export function hasAspectRatioChange(
  source: ExportSourceOption,
  width: number,
  height: number,
): boolean {
...
export function estimateExport(
  ...
  return {
    ...
    render_label:
      batch || format === "PDF" || format === "ZIP"
        ? "Server render / packaging"
        : "Server render",
...
export function buildExportSpec(args: {
  ...
  assertExactVersionId(
    draft.source.design_document_version_id,
    "design_version_id",
  );
  if (draft.target_width <= 0 || draft.target_height <= 0)
    throw exportProblem("EXPORT_DIMENSIONS_INVALID");
  if (draft.format === "JPEG" && draft.transparent_background)
    throw exportProblem("EXPORT_JPEG_ALPHA_UNSUPPORTED");
  if (draft.quality !== null && (draft.quality < 1 || draft.quality > 100))
    throw exportProblem("EXPORT_QUALITY_INVALID");
    ...
    variants: [
      {
        ...
        ...(draft.format === "PNG" || draft.format === "WEBP"
...
export function safeExportError(code: string | undefined): string {
  ...
  let message: string;
  switch (safeCode) {
    case "EXPORT_EXACT_SOURCE_NOT_FOUND":
      message =
        "The exact source version is no longer available to this request.";
      break;
    case "EXPORT_DOWNLOAD_FORBIDDEN":
      message = "Download permission changed. Ask a project owner for access.";
      break;
    case "EXPORT_JOB_EXPIRED":
      message =
        "This export expired. Create a new export from the same exact source version.";
      break;
    case "EXPORT_STORAGE_OBJECT_NOT_FOUND":
      message = "The verified export file is unavailable in storage.";
      break;
    case "EXPORT_VIDEO_NODE_REQUIRES_NODE_48":
      message = "This selection contains video. Use the video export workflow.";
      break;
    default:
      message =
        "Export could not be completed without exposing internal details.";
  }
