  ...
  sources: [
    {
      ...
      design_document_version_id: "design-v4",
      frame_ids: ["frame-1"],
      width: 1080,
      height: 1350,
...
const spec: ExportSpec = {
  ...
  filename_template: "x-{variant}.{ext}",
...
describe("Deterministic NODE-60 gateway", () => {
  it("advances only through canonical job states and then exposes READY file", async () => {
    ...
    for (let index = 0; index < 4; index += 1)
      job = await gateway.getExport(job.export_job_id);
  ...
  it("refreshes a signed URL without rerendering", async () => {
    ...
    for (let index = 0; index < 4; index += 1)
      job = await gateway.getExport(job.export_job_id);
    const first = await gateway.getDownload(
      job.export_job_id,
      job.files[0]!.file_id,
    );
    const second = await gateway.getDownload(
      job.export_job_id,
      job.files[0]!.file_id,
    );
  ...
  it("rejects floating exact-version aliases", async () => {
    const gateway = new DeterministicExportGateway(workspace);
    await expect(
      gateway.createExport({
        ...spec,
        artifact_version_id: "latest",
        operation_id: "op-2",
      }),
    ).rejects.toThrow("EXPORT_VERSION_MUST_BE_EXACT");
  });
});
