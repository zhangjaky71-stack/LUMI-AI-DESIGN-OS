...
const source: ExportSourceOption = {
  ...
  approved: true,
  brand_rule_set_version: "brand-v2",
};
...
describe("NODE-60 export contracts", () => {
  ...
  it("rejects floating version aliases", () => {
    expect(() =>
      buildExportSpec({
        ...
        actorId: "user-1",
        operationId: "op-1",
        draft: draft({ source: { ...source, artifact_version_id: "latest" } }),
      }),
  ...
  it("only exposes multi-frame formats for batch source", () => {
    const caps = capabilitiesForSource(
      { ...source, frame_ids: ["a", "b"] },
      VERIFIED_EXPORT_CAPABILITIES,
    );
    expect(caps.map((item) => item.format)).toEqual([
      "PDF",
      "ZIP",
      "LUMI_PACKAGE",
    ]);
  ...
  it("rejects JPEG alpha", () => {
    ...
    ).toThrow("EXPORT_JPEG_ALPHA_UNSUPPORTED");
  });
  ...
  it("shows opaque request IDs without exposing raw payloads", () => {
    expect(safeExportError("EXPORT_DOWNLOAD_FORBIDDEN::request:req-1234")).toBe(
      "Download permission changed. Ask a project owner for access. Request ID: req-1234",
    );
  });
});
