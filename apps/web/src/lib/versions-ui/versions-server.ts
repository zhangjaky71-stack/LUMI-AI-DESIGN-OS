...
function version(
  ...
  return {
    ...
    design_document_version_id:
      artifactId === "artifact-campaign-canvas"
...
function provenance(
  ...
  return {
    ...
    prompt_template_version:
      options.createdBy === "AGENT" ? "visual-direction@3" : "manual-edit@1",
    ...
    skill_versions:
      options.createdBy === "AGENT"
        ? { composition: "2.1.0", brand: "1.4.2" }
        : {},
  };
}
...
export function versionsSeed(projectId: string): VersionsBootstrapSeed {
  ...
  const changes: Record<string, readonly VersionSemanticChange[]> = {
    "design-v1": [
      {
        id: "c-v1-create",
        kind: "OTHER",
        label: "Initial campaign composition",
        node_id: null,
        node_name: null,
        property: "document",
        before: null,
    ...
    "design-v2": [
      ...
      },
      {
        id: "c-v2-product",
        kind: "IDENTITY",
        label: "Product identity unchanged",
        node_id: "node-hero-product",
        node_name: "Hero Product",
        property: "asset_id",
        before: "asset-lumi-product",
        after: "asset-lumi-product",
    ...
    "design-v3": [
      {
        id: "c-v3-bg",
        kind: "STYLE",
        label: "Background warm neutral→charcoal",
        node_id: "frame-feed",
        node_name: "Feed / 4:5",
        property: "fill",
        before: "#F3EBDD",
    ...
    "design-v4": [
      {
        id: "c-v4-offer",
        kind: "LAYOUT",
        label: "Offer badge moved +24px",
        node_id: "node-offer",
        node_name: "Offer Badge",
        property: "x",
        before: 920,
        after: 944,
        protected_identity: false,
      },
      {
        id: "c-v4-title",
        kind: "TEXT",
        label: "Headline copy refined",
        node_id: "node-headline",
        node_name: "Headline",
        property: "text",
        before: "SUMMER DROP",
        after: "SUMMER FLAVOR DROP",
        protected_identity: false,
      },
    ],
    "raster-v1": [
    ...
    "raster-v2": [
      {
        id: "r2",
        kind: "STYLE",
        label: "Lighting softened",
        node_id: null,
        node_name: null,
        property: "lighting",
        before: "hard side light",
        after: "soft daylight",
        protected_identity: false,
      },
    ],
    "raster-v3": [
      {
        id: "r3",
        kind: "STYLE",
        label: "Background shifted darker",
        node_id: null,
        node_name: null,
        property: "background",
        before: "warm oat",
  ...
  const previews: Record<string, VersionPreview> = {
    "design-v1": {
      kind: "DESIGN_IR",
      label: "4:5 campaign · oat",
      width: 1080,
      height: 1350,
      background: "#F3EBDD",
      accent: "#D9A441",
      secondary: "#1C1917",
      image_asset_id: "asset-lumi-product",
    },
    "design-v2": {
      kind: "DESIGN_IR",
      label: "4:5 campaign · approved",
      width: 1080,
      height: 1350,
      background: "#F3EBDD",
      accent: "#D9A441",
    ...
    "raster-v1": {
      ...
      accent: "#D9A441",
      secondary: "#2D2926",
      image_asset_id: "asset-render-v1",
    },
    "raster-v2": {
      kind: "RASTER",
      label: "Render v2 · approved",
      width: 1080,
      height: 1350,
      background: "#F3EBDD",
      accent: "#E4B85D",
      secondary: "#1C1917",
      image_asset_id: "asset-render-v2",
    },
    "raster-v3": {
      kind: "RASTER",
      label: "Render v3 · dark",
      width: 1080,
      height: 1350,
      background: "#201D1B",
      accent: "#D9A441",
      secondary: "#F3EBDD",
      image_asset_id: "asset-render-v3",
    },
  };
  const approval: Record<string, VersionApprovalSummary> = Object.fromEntries(
    versions.map((item) => [
      item.id,
      {
        status: item.status,
        approved_by:
          item.status === "APPROVED" ? "user:creative-director" : null,
        approved_at:
          item.status === "APPROVED" ? "2026-08-14T09:05:00.000Z" : null,
        validation_label:
          item.status === "APPROVED"
            ? "Brand + identity + visual QA passed"
            : null,
      },
    ]),
  );
        ...
        label:
          item.quality_score == null
            ...
            : item.quality_score >= 94
              ? "Excellent"
              : item.quality_score >= 88
                ? "Ready"
                : "Review",
        checks:
          item.status === "APPROVED"
            ? ["Brand compliance", "Identity lock", "Visual critic"]
            : ["Constraint preflight", "Identity lock"],
      },
    ]),
  );
  const safeSummaries: Record<string, string> = {
    ...
    "design-v2":
      "Typography refined while protected product identity stayed unchanged.",
  ...
  const provenanceRows: ArtifactProvenance[] = [
    ...
    provenance("design-v2", {
      createdBy: "AGENT",
      model: "gpt-image-design-router",
      provider: "primary-image-provider",
      run: "run-summer-17",
      task: "task-composition",
      promptHash: H.b,
      recipe: "campaign-kv@4",
      inputs: ["design-v1"],
    }),
    provenance("design-v3", { createdBy: "USER", inputs: ["design-v2"] }),
    provenance("design-v4", {
      createdBy: "AGENT",
      model: "gpt-image-design-router",
      provider: "backup-image-provider",
      run: "run-summer-21",
      task: "task-layout-repair",
      promptHash: H.d,
      recipe: "campaign-kv@4",
      inputs: ["design-v3"],
    }),
    provenance("raster-v1", {
      createdBy: "AGENT",
      model: "renderer-v2",
      provider: "lumi-render",
      run: "run-summer-17",
      task: "task-render",
      promptHash: H.a,
      recipe: "render@2",
...
export function getVersionsBootstrap(projectId: string): VersionsBootstrap {
  const e2e =
    ...
    process.env.LUMI_VERSIONS_E2E === "1";
  return e2e
    ? { mode: "e2e", seed: versionsSeed(projectId) }
    : { mode: "http", seed: null };
}
