...
describe("Deterministic Versions UI gateway", () => {
  it("restores an old version by appending a new DRAFT while preserving approved history", async () => {
    ...
    const newHead = restored.versions.find(
      (item) => item.version.id === restored.head_version_id,
    )!;
    ...
    expect(
      restored.versions.find((item) => item.version.id === "design-v2")?.version
        .status,
    ).toBe("APPROVED");
    expect(
      restored.versions.some((item) => item.version.id === "design-v4"),
    ).toBe(true);
    expect(
      restored.lineage.some(
        (edge) =>
          edge.from_version_id === "design-v2" &&
          edge.to_version_id === newHead.version.id &&
  ...
  it("fails closed on a stale branch head before mutating restore history", async () => {
    ...
    expect(
      after.versions.filter((item) =>
        item.safe_change_summary.includes("Restored immutable"),
      ),
    ).toHaveLength(0);
  });
  ...
  it("compares exact version identities using structured semantic changes", async () => {
    ...
    expect(
      result.semantic_changes.some(
        (change) => change.node_id === "node-offer" && change.property === "x",
      ),
    ).toBe(true);
    expect(
      result.semantic_changes.some(
        (change) =>
          change.node_id === "node-headline" && change.property === "text",
      ),
    ).toBe(true);
  });
  ...
  it("surfaces a concurrent new head without deleting historical compare targets", async () => {
    const runtime = gateway();
    const before = await runtime.getWorkspace(ORG, PROJECT);
    const updated = await runtime.checkForUpdates(
      ORG,
      PROJECT,
      before.active_artifact.id,
    );
    expect(updated.concurrent_head_version_id).not.toBeNull();
    expect(updated.notice?.kind).toBe("WARNING");
    expect(
      updated.versions.some((item) => item.version.id === "design-v2"),
    ).toBe(true);
    expect(
      updated.versions.some((item) => item.version.id === "design-v4"),
    ).toBe(true);
  ...
  it("switches to raster Artifact history while preserving exact Artifact identity", async () => {
    const runtime = gateway();
    const raster = await runtime.getWorkspace(
      ORG,
      PROJECT,
      "artifact-hero-raster",
    );
    expect(raster.active_artifact.type).toBe("RASTER_IMAGE");
    expect(raster.versions.map((item) => item.version.id)).toEqual([
      "raster-v3",
      "raster-v2",
      "raster-v1",
    ]);
