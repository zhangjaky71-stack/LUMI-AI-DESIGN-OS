...
const base = {
  session_id: "s1",
  user: { id: "u1", display_name: "User", email_hint: "u•••@example.test" },
  organizations: [{ id: "org-1", name: "One", slug: "one", role: "OWNER" as const }],
...
describe("shell session contract", () => {
  ...
  it("rejects cross-organization active ids", () => {
    expect(() => assertShellSession({ ...base, active_organization_id: "org-other" })).toThrow("SHELL_SESSION_ACTIVE_ORGANIZATION_INVALID");
  });
});
