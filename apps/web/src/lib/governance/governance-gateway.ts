...
async function optionalScope<T>(promise: Promise<T>, fallback: T): Promise<T> {
  try {
    ...
    if (error instanceof LumiApiError && error.problem.status === 403)
      return fallback;
    throw error;
  }
}
...
class HttpGovernanceGateway implements GovernanceGateway {
  constructor(
    private readonly api: LumiApiClient,
  ...
  async load(signal?: AbortSignal): Promise<GovernanceWorkspace> {
    ...
    const [policies, candidates, holds, deletions, exports] = await Promise.all(
      [
        optionalScope(
          this.api.get<{ items: GovernanceWorkspace["retention_policies"] }>(
        ...
        optionalScope(
          this.api.get<{ items: GovernanceWorkspace["retention_candidates"] }>(
            "/governance/retention/candidates",
            options,
          ),
          { items: [] },
        ),
        optionalScope(
          this.api.get<{ items: GovernanceWorkspace["legal_holds"] }>(
        ...
        optionalScope(
          this.api.get<{ items: GovernanceWorkspace["deletions"] }>(
        ...
        optionalScope(
          this.api.get<{ items: GovernanceWorkspace["exports"] }>(
  ...
  searchAudit(
    ...
    return this.api.get<AuditPageView>(
      `/governance/audit?${queryString(filters)}`,
      request(signal),
    );
  }
  ...
  releaseHold(
    ...
    return this.api.post<
      LegalHoldView,
      { reason_code: string; ticket_ref: string }
  ...
  requestDeletion(subjectUserId: string): Promise<DeletionView> {
    return this.api.post<
      DeletionView,
      { subject_user_id: string; organization_id: string; reason_code: string }
    >("/governance/deletions", {
  ...
  createExport(
    ...
    return this.api.post<AuditExportJobView, AuditExportRequest>(
      "/governance/audit/exports",
      body,
    );
  }
...
class DeterministicGovernanceGateway implements GovernanceGateway {
  ...
  async searchAudit(
    ...
    if (filters.action)
      items = items.filter((item) =>
        item.action.includes(filters.action ?? ""),
      );
    if (filters.result)
      items = items.filter((item) => item.result === filters.result);
    if (filters.resource_type)
      items = items.filter(
        (item) => item.resource_type === filters.resource_type,
      );
    if (filters.resource_id)
      items = items.filter((item) =>
        item.resource_id.includes(filters.resource_id ?? ""),
      );
    if (filters.trace_id)
      items = items.filter((item) => item.trace_id === filters.trace_id);
    const start = filters.cursor
      ? Number.parseInt(filters.cursor.replace("cursor-", ""), 10) || 0
      : 0;
  ...
  async publishRetention(
    ...
    if (!this.workspace.capabilities.can_manage_retention)
      throw new Error("GOVERNANCE_FORBIDDEN");
    ...
    this.workspace = {
      ...this.workspace,
      retention_policies: [...this.workspace.retention_policies, policy],
    };
    return policy;
  }
  ...
  async createHold(
    ...
    if (!this.workspace.capabilities.can_manage_holds)
      throw new Error("GOVERNANCE_FORBIDDEN");
    ...
    this.workspace = {
      ...this.workspace,
      legal_holds: [hold, ...this.workspace.legal_holds],
    };
    return hold;
  }

  async releaseHold(holdId: string): Promise<LegalHoldView> {
    const hold = this.workspace.legal_holds.find(
      (item) => item.hold_id === holdId,
    );
    if (!hold) throw new Error("LEGAL_HOLD_NOT_ACTIVE");
    this.workspace = {
      ...this.workspace,
      legal_holds: this.workspace.legal_holds.filter(
        (item) => item.hold_id !== holdId,
      ),
    };
    return hold;
  ...
  async requestDeletion(subjectUserId: string): Promise<DeletionView> {
    if (!this.workspace.capabilities.can_manage_deletion)
      throw new Error("GOVERNANCE_FORBIDDEN");
  ...
  async executeDeletion(requestId: string): Promise<DeletionView> {
    ...
    this.workspace = {
      ...this.workspace,
      deletions: this.workspace.deletions.map((item) =>
        item.request_id === requestId ? updated : item,
      ),
    };
    return updated;
  }
  ...
  async createExport(format: ExportFormat): Promise<AuditExportJobView> {
    if (!this.workspace.capabilities.can_export_audit)
      throw new Error("GOVERNANCE_FORBIDDEN");
    ...
    this.workspace = {
      ...this.workspace,
      exports: [job, ...this.workspace.exports],
    };
    return job;
  }
  ...
  async getDownload(jobId: string): Promise<AuditDownloadLeaseView> {
    ...
    if (!job || job.status !== "READY")
      throw new Error("AUDIT_EXPORT_NOT_READY");
    this.leaseCounter += 1;
    return {
      job_id: jobId,
