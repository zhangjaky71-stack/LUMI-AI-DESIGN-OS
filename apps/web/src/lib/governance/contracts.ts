...
export function latestRetentionPolicies(
  ...
  for (const item of values) {
    ...
    if (!prior || item.version > prior.version)
      latest.set(item.retention_class, item);
  }
  ...
  if (value instanceof LumiApiError) {
    ...
    const known: Record<string, string> = {
      GOVERNANCE_FORBIDDEN:
        "You do not have permission for this governance operation.",
      AUDIT_TENANT_SCOPE_MISMATCH:
        "Audit access is limited to the active organization.",
      GOVERNANCE_TENANT_SCOPE_MISMATCH:
        "Governance access is limited to the active organization.",
...
export function safeAuditDownloadUrl(
  lease: AuditDownloadLeaseView | null,
): string | null {
...
export function canDownload(job: AuditExportJobView): boolean {
  return (
    job.status === "READY" &&
    Boolean(job.file_name && job.checksum_sha256 && job.size_bytes !== null)
  );
}

export function workspaceHasSevenRetentionClasses(
  workspace: GovernanceWorkspace,
): boolean {
  return (
    latestRetentionPolicies(workspace.retention_policies).length ===
