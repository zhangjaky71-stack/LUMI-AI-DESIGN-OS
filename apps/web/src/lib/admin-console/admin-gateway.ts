...
class HttpAdminGateway implements AdminGateway {
  ...
  retryRun(runId: string, input: SensitiveActionInput): Promise<AdminRun> {
    return this.api.post<AdminRun, SensitiveActionInput>(
      `/admin/runs/${encodeURIComponent(runId)}:retry`,
      input,
    );
  }
  ...
  disableProvider(
    ...
    return this.api.post<
      AdminProvider,
      SensitiveActionInput & { expires_at: string }
  ...
  requeue(
    ...
    return this.api.post<AdminQueueItem, SensitiveActionInput>(
      `/admin/queue/${encodeURIComponent(queueItemId)}:requeue`,
      input,
    );
  }
  ...
  setRegistryEnabled(
    ...
    return this.api.post<
      AdminRegistryItem,
      SensitiveActionInput & { enabled: boolean }
    >(
  ...
  adjustBilling(
    ...
    return this.api.post<
      AdminBillingView,
      SensitiveActionInput & { delta_credits: number }
    >(
...
class DeterministicAdminGateway implements AdminGateway {
  ...
  async retryRun(
    ...
    this.workspace = {
      ...this.workspace,
      runs: this.workspace.runs.map((item) =>
        item.run_id === runId ? updated : item,
      ),
    };
    return updated;
  }
  ...
  async disableProvider(
    ...
    void _input;
    const current = this.workspace.providers.find(
      (item) => item.provider_id === providerId,
    );
    if (!current) throw new Error("ADMIN_PROVIDER_NOT_FOUND");
    const updated: AdminProvider = {
      ...current,
      health: "DISABLED",
      routing_weight_basis_points: 0,
      disabled_until: expiresAt,
      disabled_reason: "confirmed admin action",
    };
    this.workspace = {
      ...this.workspace,
      providers: this.workspace.providers.map((item) =>
        item.provider_id === providerId ? updated : item,
      ),
    };
    return updated;
  }
  ...
  async requeue(
    ...
    void _input;
    const current = this.workspace.queue.find(
      (item) => item.queue_item_id === queueItemId,
    );
    if (!current) throw new Error("ADMIN_QUEUE_ITEM_NOT_FOUND");
    const updated: AdminQueueItem = {
      ...current,
      state: "READY",
    ...
    this.workspace = {
      ...this.workspace,
      queue: this.workspace.queue.map((item) =>
        item.queue_item_id === queueItemId ? updated : item,
      ),
    };
    return updated;
  }

  async setRegistryEnabled(
    item: AdminRegistryItem,
    enabled: boolean,
    _input: SensitiveActionInput,
  ): Promise<AdminRegistryItem> {
    ...
    this.workspace = {
      ...this.workspace,
      registry: this.workspace.registry.map((value) =>
        value.registry_id === item.registry_id && value.kind === item.kind
          ? updated
          : value,
      ),
    };
  ...
  async adjustBilling(
    ...
    return {
      organization_id: organizationId,
      plan_version_id: "pro-v2",
      subscription_state: "ACTIVE",
      credit_balance: 1200 + deltaCredits,
      invoice_refs: ["inv-6401"],
    };
  }
  ...
  async startViewAs(
    ...
    return {
      session_id: "view-node64",
      admin_actor_id: this.workspace.actor.actor_id,
      target_user_id: userId,
      target_organization_id: organizationId,
      readonly: true,
      started_at: started.toISOString(),
      expires_at: new Date(started.getTime() + 10 * 60_000).toISOString(),
      ended_at: null,
    };
  }
}
...
export function createAdminGateway(
  ...
  if (bootstrap.deterministic && bootstrap.workspace)
    return new DeterministicAdminGateway(bootstrap.workspace);
