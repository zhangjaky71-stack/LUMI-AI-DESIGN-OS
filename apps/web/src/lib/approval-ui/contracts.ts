...
export function assertExactApprovalSubject(record: ApprovalRecord): void {
  ...
  if (!version || FLOATING.test(version))
    throw new Error("APPROVAL_SUBJECT_VERSION_MUST_BE_EXACT");
}
...
export function historyApprovals(
  workspace: ApprovalWorkspace,
): readonly ApprovalRecord[] {
  return workspace.approvals.filter((item) => item.status !== "PENDING");
}
...
export function policyLabel(record: ApprovalRecord): string {
  ...
  if (policy.mode === "MIN_N")
    return `MIN_N · ${policy.min_approvals} approvals`;
  if (policy.mode === "ROLE_BASED_SEQUENCE")
    return `Sequence · ${policy.sequence_roles.join(" → ")}`;
  if (policy.mode === "ALL")
    return `ALL · ${policy.required_roles.join(" + ")}`;
  return "ANY_ONE";
}

export function safeApprovalError(error: unknown): {
  message: string;
  ...
  return {
    message: safeCode(
      error instanceof Error ? error.message : "APPROVAL_REQUEST_FAILED",
    ),
    request_id: null,
  };
}
