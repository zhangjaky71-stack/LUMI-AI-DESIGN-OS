...
export class DeterministicApprovalGateway implements ApprovalGateway {
  ...
  constructor(workspace: ApprovalWorkspace) {
  ...
  async decide(
    ...
    this.#assert(projectId, signal);
    const approval = this.#workspace.approvals.find(
    ...
    if (
      ...
      !input.feedback?.comment.trim() &&
      !input.feedback?.requested_changes.length
    ) {
    ...
    this.#workspace = {
      ...this.#workspace,
      approvals: this.#workspace.approvals.map((item) =>
  ...
  #assert(projectId: string, signal?: AbortSignal): void {
    ...
    if (projectId !== this.#workspace.project_id)
      throw new Error("APPROVAL_PROJECT_NOT_FOUND");
  }
}
