...
function fixture(projectId: string): ApprovalWorkspace {
  const pending: ApprovalRecord = {
    ...
    subject: {
      ...
      subject_id: "artifact-summer-launch",
      subject_version: "artifact-v4",
    },
    status: "PENDING",
    ...
    payload_summary:
      "Approve the exact campaign master before export and publish.",
  ...
  const changes: ApprovalRecord = {
    ...
    subject: {
      subject_type: "CREATIVE_DIRECTION",
      subject_id: "direction-editorial-a",
      subject_version: "direction-v2",
    },
    status: "CHANGES_REQUESTED",
    payload_summary:
      "Review editorial direction before generating the final family.",
    ...
    feedback: {
      comment:
        "Reduce visual density and preserve the approved logo safe zone.",
      node_refs: ["hero-title"],
      region_refs: ["frame-instagram:hero"],
      requested_changes: ["Reduce decorative elements", "Keep logo safe zone"],
    },
    decisions: [
      {
        ...
        reason: null,
        decided_subject_version: "direction-v2",
        idempotency_key: "fixture-changes",
        created_at: "2026-08-15T06:48:00.000Z",
      },
    ],
  };
  return {
...
export function getApprovalBootstrap(projectId: string): ApprovalBootstrap {
  const deterministic =
    ...
    process.env.LUMI_APPROVAL_E2E === "1";
