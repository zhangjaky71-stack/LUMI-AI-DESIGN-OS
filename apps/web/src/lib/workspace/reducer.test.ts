...
function event(
  eventId: string,
  eventType: SafeRunEvent["eventType"],
...
describe("workspace event reducer", () => {
  ...
  it("admits artifact cards only with an exact artifact version", () => {
    const state = reduceWorkspaceEvent(
      initialWorkspaceRuntimeState(),
      event("evt-2", "artifact.created", {
  ...
  it("refuses to fabricate a link for artifact-only events", () => {
    const state = reduceWorkspaceEvent(
      initialWorkspaceRuntimeState(),
      event("evt-3", "artifact.created", { artifact_id: "artifact-1" }),
    );
    expect(state.artifacts).toHaveLength(0);
    expect(state.timeline[0]?.kind).toBe("warning");
