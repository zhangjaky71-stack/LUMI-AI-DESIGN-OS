...
export function assertExactCollaborationVersion(
  value: string,
  label: string,
): void {
...
export function validateCommentBody(value: string): string {
  ...
  if (normalized.length > 4000)
    throw new Error("COLLABORATION_COMMENT_TOO_LONG");
  return normalized;
}
...
export function validateAnchor(
  ...
  assertExactCollaborationVersion(
    anchor.design_document_version_id,
    "design_version",
  );
  return anchor;
}
...
export function mentionOptions(
  ...
  return members.filter(
    (member) =>
      member.actor_id !== current.actor_id && member.role !== "BILLING",
  );
}
...
export function operationConflictKey(
  operation: CollaborationOperation,
): string {
  return `${operation.node_id}::${operation.property_name}`;
}

export function safeCollaborationError(error: unknown): {
  message: string;
  ...
  const code =
    error instanceof Error ? error.message : "COLLABORATION_REQUEST_FAILED";
...
export function shortAgentRun(actor: CollaborationActorSummary): string | null {
  ...
  return actor.agent_run_id.length > 18
    ? `${actor.agent_run_id.slice(0, 18)}…`
    : actor.agent_run_id;
}
