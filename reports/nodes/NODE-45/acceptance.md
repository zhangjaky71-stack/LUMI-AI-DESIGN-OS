...
| USER metadata outranks AUTO | `merge_metadata` + test | Implemented |
...
| Multimodal embedding contract | `AnalyzerModelSnapshot`, `AnalyzerOutput` | Implemented |
| NODE-23 registry snapshot boundary | `CapabilityRegistryPort` / bundle validation | Implemented |
| Embedding model/version/dimension pinned | index + ingestion + query guard | Implemented |
...
| TEXT search | `search.py` | Implemented |
| OCR search | `search.py` | Implemented |
| SEMANTIC search | `search.py`, `query_embedding.py` | Implemented |
| SIMILAR_TO search | `search.py` | Implemented |
| HYBRID ranking | `AssetRankingProfile`, `search.py` | Implemented |
| Tenant/permission/rights before scoring | repository + SQL scoped function + tests | Implemented |
...
| Reindex build/compare/switch | `index_catalog.py` | Implemented |
| No mixed embedding space | index/query/ingestion guards + test | Implemented |
...
| Dedicated CI | `.github/workflows/asset-intelligence.yml` | Pending creation in this branch |
...
4. NODE-44 remains identity scoring/calibration authority; NODE-45 only provides evidence.
...
7. Different embedding spaces never share an ACTIVE index.
...
10. User selection/approval is a ranking signal, not model-training consent.
11. Commercial-use permission and training authorization are separate fields.
12. NODE-45 does not create a persistent/cross-tenant biometric index.
...
- perceptual near-duplicate classification;
- semantic-similar non-duplicate classification;
- OCR search with retained bbox/confidence;
- semantic search cross-tenant leak bait excluded;
...
- NODE-44 evidence has versions but no identity score;
...
`scripts/benchmark_asset_intelligence.py` benchmarks only the dependency-free in-memory scoped ranking core. It intentionally excludes remote analyzer inference, queue delay, PostgreSQL/pgvector execution and network/storage latency. The node does not invent a production latency SLO where the architecture baseline did not define one.

## Hosted validation

Completion requires the dedicated release workflow to actually execute green:
