...
- Transactional `ProjectAuditEntry` contract paired with Project events.
...
- DRAFT/ACTIVE/PAUSED/ARCHIVED runtime lifecycle including explicit restore.
...
- `project_brief_versions`, `project_branch_defaults`, `project_summaries`,
...
- PostgreSQL invariant test harness using savepoints for expected rejection evidence.
- 8 machine-readable JSON Schema exports.
...
PYTHONPATH=apps/api/src uv run pytest -q \
  apps/api/tests/test_project_core_*.py \
...
uv run ruff check apps/api/src/lumi_api/projects \
  ...
  apps/api/src/lumi_api/api/v1/ports.py \
  apps/api/src/lumi_api/persistence/models_project_core.py \
  apps/api/tests/test_project_core_*.py \
  tools/node17
...
Most importantly, this node does **not** claim a production PostgreSQL ProjectRepository
binding or durable project-create idempotency replay yet. It also does not claim the
eventual summary projector worker is running.

## Completion rule

Do not mark NODE-17 `COMPLETE` from static code review, a workflow file, or a job that
never received a runner. `runner_id=0 / steps=[]` is `BLOCKED_EXTERNAL`, not PASS.
