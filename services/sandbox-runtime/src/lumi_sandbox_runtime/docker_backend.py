...
class DockerSandboxBackend:
    """Local/CI backend. Agent data is never interpolated into a host shell command."""
    ...
    def create(self, spec: SandboxSpec) -> UUID:
        ...
        log_dir = self.log_root / str(sandbox_id)
        ...
        create_args = [
            ...
            "--tmpfs",
            f"/tmp:rw,nosuid,nodev,noexec,size={tmp_mb}m",
            "--workdir",
            "/workspace/work",
            "--stop-timeout",
            "2",
        ...
        try:
            self._control(create_args, timeout=60)
            self._control(["start", container_name], timeout=30)
        except Exception:
    ...
    def exec(self, sandbox_id: UUID, request: ExecRequest) -> ExecResult:
        ...
        with record.lock:
            ...
            stderr_thread.join(timeout=5)
            duration_ms = int((time.monotonic() - started) * 1000)
            stdout, stdout_truncated = _read_limited(stdout_path, record.spec.max_output_bytes)
            stderr, stderr_truncated = _read_limited(stderr_path, record.spec.max_output_bytes)
    ...
    def read_file(self, sandbox_id: UUID, path: str, *, max_bytes: int | None = None) -> bytes:
        ...
        with record.lock:
            ...
            with tempfile.TemporaryDirectory(dir=record.root / "staging") as temp_dir:
                ...
                self._control(["cp", f"{record.container_name}:{resolved}", str(target)], timeout=30)
                if target.is_symlink() or not target.is_file():
                    raise SandboxPolicyError("SANDBOX_FILE_TYPE_FORBIDDEN")
                if target.stat().st_size > limit:
                    raise SandboxPolicyError("SANDBOX_FILE_TOO_LARGE")
                data = target.read_bytes()
            self._audit(record, "sandbox.file.read", resource=path, detail={"bytes": len(data)})
            return data
    ...
    def write_file(self, sandbox_id: UUID, path: str, data: bytes) -> None:
        ...
        with record.lock:
            ...
            with tempfile.NamedTemporaryFile(dir=record.root / "staging", delete=False) as handle:
                handle.write(data)
                staged = Path(handle.name)
            try:
                self._control(["cp", str(staged), f"{record.container_name}:{resolved}"], timeout=30)
            finally:
                staged.unlink(missing_ok=True)
            self._audit(record, "sandbox.file.written", resource=path, detail={"bytes": len(data)})
    ...
    def list_files(self, sandbox_id: UUID, path: str) -> tuple[FileEntry, ...]:
        ...
        with record.lock:
            ...
            result = self._exec_internal(record, ["python", "-c", _LIST_SCRIPT, root, target])
            ...
            self._audit(record, "sandbox.file.listed", resource=path, detail={"count": len(entries)})
            return entries
    ...
    def upload_asset(self, sandbox_id: UUID, asset_ref: str) -> str:
        ...
        with record.lock:
            ...
            if len(asset.data) > record.spec.disk_limit_mb * 1024 * 1024:
                raise SandboxPolicyError("SANDBOX_INPUT_ASSET_TOO_LARGE")
            digest = _sha256_bytes(asset.data)
    ...
    def collect_artifact(self, sandbox_id: UUID, path: str) -> CollectedArtifact:
        ...
        with record.lock:
            ...
            with tempfile.TemporaryDirectory(dir=record.root / "staging") as temp_dir:
                ...
                self._control(["cp", f"{record.container_name}:{resolved}", str(staged)], timeout=60)
                if staged.is_symlink() or not staged.is_file():
                    raise SandboxPolicyError("SANDBOX_ARTIFACT_FILE_TYPE_INVALID")
                size = staged.stat().st_size
                if size > record.spec.disk_limit_mb * 1024 * 1024:
    ...
    def reap_expired(self) -> tuple[UUID, ...]:
        ...
        with self._records_lock:
            expired = tuple(
                ...
                if record.state not in {SandboxState.TERMINATED, SandboxState.TERMINATING}
    ...
    def _prepare_write_path(self, record: _SandboxRecord, *, zone: str, target: str) -> str:
        root = f"/workspace/{zone}"
        result = self._exec_internal(
            record,
    ...
    def _exec_internal(self, record: _SandboxRecord, command: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    def _best_effort_remove(self, container: str) -> None:
        try:
            self._control(["rm", "-f", "-v", container], timeout=20, allow_failure=True)
        except Exception:
            pass

    def _best_effort_kill(self, container: str) -> None:
        try:
            self._control(["kill", container], timeout=10, allow_failure=True)
        except Exception:
            pass
