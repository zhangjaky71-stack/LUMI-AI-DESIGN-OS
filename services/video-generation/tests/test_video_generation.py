...
from lumi_video_generation.storyboard import compile_storyboard, retry_shot_operation_id
...
class FakeGateway:
    ...
    async def estimate(self, *, spec, shot, excluded_provider_keys=()):
        return GatewayEstimate(Decimal("0.10"), "mock", "video-v1", "price-v1")
    ...
    async def poll(self, *, pending):
        status = self.poll_status.get(pending.result.provider_request_id or "", "COMPLETED")
        return GatewayVideoResult(
            status=status,
            provider=pending.result.provider,
            model=pending.result.model,
            provider_request_id=pending.result.provider_request_id,
            output_ref=(f"provider://{pending.shot_id}" if status == "COMPLETED" else None),
            cost_usd=Decimal("0.08") if status == "COMPLETED" else None,
        )
...
class FakeOutput:
    ...
    async def materialize_and_validate(self, *, spec, shot, result):
        ...
        return StoredVideoClip(
            ...
            durable_ref=f"asset:video:{shot.shot.shot_id}:{shot.retry_ordinal}",
...
class FakeValidator:
    async def validate(self, *, spec, shot, clip, provider_result):
        return ShotValidationReport(ValidationDecision.PASS, (), True, True)


class FakeRenderer:
    async def render(self, *, timeline):
        duration = sum((clip.duration_seconds for clip in timeline.clips), Decimal("0"))
...
class FakeArtifacts:
    ...
    async def append_clip(self, *, job, clip):
        self.clip_count += 1
        return f"artifact:{clip.shot_id}"
...
def make_spec(*, operation_id: str = "op-1", shots: int = 2) -> VideoTaskSpec:
...
def test_storyboard_operation_ids_are_deterministic_and_retry_is_distinct():
    ...
    retry = retry_shot_operation_id(spec.operation_id, first.shot.shot_id, 1)
...
def test_storyboard_to_final_video_and_provenance():
    async def scenario():
        ...
        assert completed.provenance.git_commit == "deadbeef"
...
def test_operation_id_is_idempotent_but_semantic_reuse_conflicts():
    async def scenario():
        ...
        first = await pipeline.start(make_spec(operation_id="same-op", shots=1))
        same = await pipeline.start(make_spec(operation_id="same-op", shots=1))
