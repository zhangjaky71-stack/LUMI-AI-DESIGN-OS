...
from .storyboard import compile_storyboard, retry_shot_operation_id
...
def _replace_shot(job: VideoJob, updated: ShotRuntime) -> VideoJob:
    return replace(
        job,
        shots=tuple(updated if item.shot_id == updated.shot_id else item for item in job.shots),
    )
...
def _request_hash(
    ...
    return hashlib.sha256(
        (
            f"{spec.semantic_hash}:{shot.paid_operation_id}:"
            f"{':'.join(continuity_refs)}:{':'.join(excluded_provider_keys)}"
        ).encode()
    ).hexdigest()
...
def _provider_key(result: GatewayVideoResult) -> str:
    return f"{result.provider}:{result.model}"


def _compiled_for_runtime(base: CompiledShot, runtime: ShotRuntime) -> CompiledShot:
    return replace(base, paid_operation_id=runtime.paid_operation_id)
...
class VideoGenerationPipeline:
    ...
    async def resume(self, *, organization_id: str, video_job_id: str) -> VideoJob:
        ...
        if waiting is None:
            return await self._advance(spec, job)
        pending = self.repository.get_provider_job(
            organization_id,
            video_job_id,
            waiting.shot_id,
        ...
        if result.status == "PENDING":
            ...
            return job
        terminal = replace(pending, result=result)
        self.repository.save_provider_job(terminal)
    ...
    async def cancel(self, *, organization_id: str, video_job_id: str) -> VideoJob:
        ...
        cancelled = replace(
            ...
            shots=tuple(
                ...
                if item.status in {"QUEUED", "WAITING_EXTERNAL"}
                else item
        ...
        while True:
            ...
            required_failure = next(
                (
                    ...
                    and not next(
                        item.shot.optional
                        for item in storyboard.shots
                        if item.shot.shot_id == runtime.shot_id
                    )
                ),
                None,
            )
            if required_failure is not None:
                failed = replace(
                    job,
                    status="FAILED",
                    error_code=required_failure.error_code or "VIDEO_REQUIRED_SHOT_FAILED",
                )
            ...
            if next_estimated > spec.budget_limit_usd:
                if compiled.shot.optional and spec.allow_optional_shot_drop:
                    dropped = replace(
                        queued,
                        status="DROPPED",
                        error_code="VIDEO_OPTIONAL_SHOT_DROPPED_FOR_BUDGET",
                    )
            ...
            if result.status == "PENDING":
                ...
                self.repository.save_provider_job(
                    ProviderJobRecord(
                        organization_id=spec.organization_id,
                        video_job_id=job.video_job_id,
                        ...
                        request_hash=_request_hash(
                            ...
                            compiled,
                            continuity_refs,
                            queued.excluded_provider_keys,
                        ),
                        result=result,
                    )
                )
                waiting_shot = replace(
                    queued,
    ...
    async def _finish_terminal(
        ...
        if result.status != "SUCCEEDED" or not result.output_ref:
            if result.status != "CANCELLED":
                retry = await self._quality_retry(
                    ...
                    reason=f"VIDEO_PROVIDER_{result.status}",
                )
            ...
            if shot.shot.optional and spec.allow_optional_shot_drop:
                dropped = replace(
                    ...
                    model=result.model,
                    provider_request_id=result.provider_request_id,
                    error_code=f"VIDEO_OPTIONAL_SHOT_{result.status}",
                )
                job = _replace_shot(job, dropped)
                self.repository.save(job)
                return job
        ...
        try:
            clip, probe = await self.output.materialize_and_probe(
                spec=spec,
                shot=shot,
                output_ref=result.output_ref,
                declared_mime_type=result.output_mime_type,
            )
            validation = await self.validator.validate_shot(
                spec=spec,
                shot=shot,
                clip=clip,
                probe=probe,
            ...
            if shot.shot.optional and spec.allow_optional_shot_drop:
                dropped = replace(
                    runtime,
                    status="DROPPED",
                    error_code=f"VIDEO_OPTIONAL_POSTPROCESS:{type(exc).__name__}",
                )
                job = _replace_shot(job, dropped)
                self.repository.save(job)
                return job
            return self._fail_job(job, runtime, f"VIDEO_POSTPROCESS_EXCEPTION:{type(exc).__name__}")
        continuity_refs, continuity_parent_ids = self._continuity(
            job,
            compile_storyboard(spec).shots,
            shot,
        )
        ...
        attempts = tuple(
            dict.fromkeys(runtime.attempt_artifact_version_ids + (artifact_version_id,))
        )
        if validation.decision == "PASS":
            ready = replace(
                ...
                model=result.model,
                provider_request_id=result.provider_request_id,
                clip_artifact_version_id=artifact_version_id,
                attempt_artifact_version_ids=attempts,
                clip=clip,
                validation=validation,
                error_code=None,
            )
        ...
        if shot.shot.optional and spec.allow_optional_shot_drop:
            dropped = replace(
                ...
                clip=clip,
                validation=validation,
                error_code="VIDEO_OPTIONAL_SHOT_VALIDATION_FAILED",
            )
            job = _replace_shot(job, dropped)
            self.repository.save(job)
            return job
        failed = replace(
            ...
            clip=clip,
            validation=validation,
            error_code="VIDEO_SHOT_VALIDATION_FAILED",
        )
        failed_job = replace(
            _replace_shot(job, failed),
            status="FAILED",
            error_code="VIDEO_SHOT_VALIDATION_FAILED",
        )
    ...
    async def _quality_retry(
        ...
        *,
        ...
        if runtime.attempt_count >= spec.quality_retry_limit:
            return None
        next_attempt = runtime.attempt_count + 1
        next_operation_id = retry_shot_operation_id(
            spec.operation_id,
            runtime.shot_id,
            next_attempt,
        )
        excluded = tuple(dict.fromkeys(runtime.excluded_provider_keys + (provider_key,)))
        retry_runtime = replace(
            runtime,
            status="QUEUED",
            attempt_count=next_attempt,
            paid_operation_id=next_operation_id,
            excluded_provider_keys=excluded,
            provider=None,
            model=None,
            provider_request_id=None,
            clip_artifact_version_id=None,
            clip=None,
            validation=None,
            error_code=f"VIDEO_QUALITY_RETRY:{reason}",
        )
        retry_job = replace(
            _replace_shot(job, retry_runtime),
            status="SUBMITTING",
            error_code=None,
        )
        ...
        await self.events.emit(
            ...
            payload={
                ...
                "reason": reason,
                "excluded_provider_key": provider_key,
                "paid_operation_id": next_operation_id,
            },
        )
        return retry_job
    ...
    def _continuity(
        ...
        for continuity in shot.shot.continuity_refs:
            ...
            if continuity.source_shot_id:
                source = next(
                    (item for item in job.shots if item.shot_id == continuity.source_shot_id),
                    None,
                )
                if (
                    source is None
                    or source.status != "READY"
                    or source.clip is None
                    or not source.clip.tail_frame_ref
                ):
        ...
        if shot.ordinal > 1 and not any(
            item.kind == "PREVIOUS_TAIL" for item in shot.shot.continuity_refs
        ):
    ...
    async def _compose(self, spec: VideoTaskSpec, job: VideoJob) -> VideoJob:
        ready = [
            item
            for item in sorted(job.shots, key=lambda value: value.ordinal)
            if item.status == "READY" and item.clip is not None and item.clip_artifact_version_id
        ]
        if not ready:
        ...
        clips = tuple(
            TimelineClip(
                ...
                duration_seconds=next(
                    compiled.shot.duration_seconds
                    for compiled in storyboard.shots
                    if compiled.shot.shot_id == item.shot_id
                ),
            )
            for item in ready
        )
        transitions = tuple(
            TimelineTransition(
                ...
                kind=next(
                    compiled.shot.transition_to_next
                    for compiled in storyboard.shots
                    if compiled.shot.shot_id == clips[index].shot_id
                ),
            )
        ...
        timeline = VideoTimeline(
            ...
            overlays=(),
            audio_tracks=tuple(
                TimelineAudioTrack(item.durable_ref, item.offset_seconds, item.gain_db)
        ...
        try:
            ...
            validation = await self.validator.validate_final(
                spec=spec,
                timeline=timeline,
                rendered=rendered,
            )
            ...
            failed = replace(
                composing,
                status="FAILED",
                error_code=f"VIDEO_COMPOSITION_EXCEPTION:{type(exc).__name__}",
            )
            self.repository.save(failed)
            return failed
        ...
        if validation.decision != "PASS":
            failed = replace(
                ...
                error_code="VIDEO_FINAL_VALIDATION_FAILED",
            )
            self.repository.save(failed)
            return failed
        ...
        completed = replace(
            ...
            final_artifact_version_id=final_version_id,
            error_code="VIDEO_COMPLETED_WITH_OPTIONAL_DROPS" if dropped else None,
        )
        ...
        await self.events.emit(
            ...
            organization_id=spec.organization_id,
            video_job_id=job.video_job_id,
            payload={"status": completed.status, "artifact_version_id": final_version_id},
        )
        return completed
