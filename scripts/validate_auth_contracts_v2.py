...
def validate_router_security() -> None:
    router = (AUTH_SRC / "router.py").read_text(encoding="utf-8")
    schemas = (AUTH_SRC / "schemas_v1.py").read_text(encoding="utf-8")
    assert_contains(router, "httponly=True", "HttpOnly session cookie")
    assert_contains(router, "samesite=\"lax\"", "SameSite session cookie")
    assert_contains(router, "secure=secure_cookie", "Secure cookie configuration")
    assert_contains(router, "application/problem+json", "Problem Details errors")
    assert_contains(router, "status_code=202", "enumeration-safe accepted endpoints")
    assert_contains(router, "X-CSRF-Token", "CSRF header contract")
    assert "VerificationTokenResponse" not in schemas
    assert "PasswordResetTokenResponse" not in schemas
    assert "InviteTokenResponse" not in schemas
    assert_contains(schemas, "class ApiTokenCreateResponse", "one-time API token response")
    assert_contains(router, "notifications.send_email_verification", "verification secret delivery port")
    assert_contains(router, "notifications.send_password_reset", "reset secret delivery port")
    assert_contains(router, "notifications.send_organization_invite", "invite secret delivery port")


def validate_policy_dependency_boundary() -> None:
    forbidden = (
        "fastapi","sqlalchemy","alembic","argon2","langchain","langgraph","openai",
        "anthropic","boto3","httpx","requests","celery","pika",
    )
...
def main() -> None:
    ...
    print("Auth/Tenant security contract OK: migrations, Argon2id, cookie/CSRF and secret-delivery boundary validated")
