...
def require(path: str, *needles: str) -> str:
    ...
    for needle in needles:
        if needle not in text:
            raise SystemExit(f"{path}: missing NODE-28 contract marker: {needle}")
    return text
...
def forbid(path: str, *needles: str) -> None:
    ...
    for needle in needles:
        if needle in text:
            raise SystemExit(f"{path}: forbidden NODE-28 contract marker: {needle}")
...
def assert_no_ambient_authority() -> None:
    ...
    for path in root.rglob("*.py"):
        ...
        for marker in forbidden_markers:
            if marker in text:
                raise SystemExit(f"{path}: ambient secret/authority marker: {marker}")
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                roots = {alias.name.split(".", 1)[0] for alias in node.names}
                if roots & forbidden_imports:
                    raise SystemExit(f"{path}: forbidden runtime import: {roots & forbidden_imports}")
            if isinstance(node, ast.ImportFrom) and node.module:
                root_name = node.module.split(".", 1)[0]
                if root_name in forbidden_imports:
                    raise SystemExit(f"{path}: forbidden runtime import: {root_name}")
...
def main() -> int:
    ...
    if "for definition in self.definitions" in durable:
        raise SystemExit("durable executor must not scan graphs to guess thread binding")
    require(
