...
from lumi_agent_runtime.agent_registry import AgentReleaseManager, AgentReleaseStatus, EvalEvidence, load_definitions, load_release_manifest
...
class Gate:
    def evaluate(self, definition):
        return EvalEvidence(True, f"eval://{definition.agent_id}/{definition.version}/release-pass")


def main() -> int:
    definitions = {item.identity: item for item in load_definitions(ROOT / "agents")}
    manifest = load_release_manifest(ROOT / "agents/registry.json")
    manager = AgentReleaseManager(Gate())
    ...
    target = next(item for item in promoted.releases if item.agent_id == "creative-director" and item.version == "1.2.0")
