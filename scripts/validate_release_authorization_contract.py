...
def main() -> int:
    ...
    require(policy.get("base_ref") == collector.EXPECTED_BASE_REF and policy.get("head_ref") == collector.EXPECTED_HEAD_REF, "approval policy template refs mismatch")
    require(policy.get("minimum_distinct_actors") == 3, "approval policy distinct actor floor must be 3")
    roles = policy.get("roles")
    require(isinstance(roles, dict) and set(roles) == set(collector.ROLES), "approval policy role set mismatch")
    for role in collector.ROLES:
        require(roles[role].get("allowed_logins") == ["PENDING"], f"role {role} must remain fail-closed until real principals are configured")
    ...
    require(request.get("approval_policy") == {"path": "PENDING", "sha256": "PENDING"}, "authorization request must freeze approval policy")
    ...
    require(authorization.get("status") == "PENDING" and authorization.get("distinct_approver_count") == 0, "authorization output template must not pre-approve")
    auth_roles = authorization.get("approvals")
    require(isinstance(auth_roles, dict) and set(auth_roles) == set(collector.ROLES), "authorization output role set mismatch")
    for role in collector.ROLES:
        item = auth_roles[role]
        require(isinstance(item, dict) and item.get("status") == "PENDING" and item.get("review_id") == 0, f"authorization role {role} must start PENDING")
    ...
    require_markers(FINAL_DECISION, (
        ...
        'runtime_dir / "release-authorization-live.json"',
    ...
    for marker in (
        ...
        require(marker in workflow, f"Final Decision missing authorization wrapper marker: {marker}")
    require("pull-requests: write" not in workflow, "Final Acceptance must never receive PR write permission")
