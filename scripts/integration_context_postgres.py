...
async def main_async() -> None:
    ...
    try:
        assert (
            await migration.fetchval("SELECT count(*) FROM organizations WHERE id=$1", ORG_ID) == 1
        )
        assert (
            await migration.fetchval("SELECT count(*) FROM projects WHERE id=$1", PROJECT_ID) == 1
        )
        ...
        bundle = instantiate_compiled_recipe(
            compiled, organization_id=ORG_ID, project_id=PROJECT_ID, agent_run_id=AGENT_RUN_ID
        )
        ...
        if graph_id is not None:
            await migration.execute(
                "DELETE FROM outbox_events WHERE payload_json->>'graph_id'=$1", str(graph_id)
            )
        await migration.execute("DELETE FROM agent_runs WHERE id=$1", AGENT_RUN_ID)
