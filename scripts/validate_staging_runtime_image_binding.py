...
def _validate_run_id(value: str) -> str:
    if not value.isdecimal() or int(value) <= 0:
        raise BindingError(
            "runtime image build run id must be a positive decimal GitHub Actions run id"
        )
    return value
...
def _run_identity(build_run_url: str) -> tuple[str, str]:
    ...
    if (
        ...
        raise BindingError(
            "runtime image build_run_url must be a canonical github.com HTTPS run URL"
        )
    parts = [part for part in parsed.path.split("/") if part]
    if len(parts) != 5 or parts[2:4] != ["actions", "runs"]:
        raise BindingError(
            "runtime image build_run_url must use /<owner>/<repo>/actions/runs/<id>"
        )
...
def expected_image_set_ref(*, build_run_url: str, git_sha: str) -> str:
    ...
    if len(git_sha) != 40 or any(
        char not in "0123456789abcdef" for char in git_sha.lower()
    ):
...
def _build_recipes_from_frozen(frozen: dict[str, Any]) -> dict[str, str]:
    image_set = frozen.get("container_image_set")
    if not isinstance(image_set, dict):
        raise BindingError("frozen runtime image set is missing container_image_set")
    provenance = image_set.get("provenance")
    if not isinstance(provenance, dict) or set(provenance) != REQUIRED_RUNTIMES:
        raise BindingError(
            "frozen runtime image set must contain exactly six provenance records"
        )
    recipes: dict[str, str] = {}
    for service in sorted(REQUIRED_RUNTIMES):
        item = provenance.get(service)
        if not isinstance(item, dict):
            raise BindingError(f"frozen provenance missing for {service}")
        recipe = item.get("build_recipe_ref")
        if not isinstance(recipe, str) or not recipe.strip():
            raise BindingError(f"frozen build recipe missing for {service}")
...
def _validate_report_binding(
    *,
    ...
    if len(attestation_report_sha256) != 64 or any(
        c not in SHA256 for c in attestation_report_sha256
    ):
        raise BindingError("attestation verification report SHA-256 is invalid")
    metadata = frozen.get("attestation_verification")
    if not isinstance(metadata, dict):
        raise BindingError(
            "frozen runtime image set is missing attestation_verification metadata"
        )
    required_metadata = {
        "schema_version",
        "kind",
    ...
    if metadata.get("report_file") != "attestation-verification.json":
        raise BindingError(
            "frozen runtime image set binds an unexpected attestation report filename"
        )
    if metadata.get("sha256") != attestation_report_sha256:
        raise BindingError(
            "downloaded attestation verification report SHA-256 differs from frozen image set"
        )
    if metadata.get("repository") != expected_repository:
        raise BindingError(
            "frozen attestation verification repository differs from build-run repository"
        )
    if metadata.get("source_digest") != expected_git_sha.lower():
        raise BindingError(
            "frozen attestation source digest differs from accepted RC SHA"
        )

    module = _load_image_set_module()
    build_recipes = _build_recipes_from_frozen(frozen)
    try:
        ...
        raise BindingError(
            f"downloaded attestation verification report is invalid: {exc}"
        ) from exc
    for key in (
        "schema_version",
        "kind",
    ...
    if summary.get("repository") != expected_repository:
        raise BindingError(
            "attestation verification report repository differs from build-run repository"
        )
...
def validate_binding(
    ...
    *,
    ...
    if (
        ...
        or frozen.get("kind") != "LUMI_RUNTIME_IMAGE_SET_V1"
    ):
    ...
    if not isinstance(evidence_sha, str) or evidence_sha.lower() != frozen_sha:
        raise BindingError(
            "staging evidence RC SHA does not equal frozen runtime image-set SHA"
        )
    if evidence_rc.get("version") != frozen_rc.get("version"):
        raise BindingError(
            "staging evidence RC version does not equal frozen runtime image-set version"
        )
    ...
    if expected_run_id is not None and build_run_id != _validate_run_id(expected_run_id):
        raise BindingError(
            "downloaded runtime image set does not originate from requested build run id"
        )
    expected_ref = expected_image_set_ref(
        build_run_url=build_run_url,
        git_sha=evidence_sha,
    )
    if evidence_rc.get("container_image_set_ref") != expected_ref:
        raise BindingError(
            "release_candidate.container_image_set_ref does not bind the frozen build artifact"
        )
    ...
    if evidence_set != frozen_set:
        raise BindingError(
            "staging evidence container_image_set differs from frozen build artifact"
        )
    ...
    if normalized != frozen_set:
        raise BindingError(
            "frozen runtime image set is not NODE-71 canonical after normalization"
        )
    ...
    if not isinstance(provenance, dict) or set(provenance) != REQUIRED_RUNTIMES:
        raise BindingError(
            "bound image set does not contain exactly six provenance records"
        )
...
def _fixture() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], str]:
    ...
    if not isinstance(runtimes, dict) or set(runtimes) != REQUIRED_RUNTIMES:
        raise BindingError(
            "runtime manifest fixture does not contain exactly six runtimes"
        )
    git_sha = "a" * 40
    ...
    for index, service in enumerate(sorted(REQUIRED_RUNTIMES), start=1):
        ...
        if not isinstance(dockerfile, str):
            raise BindingError(f"runtime Dockerfile fixture missing: {service}")
        images[service] = image
        provenance[service] = {
            "git_sha": git_sha,
            "build_recipe_ref": dockerfile,
    ...
    for result in report_results:
        result["github_attestation_policy"] = dict(policy)
    report = {
        "schema_version": 1,
...
def self_test() -> dict[str, Any]:
    ...
    malformed_run_url["build_run_url"] = (
        "https://github.com/example/lumi/actions/runs/123?x=1"
    )
    _must_block(
        evidence,
        malformed_run_url,
    ...
    _must_block(
        ...
        report,
        report_hash,
        "frozen attestation source swap",
    )

    frozen_base_swap = copy.deepcopy(frozen)
    frozen_base_swap["attestation_verification"]["base_images"]["PYTHON_BASE_IMAGE"] = (
        "python@sha256:" + "d" * 64
    )
    _must_block(
        evidence,
        ...
        report,
        report_hash,
        "frozen base-image identity swap",
    )
    ...
    _must_block(
        ...
        report_status_swap,
        report_hash,
        "attestation report status swap",
    )

    report_image_swap = copy.deepcopy(report)
    report_image_swap["results"][0]["image"] = (  # type: ignore[index]
    ...
    _must_block(
        evidence,
        frozen,
        report_repository_swap,
        report_hash,
        "attestation report repository swap",
    )
    ...
    _must_block(
        ...
        report_source_swap,
        report_hash,
        "attestation report source swap",
    )

    runtime_policy_swap = copy.deepcopy(report)
    runtime_policy_swap["results"][0]["github_attestation_policy"][  # type: ignore[index]
        "source_digest"
    ] = "b" * 40
    _must_block(
        evidence,
        ...
        runtime_policy_swap,
        report_hash,
        "runtime attestation policy swap",
    )

    runtime_base_swap = copy.deepcopy(report)
    runtime_base_swap["results"][0]["buildkit_provenance"]["base_images"][  # type: ignore[index]
        "UV_BASE_IMAGE"
    ] = "ghcr.io/astral-sh/uv@sha256:" + "e" * 64
    _must_block(
        evidence,
        frozen,
        runtime_base_swap,
        report_hash,
        "runtime base-image identity swap",
    )

    runtime_source_uri_swap = copy.deepcopy(report)
    runtime_source_uri_swap["results"][0]["buildkit_provenance"][  # type: ignore[index]
        "source_uri"
    ] = f"https://github.com/example/other.git#{'a' * 40}"
    _must_block(
        evidence,
        frozen,
        runtime_source_uri_swap,
        report_hash,
        "runtime BuildKit source URI swap",
    )
    ...
    return {
        ...
        "negative_drills": [
            ...
            "frozen_base_image_swap_blocked",
            ...
            "runtime_base_image_swap_blocked",
            "runtime_buildkit_source_uri_swap_blocked",
        ],
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Bind NODE-71 evidence to one frozen and verified runtime image build"
    )
