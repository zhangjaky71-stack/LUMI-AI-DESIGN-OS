...
def main() -> None:
    ...
    service_missing = REQUIRED_SERVICE - service_files
    api_missing = REQUIRED_API - api_files
    assert not service_missing, service_missing
    assert not api_missing, api_missing

    for path in sorted(SERVICE.glob("*.py")):
        source = parse(path)
        assert "lumi_api" not in source, (
            f"API dependency leaked into {path.name}"
        )
    ...
    schema_source = critic.split("_CRITIC_SCHEMA", 1)[1].split(
        "class ModelGatewayVisualGraderAdapter",
        1,
    )[0]
    assert '"INFO"' in schema_source
    assert '"WARNING"' in schema_source
    assert '"ERROR"' in schema_source
    assert '"HARD"' not in schema_source
    ...
    assert (
        ...
        "REFERENCES artifact_versions" in sql_source
    )
