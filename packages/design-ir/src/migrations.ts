...
export class DesignIrMigrationRegistry {
  ...
  register(step: MigrationStep): this {
    if (step.from === step.to)
      throw new Error("Migration must advance schema_version");
    if (this.steps.has(step.from))
      throw new Error(`Migration from ${step.from} is already registered`);
  ...
  migrate(document: DesignDocument, targetVersion: string): DesignDocument {
    ...
    while (current.schema_version !== targetVersion) {
      if (visited.has(current.schema_version)) {
        throw new Error(
          `Migration cycle detected at ${current.schema_version}`,
        );
      }
      ...
      if (!step) {
        throw new Error(
          `No Design IR migration path from ${current.schema_version} to ${targetVersion}`,
        );
      }
      ...
      if (migrated.schema_version !== step.to) {
        throw new Error(
          `Migration ${step.from} -> ${step.to} returned ${migrated.schema_version}`,
        );
      }
      current =
