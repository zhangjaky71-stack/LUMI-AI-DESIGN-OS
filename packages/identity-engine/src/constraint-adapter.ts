...
function assertPinnedConstraint(
  ...
  if (identityId !== report.identity_id)
    throw new Error("IDENTITY_CONSTRAINT_REPORT_MISMATCH");
  const referenceVersion = stringParameter(constraint, "reference_set_version");
  if (referenceVersion && referenceVersion !== report.reference_set_version)
    throw new Error("IDENTITY_REFERENCE_VERSION_MISMATCH");
  const profileId = stringParameter(constraint, "threshold_profile_id");
  if (profileId && profileId !== report.threshold_profile_id)
    throw new Error("IDENTITY_THRESHOLD_PROFILE_MISMATCH");
  const profileVersion = stringParameter(
    constraint,
    "threshold_profile_version",
  );
  if (profileVersion && profileVersion !== report.threshold_profile_version)
    throw new Error("IDENTITY_THRESHOLD_PROFILE_VERSION_MISMATCH");
...
export class CalibratedIdentityConstraintAdapter
  implements IdentitySimilarityValidator
{
  ...
  async validate(
    ...
    if (report.status === "UNAVAILABLE")
      throw new Error(report.reason_code ?? "IDENTITY_VALIDATION_UNAVAILABLE");
    ...
    const repairHint = asJsonObject({
      action:
        report.status === "REVIEW"
          ? "manual_review"
          : "regenerate_with_canonical_reference",
    ...
    return [
      {
        ...
        ...(context.document.root_id
          ? {
              target_id:
                constraint.scope.node_ids?.[0] ?? context.document.root_id,
            }
          : {}),
        ...(report.identity_score === null
          ? {}
          : { score: report.identity_score }),
