...
import {
  DEFAULT_IDENTITY_PRIVACY_POLICY,
...
function validateProfile(
  ...
  if (identity.status !== "PUBLISHED")
    throw new Error("IDENTITY_REFERENCE_SET_NOT_PUBLISHED");
  if (profile.status !== "PUBLISHED")
    throw new Error("IDENTITY_THRESHOLD_PROFILE_NOT_PUBLISHED");
  if (identity.organization_id !== profile.organization_id)
    throw new Error("IDENTITY_PROFILE_TENANT_MISMATCH");
  if (identity.type !== profile.identity_type)
    throw new Error("IDENTITY_PROFILE_TYPE_MISMATCH");
  if (
    identity.threshold_profile_id !== profile.profile_id ||
    identity.threshold_profile_version !== profile.version
  ) {
    throw new Error("IDENTITY_PROFILE_VERSION_MISMATCH");
  }
  if (profile.model_bundle_version !== provider.provider_version)
    throw new Error("IDENTITY_PROVIDER_VERSION_MISMATCH");
  if (profile.preprocessor_version !== provider.preprocessor_version)
    throw new Error("IDENTITY_PREPROCESSOR_VERSION_MISMATCH");
  assertScore(profile.threshold, "threshold");
  assertScore(profile.review_floor, "review_floor");
  if (profile.review_floor > profile.threshold)
    throw new Error("IDENTITY_REVIEW_FLOOR_INVALID");
  if (
    !Number.isFinite(profile.minimum_confidence) ||
    profile.minimum_confidence < 0 ||
    profile.minimum_confidence > 1
  ) {
    throw new Error("IDENTITY_MINIMUM_CONFIDENCE_INVALID");
  }
  if (
    (identity.type === "PRODUCT" || identity.type === "LOGO") &&
    new Set(profile.required_signals).size < 2
  ) {
...
function validateReferences(
  ...
  if (!identity.reference_views.length || !identity.canonical_asset_ids.length)
    throw new Error("IDENTITY_REFERENCE_SET_EMPTY");
  const views = new Map(
    identity.reference_views.map((view) => [
      `${view.asset_id}@${view.asset_version}`,
      view,
    ]),
  );
  ...
  for (const reference of references) {
    if (reference.organization_id !== identity.organization_id)
      throw new Error("IDENTITY_REFERENCE_TENANT_MISMATCH");
...
function validateCandidateTarget(input: IdentityValidationInput): void {
  ...
  if (
    input.candidate.metadata?.target_detected === true ||
    input.candidate.metadata?.whole_artifact_target === true
  )
    return;
  throw new Error("IDENTITY_TARGET_REGION_UNAVAILABLE");
}

function selectSignalScores(
  scores: readonly IdentitySignalScore[],
): Map<string, IdentitySignalScore> {
  ...
  for (const row of scores) {
    ...
    if (
      ...
      (row.score === existing.score && row.confidence > existing.confidence)
    ) {
...
function weightedAggregate(
  ...
  let totalWeight = 0;
  for (const [signal, weight] of Object.entries(profile.signal_weights).sort(
    ([a], [b]) => a.localeCompare(b),
  )) {
    const row = selected.get(signal);
    if (!row) continue;
    if (!Number.isFinite(weight) || weight <= 0)
      throw new Error(`IDENTITY_SIGNAL_WEIGHT_INVALID:${signal}`);
...
function collectEvidence(
  ...
  const evidence: IdentityEvidenceRef[] = [
    {
      kind: "CALIBRATION",
      ref: `${profile.calibration_dataset_version}:${profile.profile_id}@${profile.version}`,
    },
  ];
...
export class IdentityValidationRuntime {
  ...
  async validate(
    ...
    if (input.identity.organization_id !== input.candidate.organization_id)
      throw new Error("IDENTITY_CANDIDATE_TENANT_MISMATCH");
    if (input.profile.scenario !== input.scenario)
      throw new Error("IDENTITY_SCENARIO_PROFILE_MISMATCH");
    if (input.identity.type === "STYLE_REFERENCE" && input.severity === "HARD")
      throw new Error("STYLE_REFERENCE_CANNOT_BE_HARD");
    ...
    const missing = input.profile.required_signals.filter(
      (signal) => !selected.has(signal),
    );
    if (missing.length)
      throw new Error(
        `IDENTITY_REQUIRED_SIGNAL_UNAVAILABLE:${missing.sort().join(",")}`,
      );
    if (
      (input.identity.type === "PRODUCT" || input.identity.type === "LOGO") &&
      selected.size < 2
    ) {
    ...
    const status =
      aggregate.confidence < input.profile.minimum_confidence
        ? "REVIEW"
        : aggregate.score >= input.profile.threshold
          ...
          : aggregate.score >= input.profile.review_floor
            ? "REVIEW"
            : "FAIL";
    const orderedScores = [...selected.values()].sort((a, b) =>
      a.signal.localeCompare(b.signal),
    );
    const validationSnapshotId = await snapshotId(
      input.identity,
      input.profile,
      this.provider,
      selected,
    ...
    const reasonCode =
      status === "FAIL"
        ...
        : status === "REVIEW"
          ? aggregate.confidence < input.profile.minimum_confidence
            ? "IDENTITY_CONFIDENCE_BELOW_MINIMUM"
    ...
    return {
      ...
      ...(input.candidate.target_region
        ? { candidate_region: input.candidate.target_region }
        : {}),
...
export async function identityValidationBatchSnapshotId(
  reports: readonly IdentityValidationReport[],
): Promise<string> {
  if (!reports.length) throw new Error("IDENTITY_VALIDATION_BATCH_EMPTY");
  const organizations = new Set(
    reports.map((report) => report.organization_id),
  );
  if (organizations.size !== 1)
    throw new Error("IDENTITY_VALIDATION_BATCH_TENANT_MISMATCH");
  ...
  const digest = await canonicalSha256({
    ...
    reports: [...reports]
      .sort(
        (a, b) =>
          a.identity_id.localeCompare(b.identity_id) ||
