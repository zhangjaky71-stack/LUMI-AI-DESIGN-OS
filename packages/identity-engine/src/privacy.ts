...
export const DEFAULT_IDENTITY_PRIVACY_POLICY: IdentityPrivacyPolicy =
  Object.freeze({
...
export function assertReferencePrivacy(
  ...
  if (!policy.allow_face_processing)
    throw new Error("FACE_PROCESSING_NOT_ALLOWED");
  ...
  if (!facePolicy?.explicit_processing_consent)
    throw new Error("FACE_EXPLICIT_CONSENT_REQUIRED");
  if (!facePolicy.purpose.trim())
    throw new Error("FACE_PROCESSING_PURPOSE_REQUIRED");
  if (facePolicy.persistent_biometric_index !== false)
    throw new Error("PERSISTENT_FACE_INDEX_FORBIDDEN");
