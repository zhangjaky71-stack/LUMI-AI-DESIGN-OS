...
export function evaluateIdentityArtifactApproval(
  ...
  if (version.organization_id !== provenance.organization_id)
    reasons.add("IDENTITY_PROVENANCE_TENANT_MISMATCH");
  if (version.id !== provenance.artifact_version_id)
    reasons.add("IDENTITY_PROVENANCE_ARTIFACT_MISMATCH");
  ...
  if (versionSnapshot !== provenanceSnapshot)
    reasons.add("IDENTITY_SNAPSHOT_PROVENANCE_MISMATCH");
  if (
    expectedBatchSnapshotId !== null &&
    versionSnapshot !== expectedBatchSnapshotId
  )
    reasons.add("IDENTITY_SNAPSHOT_VERSION_MISMATCH");
  ...
  for (const report of reports) {
    ...
    if (report.status === "UNAVAILABLE")
      reasons.add("IDENTITY_VALIDATION_UNAVAILABLE");
    else if (report.status === "REVIEW")
      reasons.add("IDENTITY_MANUAL_REVIEW_REQUIRED");
