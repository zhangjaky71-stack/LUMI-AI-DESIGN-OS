...
describe("NODE-44 Artifact identity provenance", () => {
  it("keeps the legacy stable manifest hash unchanged when identity validation is absent", async () => {
    const before = await artifactManifestSha256(version(), provenance(), [
      file,
    ]);
    const after = await artifactManifestSha256(
  ...
  it("changes the canonical manifest hash when an identity snapshot is pinned", async () => {
    ...
    const pinned = await artifactManifestSha256(
      version("identity-batch:one"),
      provenance("identity-batch:one"),
      [file],
    );
    expect(pinned).not.toBe(legacy);
    const manifest = await buildArtifactExportManifest(
      version("identity-batch:one"),
      provenance("identity-batch:one"),
      [file],
    );
  ...
  it("rejects version/provenance snapshot disagreement", async () => {
    await expect(
      artifactManifestSha256(
        version("identity-batch:one"),
        provenance("identity-batch:two"),
        [file],
      ),
    ).rejects.toThrow("identity validation snapshot mismatch");
  });

  it("requires the exact batch snapshot and all HARD identity reports to pass approval", () => {
    expect(
      evaluateIdentityArtifactApproval(
        version("identity-batch:one"),
        provenance("identity-batch:one"),
        [report("PASS")],
        "identity-batch:one",
    ...
    ).toContain("IDENTITY_HARD_VIOLATION");
    expect(
      evaluateIdentityArtifactApproval(
        version("identity-batch:one"),
        provenance("identity-batch:one"),
    ...
    ).toContain("IDENTITY_MANUAL_REVIEW_REQUIRED");
    expect(
      evaluateIdentityArtifactApproval(
        version("identity-batch:one"),
        provenance("identity-batch:one"),
    ...
    ).toContain("IDENTITY_SNAPSHOT_VERSION_MISMATCH");
  });
});
