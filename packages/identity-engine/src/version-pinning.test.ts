...
describe("NODE-44 authoritative version pinning", () => {
  it("rejects a reference set pointing at another threshold profile version", async () => {
    const runtime = new IdentityValidationRuntime(
      new StructuredIdentitySignalProvider("fixture", "model@3", "prep@2"),
    );
    await expect(
      runtime.validate({
        identity: { ...identity, threshold_profile_version: "2" },
        profile,
        references: [asset],
        candidate,
        severity: "HARD",
    ...
    ).rejects.toThrow("IDENTITY_PROFILE_VERSION_MISMATCH");
  });

  it("rejects a model upgrade until a newly calibrated profile pins it", async () => {
    const runtime = new IdentityValidationRuntime(
      new StructuredIdentitySignalProvider("fixture", "model@4", "prep@2"),
    );
    await expect(
      runtime.validate({
        identity,
    ...
    ).rejects.toThrow("IDENTITY_PROVIDER_VERSION_MISMATCH");
  });

  it("rejects a preprocessor upgrade until the profile is recalibrated", async () => {
    const runtime = new IdentityValidationRuntime(
      new StructuredIdentitySignalProvider("fixture", "model@3", "prep@3"),
    );
    await expect(
      runtime.validate({
        identity,
    ...
    ).rejects.toThrow("IDENTITY_PREPROCESSOR_VERSION_MISMATCH");
  });
});
