...
export async function compareIdentityCandidates(
  ...
  if (left.organization_id !== right.organization_id)
    throw new Error("IDENTITY_COMPARE_TENANT_MISMATCH");
  ...
  for (const row of rows) {
    if (!Number.isFinite(row.score) || row.score < 0 || row.score > 100)
      throw new Error("IDENTITY_COMPARE_SIGNAL_INVALID");
    if (
  ...
  if ((type === "PRODUCT" || type === "LOGO") && selected.size < 2)
    throw new Error("IDENTITY_MULTI_SIGNAL_EVIDENCE_REQUIRED");
  let score = 0;
  let confidence = 0;
  let totalWeight = 0;
  for (const [signal, weight] of Object.entries(signalWeights).sort(
  ...
  const signalScores = [...selected.values()].sort((a, b) =>
  ...
  const digest = await canonicalSha256({
    ...
    identity_type: type,
    left: {
      artifact_id: left.artifact.artifact_id,
      version: left.artifact.version,
    },
    right: {
      artifact_id: right.artifact.artifact_id,
      version: right.artifact.version,
    },
