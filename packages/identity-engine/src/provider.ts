...
function parseStructuredSignals(
...
function exactHashSignal(
  request: IdentitySignalRequest,
): IdentitySignalScore | null {
  ...
  const matched = request.references.find(
    (reference) => reference.checksum_sha256 === candidateChecksum,
  );
  const matchedViewId = matched
    ? request.identity.reference_views.find(
        (view) =>
          view.asset_id === matched.asset_id &&
          view.asset_version === matched.asset_version,
      )?.view_id
    : undefined;
  return {
    signal: "exact_hash",
    ...
    evidence_refs: [
      evidence(
        ...
        matched ? "exact canonical asset match" : "no canonical checksum match",
      ),
    ],
  };
}
...
function wordmarkSignal(
  ...
  const scored = references
    .map((text) => ({ text, score: tokenSimilarity(candidateText, text) }))
  ...
  return {
    ...
    evidence_refs: [
      evidence(
        "OCR",
        `ocr:${normalizedText(candidateText)}`,
        `closest canonical text: ${normalizedText(best.text)}`,
      ),
    ],
  };
}
 ...
 */
export class StructuredIdentitySignalProvider
  implements IdentitySignalProvider
{
  constructor(
  ...
  async score(
    ...
    return signals.sort(
      (a, b) => a.signal.localeCompare(b.signal) || b.score - a.score,
    );
  }
}
