...
function samples(
  type: "LOGO" | "PRODUCT",
  scenario: "STRICT_PRESERVE" | "BACKGROUND_REPLACEMENT",
): CalibrationSample[] {
  const rows =
    type === "LOGO"
      ? ([
  ...
  return rows.map(([sample_id, label, score]) => ({
    sample_id,
    label,
    score,
    identity_type: type,
...
function candidate(
  identity_signal_scores: Record<string, number>,
  checksum = H2,
  ocr = "LUMI COFFEE",
): IdentityCandidate {
  return {
...
const provider = new StructuredIdentitySignalProvider(
  "structured-fixture",
  "fixture-model@1",
  "prep@1",
);
...
describe("NODE-44 calibrated Identity Engine", () => {
  ...
  it("passes an exact logo using multiple signals and emits traceable evidence", async () => {
    ...
    expect(report.signal_scores.map((row) => row.signal)).toEqual([
      "exact_hash",
      "feature",
      "ocr_wordmark",
      "perceptual",
    ]);
    expect(report.identity_validation_snapshot_id).toMatch(
      /^identity-validation:[0-9a-f]{64}$/,
    );
  });
  ...
  it("does not enable face processing or persistent biometric indexing by default", async () => {
    ...
    const faceProfile: ThresholdCalibrationProfile = {
      ...base,
      profile_id: "face",
      identity_type: "FACE",
      scenario: "STRICT_PRESERVE",
      required_signals: ["face_similarity"],
    };
    const face: IdentityReferenceSet = {
      ...
      face_policy: {
        explicit_processing_consent: true,
        purpose: "user requested identity preservation",
    ...
    await expect(
      runtime.validate({
        ...
        profile: faceProfile,
        references: [verifiedReference()],
        candidate: candidate({ face_similarity: 99 }),
        severity: "HARD",
        scenario: "STRICT_PRESERVE",
      }),
    ).rejects.toThrow("FACE_PROCESSING_NOT_ALLOWED");
  });
  ...
  it("invalidates cache keys when calibration or provider versions change", async () => {
    ...
    const second = await identityCacheKey({
      ...base,
      profile: { ...profile, version: "4" },
    });
    const third = await identityCacheKey({
      ...base,
      provider_version: "fixture-model@2",
    });
