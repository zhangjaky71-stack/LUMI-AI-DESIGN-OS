  ...
  return {
    ...
    nodes: {
      ...
      frame: {
        ...
        transform: { x: 0, y: 0, width: 600, height: 400 },
      },
      title: {
        id: "title",
        kind: "TEXT",
        ...
        children: [],
        content: "Export",
        transform: { x: 20, y: 20, width: 200, height: 40 },
      ...
      image: {
        ...
        transform: { x: 40, y: 100, width: 300, height: 200 },
      },
    },
    resources: {
      "font-inter": {
        family: "Inter",
        version: "font-v4",
        uri: "https://signed.example/font?token=secret",
        weight: 500,
      },
      "asset-product": {
        version: "asset-v7",
        preview_uri: "https://signed.example/asset?token=secret",
        mime_type: "image/png",
        width: 1600,
        height: 1200,
      },
...
describe("NODE-49 exact source resolver", () => {
  it("compiles the exact DesignVersion and strips signed resource URIs from durable snapshot", async () => {
    ...
    const resolver = new ArtifactEngineExportSource({
      ...
      designs: {
        async loadExact(args) {
          expect(args.design_document_version_id).toBe("design-v9");
          return exact;
        },
      },
      ...
      metadata: {
        async rightsSummary() {
          return { commercial_use: "ALLOWED" };
        },
        async modelRefs() {
          return ["model:source"];
        },
        async provenanceRefs() {
    ...
    expect(snapshot.compiler_provenance.resource_versions).toEqual({
      "asset-product": "asset-v7",
    });
    expect(snapshot.compiler_provenance.font_versions).toEqual({
      "font-inter": "font-v4",
    });
    expect(snapshot.compiler_provenance.compile_hash).toMatch(/^[0-9a-f]{64}$/);
    const durable = JSON.stringify({
      design_document: snapshot.design_document,
  ...
  it("rejects an ArtifactVersion that does not pin the requested DesignVersion", async () => {
    ...
    await expect(
      resolver.resolveExactSnapshot({
        ...exportSpec(),
        design_document_version_id: "design-v10",
      }),
    ).rejects.toThrow("EXPORT_SOURCE_DESIGN_VERSION_NOT_EXACT");
  });
});
