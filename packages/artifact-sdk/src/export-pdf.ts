...
export function writeRasterPdf(pages: readonly PdfRasterPage[]): Uint8Array {
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    if (!page.jpeg.length || page.width_px <= 0 || page.height_px <= 0)
      throw new Error("EXPORT_PDF_PAGE_INVALID");
...
export function inspectRasterPdf(bytes: Uint8Array): PdfInspection {
  ...
  const pageMatches = [
    ...text.matchAll(
      /\/Type\s*\/Page\b[\s\S]*?\/MediaBox\s*\[0\s+0\s+([0-9.]+)\s+([0-9.]+)\]/g,
    ),
  ];
  ...
  if (
    mediaBoxes.some(
      (box) =>
        !Number.isFinite(box.width_pt) ||
        !Number.isFinite(box.height_pt) ||
        box.width_pt <= 0 ||
        box.height_pt <= 0,
    )
  ) {
