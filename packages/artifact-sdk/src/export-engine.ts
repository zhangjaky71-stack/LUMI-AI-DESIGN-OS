...
function fileName(spec: ExportSpec, variant: ExportVariant): string {
  const base = sanitizeExportFilename(
    variant.filename ?? spec.filename_template,
    "export",
  );
...
function validateSpec(spec: ExportSpec): void {
  if (
    ...
    throw new Error("EXPORT_IDENTITY_REQUIRED");
  if (!spec.artifact_version_id || !spec.design_document_version_id)
    throw new Error("EXPORT_EXACT_VERSION_REQUIRED");
  if (
  ...
  if (!spec.filename_template.trim())
    throw new Error("EXPORT_FILENAME_TEMPLATE_REQUIRED");
  if (!spec.variants.length) throw new Error("EXPORT_VARIANTS_REQUIRED");
  if (
    new Set(spec.variants.map((variant) => variant.variant_id)).size !==
    spec.variants.length
  )
    throw new Error("EXPORT_VARIANT_ID_DUPLICATE");
  if (
    !Number.isInteger(spec.retention_seconds) ||
    spec.retention_seconds < 60 ||
    spec.retention_seconds > 604800
  )
  ...
  for (const variant of spec.variants) {
    ...
    if (
      !variant.frame_ids.length &&
      variant.format !== "LUMI_PACKAGE" &&
      variant.format !== "ZIP"
    )
      throw new Error("EXPORT_FRAME_IDS_REQUIRED");
    if ((variant.width === undefined) !== (variant.height === undefined))
      throw new Error("EXPORT_TARGET_DIMENSIONS_PAIR_REQUIRED");
    if (
    ...
    if (
      variant.dpi !== undefined &&
      (!Number.isInteger(variant.dpi) || variant.dpi < 36 || variant.dpi > 1200)
    )
...
function validateStoredFormat(
  ...
  if (file.mime_type === "image/png") {
    if (!hasPrefix(bytes, [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]))
      throw new Error("EXPORT_PNG_SIGNATURE_INVALID");
  ...
  if (file.mime_type === "image/jpeg") {
    if (
      ...
      throw new Error("EXPORT_JPEG_SIGNATURE_INVALID");
  ...
  if (file.mime_type === "image/webp") {
    ...
    if (
      ...
      throw new Error("EXPORT_WEBP_SIGNATURE_INVALID");
  ...
  if (file.mime_type === "image/svg+xml") {
    ...
    if (
      ...
      /\b(?:href|src)=["']https?:/i.test(text)
    )
      throw new Error("EXPORT_SVG_STORED_INVALID");
    return { mime_type: file.mime_type, safe_svg: true };
  }
  if (file.mime_type === "application/pdf") {
    const inspection = inspectRasterPdf(bytes);
    if (
      file.page_count !== undefined &&
      inspection.page_count !== file.page_count
    )
      throw new Error("EXPORT_PDF_STORED_PAGE_COUNT_MISMATCH");
    return {
      mime_type: file.mime_type,
      page_count: inspection.page_count,
      media_boxes: inspection.media_boxes,
    };
  }
...
export class ExportEngine {
  ...
  async start(spec: ExportSpec): Promise<ExportJob> {
    validateSpec(spec);
    const existing = await this.#jobs.findByOperation(
      spec.organization_id,
      spec.operation_id,
    );
    if (existing) {
      ...
      if (existing.export_fingerprint !== fingerprint)
        throw new Error("EXPORT_OPERATION_SEMANTIC_CONFLICT");
      return existing;
    }
    const source = await this.#source.resolveExactSnapshot(spec);
    if (
    ...
    const fingerprint = await exportFingerprint(source, spec);
    const now = this.#now();
    const expiresAt = addSeconds(now, spec.retention_seconds);
    const reusable = await this.#jobs.findReadyByFingerprint(
      spec.organization_id,
      fingerprint,
      expiresAt,
    );
  ...
  async execute(
    ...
    if (
      ...
      return current;
    if (
      current.source.artifact_version_id !== current.spec.artifact_version_id ||
      current.source.design_document_version_id !==
        current.spec.design_document_version_id
    )
      throw new Error("EXPORT_PINNED_SOURCE_CORRUPTED");
    let job: ExportJob = { ...current, status: "RENDERING", progress: 5 };
    await this.#jobs.save(job);
    await this.#events.emit("export.rendering", {
      export_job_id: job.export_job_id,
    });
    try {
      const regularVariants = job.spec.variants.filter(
        (variant) =>
          variant.format !== "ZIP" && variant.format !== "LUMI_PACKAGE",
      );
      ...
      for (let index = 0; index < regularVariants.length; index += 1) {
        ...
        if (!payload.bytes.length || payload.mime_type !== mime(variant.format))
          throw new Error("EXPORT_RENDER_PAYLOAD_INVALID");
        const filename = fileName(job.spec, variant);
        const persisted = await this.#store.put(
          storageKey(job, variant, filename),
          payload.bytes,
          payload.mime_type,
        );
        const localChecksum = await bytesSha256(payload.bytes);
        if (
          persisted.checksum_sha256 !== localChecksum ||
          persisted.size_bytes !== payload.bytes.length
        )
        ...
        job = {
          ...
          progress:
            10 +
            Math.round(
              ((index + 1) / Math.max(regularVariants.length, 1)) * 55,
            ),
        };
        await this.#jobs.save(job);
        await this.#events.emit("export.variant_ready", {
          export_job_id: job.export_job_id,
          variant_id: variant.variant_id,
          checksum_sha256: persisted.checksum_sha256,
        });
      ...
      const manifestWithoutHash: Omit<ExportManifest, "manifest_sha256"> = {
        ...
        files: stableExportFiles(renderedFiles),
      ...
      const manifest: ExportManifest = {
        ...manifestWithoutHash,
        manifest_sha256: await exportManifestHash(manifestWithoutHash),
      };
      ...
      if (job.spec.include_manifest) {
        ...
        if (persisted.checksum_sha256 !== (await bytesSha256(manifestBytes)))
          throw new Error("EXPORT_MANIFEST_CHECKSUM_MISMATCH");
        manifestFile = {
      ...
      for (const variant of job.spec.variants.filter(
        (item) => item.format === "ZIP" || item.format === "LUMI_PACKAGE",
      )) {
        ...
        if (variant.format === "ZIP") {
          for (const file of renderedFiles)
            entries.push({
              ...
              bytes: await this.#store.get(file.storage_key),
            });
          entries.push({ name: "manifest.json", bytes: manifestBytes });
        } else {
          entries.push(
            { name: "lumi/manifest.json", bytes: manifestBytes },
            {
              name: "lumi/design-document.json",
              bytes: new TextEncoder().encode(
                canonicalExportJson(job.source.design_document),
              ),
            },
            {
              name: "lumi/compiler-provenance.json",
              bytes: new TextEncoder().encode(
                canonicalExportJson(job.source.compiler_provenance),
              ),
            },
            {
              name: "lumi/rights-summary.json",
              bytes: new TextEncoder().encode(
                canonicalExportJson(job.source.rights_summary),
              ),
            },
          );
          if (job.source.project_snapshot)
            entries.push({
              name: "lumi/project-snapshot.json",
              bytes: new TextEncoder().encode(
                canonicalExportJson(job.source.project_snapshot),
              ),
            });
          for (const file of renderedFiles)
            entries.push({
              name: `lumi/exports/${file.filename}`,
              bytes: await this.#store.get(file.storage_key),
            });
        }
        ...
        if (persisted.checksum_sha256 !== (await bytesSha256(packageBytes)))
          throw new Error("EXPORT_PACKAGE_CHECKSUM_MISMATCH");
        packageFiles.push({
          file_id: `export-file:${job.export_fingerprint}:${variant.variant_id}`,
          variant_id: variant.variant_id,
          storage_key: persisted.storage_key,
      ...
      job = {
        ...
        ...(packageFiles[0] ? { package_file: packageFiles[0] } : {}),
      };
      await this.#jobs.save(job);
      for (const file of [
        ...allFiles,
        ...
        const checksum = await bytesSha256(bytes);
        if (checksum !== file.checksum_sha256)
          throw new Error("EXPORT_READBACK_CHECKSUM_MISMATCH");
        await this.#validation?.record({
          organization_id: job.organization_id,
          export_job_id: job.export_job_id,
          export_file_id: file.file_id,
          validator: "sha256-readback",
          status: "PASS",
          details: { checksum_sha256: checksum, size_bytes: bytes.length },
        });
        ...
        await this.#validation?.record({
          ...
          export_job_id: job.export_job_id,
          export_file_id: file.file_id,
          validator: "format-structure",
          status: "PASS",
          details,
        });
      }
      await this.#artifacts.persistExport({
        job,
        files: allFiles,
        manifest,
        ...(packageFiles[0] ? { package_file: packageFiles[0] } : {}),
      ...
      await this.#events.emit("export.ready", {
        export_job_id: ready.export_job_id,
        file_count: ready.files.length,
      ...
      const failed: ExportJob = {
        ...
        status: "FAILED",
        error_code:
          error instanceof Error ? error.message : "EXPORT_UNKNOWN_FAILURE",
      };
      await this.#jobs.save(failed);
      await this.#events.emit("export.failed", {
        export_job_id: failed.export_job_id,
        error_code: failed.error_code ?? "EXPORT_UNKNOWN_FAILURE",
      });
      return failed;
    }
  }
...
export class ExportDownloadService {
  ...
  async download(args: {
    ...
    if (!job || job.status !== "READY")
      throw new Error("EXPORT_DOWNLOAD_NOT_READY");
    if (job.expires_at <= this.#now())
      throw new Error("EXPORT_DOWNLOAD_EXPIRED");
    const file = [
      ...job.files,
      ...(job.manifest_file ? [job.manifest_file] : []),
    ].find((item) => item.file_id === args.file_id);
    ...
    if (!allowed) {
      ...
      throw new Error("EXPORT_DOWNLOAD_FORBIDDEN");
    }
    const ttl = args.expires_seconds ?? 300;
    if (!Number.isInteger(ttl) || ttl < 30 || ttl > 900)
      throw new Error("EXPORT_DOWNLOAD_TTL_INVALID");
