...
const RESERVED = /[<>:"/\\|?*]/g;
const SENSITIVE_KEY =
  /(?:^|_)(?:api_?key|secret|authorization|access_?token|refresh_?token|provider_?token|hidden_?prompt|system_?prompt)(?:$|_)/i;
...
export function sanitizeExportFilename(
  ...
  const withoutTraversal = collapsed
    .replace(/^\.+/, "")
    .replace(/\.+$/, "")
    .trim();
...
export function safeZipEntryName(input: string): string {
  ...
  return segments
    .map((segment) => sanitizeExportFilename(segment, "file"))
    .join("/");
}
...
export function assertExportProfile(profile: string | undefined): void {
  ...
  if (profile === "DISPLAY_P3")
    throw new Error("EXPORT_DISPLAY_P3_NOT_VERIFIED_V1");
  if (profile !== undefined && profile !== "SRGB")
...
export function assertNoSensitiveExportMetadata(
  value: unknown,
  path = "$",
  depth = 0,
): void {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      assertNoSensitiveExportMetadata(item, `${path}[${index}]`, depth + 1),
    );
    return;
  }
  if (typeof value !== "object")
    throw new Error(`EXPORT_METADATA_VALUE_UNSUPPORTED:${path}`);
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    if (SENSITIVE_KEY.test(key))
      throw new Error(`EXPORT_SENSITIVE_METADATA_FORBIDDEN:${path}.${key}`);
    assertNoSensitiveExportMetadata(child, `${path}.${key}`, depth + 1);
  }
}

export function assertNoEphemeralExportRefs(
  value: unknown,
  path = "$",
  depth = 0,
): void {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      assertNoEphemeralExportRefs(item, `${path}[${index}]`, depth + 1),
    );
    return;
  }
  if (typeof value !== "object")
    throw new Error(`EXPORT_RUNTIME_REF_VALUE_UNSUPPORTED:${path}`);
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    if (
      EPHEMERAL_REF_KEY.test(key) &&
      child !== null &&
      child !== undefined &&
      child !== ""
    ) {
