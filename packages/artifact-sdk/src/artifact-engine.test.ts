...
function version(
  ...
  return {
    ...
    created_by_id: "u1",
    created_at: `2026-08-14T00:00:0${number}Z`,
  };
}
function seeded() {
...
describe("NODE-42 Artifact Engine", () => {
  it("enforces branch CAS and monotonic version identity", () => {
    ...
    expect(() => engine.addVersion(version("v2", 2, "v1"), null)).toThrow(
      ArtifactBranchConflictError,
    );
  ...
  it("restores by appending a new version and lineage edge", () => {
    ...
    engine.addVersion(version("v1", 1), null);
    engine.addVersion(
      { ...version("v2", 2, "v1"), content_hash: "d".repeat(64) },
      "v1",
    );
  ...
  it("attaches a file only after storage HEAD/checksum/size/MIME verification", async () => {
    ...
    const file: ArtifactFile = {
      ...
      role: "PREVIEW",
      storage_key: "org/o1/artifacts/x.png",
      mime_type: "image/png",
      size_bytes: 10,
      checksum_sha256: H,
    };
    await expect(
      engine.attachVerifiedFile(file, { stat: async () => null }),
    ).rejects.toThrow("missing");
    await expect(
      engine.attachVerifiedFile(file, {
        stat: async () => ({
    ...
    await engine.attachVerifiedFile(file, {
      stat: async () => ({
        storage_key: file.storage_key,
        size_bytes: 10,
        checksum_sha256: H,
  ...
  it("keeps stable manifest hash independent from runtime signed URLs", async () => {
    ...
    const file: ArtifactFile = {
      ...
      artifact_version_id: "v1",
      role: "PREVIEW",
      storage_key: "org/o1/hash.png",
      mime_type: "image/png",
      size_bytes: 10,
      checksum_sha256: H,
      metadata: { signed_url: "https://one" },
    };
    const first = await artifactManifestSha256(v, base, [file]);
    const second = await artifactManifestSha256(v, base, [
      { ...file, metadata: { signed_url: "https://two" } },
    ]);
  ...
  it("protects branch heads and approved versions from GC", () => {
    ...
    expect(
      liveArtifactStorageKeys(
        [{ ...version("v1", 1), status: "APPROVED" }],
        [{ ...branch, head_version_id: "v1" }],
        files,
      ).has("blob1"),
    ).toBe(true);
  });
