...
function source(
  ...
  return {
    ...
    design_document: {
      document_id: "doc-1",
      nodes: {
        frame: { id: "frame", kind: "FRAME", metadata: {} },
        text: { id: "text", kind: "TEXT", metadata: {} },
      },
    },
...
const frame = {
  ...
  local_bounds: bounds,
  world_bounds: bounds,
  resolved_style: { fill: "#ffffff", opacity: 1, blend_mode: "normal" },
  placeholder: false,
};
...
describe("NODE-49 SVG fidelity boundaries", () => {
  it("preserves explicit multiline text line height and center alignment", async () => {
    const text = {
      ...
      visible: true,
      z_order: 1,
      world_matrix: { ...matrix, tx: 10, ty: 20 },
      local_bounds: { x: 0, y: 0, width: 100, height: 48 },
      world_bounds: { x: 10, y: 20, width: 100, height: 48 },
      resolved_style: {
        fill: "#111111",
        opacity: 1,
        blend_mode: "normal",
      ...
      resolved_text: {
        content: "First\nSecond",
        metrics: { width: 72, height: 48, baseline: 16 },
      },
      placeholder: false,
    };
    const [page] = await serializer().renderPages(source([frame, text]), {
      variant_id: "svg",
      frame_ids: ["frame"],
      format: "SVG",
    });
  ...
  it("rejects text when compiler metrics cannot be represented by V1 text layout", async () => {
    const text = {
      ...
      resolved_text: {
        content: "single line",
        metrics: { width: 120, height: 48, baseline: 16 },
      },
      placeholder: false,
    };
    await expect(
      serializer().renderPages(source([frame, text]), {
        variant_id: "svg",
        frame_ids: ["frame"],
        format: "SVG",
      }),
    ).rejects.toThrow("EXPORT_TEXT_METRICS_LAYOUT_UNSUPPORTED_V1");
  });
  ...
  it("rejects mask or clip links instead of silently dropping them", async () => {
    ...
    await expect(
      serializer().renderPages(source([frame, clipped]), {
        variant_id: "svg",
        frame_ids: ["frame"],
        format: "SVG",
      }),
    ).rejects.toThrow("EXPORT_SVG_MASK_CLIP_NOT_IMPLEMENTED_V1");
  });
  ...
  it("rejects non-normal blend mode instead of approximating Pixi semantics", async () => {
    const blended = {
      ...
      parent_id: "frame",
      z_order: 1,
      resolved_style: { fill: "#111", blend_mode: "multiply" },
    };
    await expect(
      serializer().renderPages(source([frame, blended]), {
        variant_id: "svg",
        frame_ids: ["frame"],
        format: "SVG",
      }),
    ).rejects.toThrow("EXPORT_SVG_BLEND_MODE_NOT_IMPLEMENTED_V1");
  });
});
