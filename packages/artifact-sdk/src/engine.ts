...
const TRANSITIONS: Readonly<
  Record<ArtifactVersionStatus, ReadonlySet<ArtifactVersionStatus>>
> = {
...
function assertTenant(expected: string, actual: string): void {
  if (expected !== actual)
    throw new ArtifactEngineError("organization scope mismatch");
}
...
export class ArtifactEngine {
  ...
  addArtifact(value: Artifact): void {
    if (this.artifacts.has(value.id))
      throw new ArtifactEngineError("artifact already exists");
  ...
  addBranch(value: ArtifactBranch): void {
    ...
    if (
      [...this.branches.values()].some(
        (b) => b.artifact_id === value.artifact_id && b.name === value.name,
      )
    ) {
  ...
  addVersion(value: ArtifactVersion, expectedBranchHead?: string | null): void {
    if (
      ...
      !SHA256.test(value.constraint_snapshot_hash)
    ) {
      throw new ArtifactEngineError("version hashes must be lowercase SHA-256");
    }
    const artifact = this.artifacts.get(value.artifact_id);
    const branch = this.branches.get(value.branch_id);
    if (!artifact || !branch || branch.artifact_id !== value.artifact_id)
      throw new ArtifactEngineError("version artifact/branch missing");
    assertTenant(artifact.organization_id, value.organization_id);
    assertTenant(branch.organization_id, value.organization_id);
    if (
      expectedBranchHead !== undefined &&
      branch.head_version_id !== expectedBranchHead
    ) {
      throw new ArtifactBranchConflictError(
        "branch head compare-and-swap conflict",
      );
    }
    if (
      [...this.versions.values()].some(
        (v) =>
          v.artifact_id === value.artifact_id &&
          v.version_number === value.version_number,
      )
    ) {
    ...
    if (value.parent_version_id) {
      ...
      if (
        !parent ||
        parent.artifact_id !== value.artifact_id ||
        parent.organization_id !== value.organization_id
      ) {
  ...
  updateBranchHead(
    ...
    if (
      ...
      version.organization_id !== branch.organization_id
    ) {
      throw new ArtifactEngineError("invalid branch head");
    }
    if (branch.head_version_id !== expectedHead)
      throw new ArtifactBranchConflictError(
        "branch head compare-and-swap conflict",
      );
  ...
  transition(
    ...
    if (!TRANSITIONS[version.status].has(target))
      throw new ArtifactEngineError(
        `invalid transition ${version.status}->${target}`,
      );
    if (target === "APPROVED" && !validationPassed)
      throw new ArtifactEngineError("APPROVED requires validation");
    const updated = {
      ...version,
      status: target,
  ...
  addProvenance(value: ArtifactProvenance): void {
    ...
    if (version.constraint_snapshot_hash !== value.constraint_snapshot_hash)
      throw new ArtifactEngineError("constraint snapshot mismatch");
    if (this.provenance.has(value.artifact_version_id))
      throw new ArtifactImmutableError("provenance is immutable");
    this.provenance.set(value.artifact_version_id, structuredClone(value));
  }

  async attachVerifiedFile(
    value: ArtifactFile,
    ...
    assertTenant(version.organization_id, value.organization_id);
    if (value.storage_key.includes("://"))
      throw new ArtifactEngineError("storage_key must not be a URL");
    if (
      [...this.files.values()].some(
        (f) =>
          f.artifact_version_id === value.artifact_version_id &&
          f.role === value.role,
      )
    ) {
    ...
    if (stat.checksum_sha256 !== value.checksum_sha256)
      throw new ArtifactEngineError("storage checksum mismatch");
    if (stat.size_bytes !== value.size_bytes)
      throw new ArtifactEngineError("storage size mismatch");
    if (stat.mime_type && stat.mime_type !== value.mime_type)
      throw new ArtifactEngineError("storage MIME mismatch");
    this.files.set(value.id, structuredClone(value));
  }

  addEdge(value: ArtifactLineageEdge): void {
    if (value.from_version_id === value.to_version_id)
      throw new ArtifactEngineError("lineage self-loop forbidden");
    const source = this.versions.get(value.from_version_id);
    const target = this.versions.get(value.to_version_id);
    if (!source || !target)
      throw new ArtifactEngineError("lineage endpoints missing");
    assertTenant(source.organization_id, target.organization_id);
    assertTenant(source.organization_id, value.organization_id);
    if (this.reachable(value.to_version_id, value.from_version_id))
      throw new ArtifactEngineError("lineage cycle forbidden");
  ...
  restore(
    ...
    const branch = this.branches.get(branchId);
    if (!source || !branch || branch.artifact_id !== source.artifact_id)
      throw new ArtifactEngineError("restore source/branch invalid");
  ...
  private reachable(start: string, goal: string): boolean {
    ...
    while (pending.length) {
      ...
      for (const edge of this.edges.values())
        if (edge.from_version_id === current) pending.push(edge.to_version_id);
    }
    return false;
  }
}
