...
export class InMemoryExportJobRepository implements ExportJobRepository {
  ...
  async findReadyByFingerprint(
    organizationId: string,
    fingerprint: string,
    minimumExpiresAtIso: string,
  ): Promise<ExportJob | null> {
    for (const job of this.byId.values()) {
      if (
        job.organization_id === organizationId &&
        job.export_fingerprint === fingerprint &&
        job.status === "READY" &&
        job.expires_at >= minimumExpiresAtIso
      )
        return job;
    }
    return null;
  ...
  async save(job: ExportJob): Promise<void> {
    ...
    if (existing && existing.export_fingerprint !== job.export_fingerprint)
...
export class StaticExportSource implements ExportSourcePort {
  ...
  add(snapshot: ExportSourceSnapshot): void {
    this.snapshots.set(
      `${snapshot.organization_id}:${snapshot.artifact_version_id}:${snapshot.design_document_version_id}`,
      snapshot,
    );
  }
  ...
  async resolveExactSnapshot(spec: ExportSpec): Promise<ExportSourceSnapshot> {
    ...
    const snapshot = this.snapshots.get(
      `${spec.organization_id}:${spec.artifact_version_id}:${spec.design_document_version_id}`,
    );
...
export class InMemoryExportObjectStore implements ExportObjectStore {
  ...
  async put(
    ...
    if (!storageKey || storageKey.includes("://") || storageKey.includes(".."))
      throw new Error("EXPORT_STORAGE_KEY_UNSAFE");
...
export class RecordingExportArtifacts implements ExportArtifactPort {
  readonly persisted: Array<
    Parameters<ExportArtifactPort["persistExport"]>[0]
  > = [];
  async persistExport(
    args: Parameters<ExportArtifactPort["persistExport"]>[0],
  ): Promise<void> {
...
export class RecordingExportValidationEvidence
  implements ExportValidationEvidencePort
{
  readonly records: Array<
    Parameters<ExportValidationEvidencePort["record"]>[0]
  > = [];
  async record(
    args: Parameters<ExportValidationEvidencePort["record"]>[0],
  ): Promise<void> {
...
export class RecordingExportDownloadAudit implements ExportDownloadAuditPort {
  readonly records: Array<Parameters<ExportDownloadAuditPort["record"]>[0]> =
    [];
  async record(
    args: Parameters<ExportDownloadAuditPort["record"]>[0],
  ): Promise<void> {
    this.records.push(args);
  }
}
...
export class RecordingExportSigner implements ExportDownloadSignerPort {
  ...
  async sign(args: {
    ...
    return {
      ...
      expires_at: new Date(
        Date.parse("2030-01-01T00:00:00.000Z") + args.expires_seconds * 1000,
      ).toISOString(),
    };
  }
}
