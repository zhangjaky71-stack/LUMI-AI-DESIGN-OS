...
function readU16(bytes: Uint8Array, offset: number): number {
  if (offset < 0 || offset + 2 > bytes.length)
    throw new Error("EXPORT_ZIP_READ_BOUNDS");
...
function parseLocalEntries(
  ...
  while (offset < centralOffset) {
    if (readU32(bytes, offset) !== 0x04034b50)
      throw new Error("EXPORT_ZIP_LOCAL_SIGNATURE_INVALID");
    if (readU16(bytes, offset + 8) !== 0)
      throw new Error("EXPORT_ZIP_COMPRESSION_UNSUPPORTED");
    const checksum = readU32(bytes, offset + 14);
    const compressedSize = readU32(bytes, offset + 18);
    const uncompressedSize = readU32(bytes, offset + 22);
    if (compressedSize !== uncompressedSize)
      throw new Error("EXPORT_ZIP_STORE_SIZE_MISMATCH");
    ...
    if (safeZipEntryName(name) !== name)
      throw new Error("EXPORT_ZIP_ENTRY_NONCANONICAL");
  ...
  if (offset !== centralOffset)
    throw new Error("EXPORT_ZIP_CENTRAL_OFFSET_MISMATCH");
  return entries;
}
...
function validateCentralDirectory(
  ...
  centralOffset: number,
  centralSize: number,
  local: readonly ParsedEntry[],
): void {
  let offset = centralOffset;
  const seen: string[] = [];
  for (const expected of local) {
    if (readU32(bytes, offset) !== 0x02014b50)
      throw new Error("EXPORT_ZIP_CENTRAL_SIGNATURE_INVALID");
    if (readU16(bytes, offset + 10) !== 0)
      throw new Error("EXPORT_ZIP_CENTRAL_COMPRESSION_UNSUPPORTED");
    ...
    if (
      name !== expected.name ||
      checksum !== expected.crc32 ||
      compressedSize !== expected.bytes.length ||
      uncompressedSize !== expected.bytes.length ||
      localOffset !== expected.local_offset
    ) {
  ...
  if (offset !== centralOffset + centralSize)
    throw new Error("EXPORT_ZIP_CENTRAL_SIZE_MISMATCH");
  if (new Set(seen).size !== seen.length)
    throw new Error("EXPORT_ZIP_ENTRY_DUPLICATE");
}

export function readStoreZipEntries(
  bytes: Uint8Array,
  ...
  if (disk !== 0 || centralDisk !== 0 || diskEntries !== totalEntries)
    throw new Error("EXPORT_ZIP_MULTIDISK_UNSUPPORTED");
  if (eocd + 22 + commentLength !== bytes.length)
    throw new Error("EXPORT_ZIP_TRAILING_DATA_FORBIDDEN");
  if (centralOffset + centralSize !== eocd)
    throw new Error("EXPORT_ZIP_EOCD_CENTRAL_MISMATCH");
  const local = parseLocalEntries(bytes, centralOffset);
  if (local.length !== totalEntries || !local.length)
    throw new Error("EXPORT_ZIP_ENTRY_COUNT_MISMATCH");
  if (new Set(local.map((entry) => entry.name)).size !== local.length)
    throw new Error("EXPORT_ZIP_ENTRY_DUPLICATE");
  validateCentralDirectory(bytes, centralOffset, centralSize, local);
  return new Map(
    local.map((entry) => [entry.name, Uint8Array.from(entry.bytes)] as const),
  );
}
