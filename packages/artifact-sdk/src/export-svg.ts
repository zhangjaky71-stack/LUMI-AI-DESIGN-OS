...
function finiteNumber(value: unknown, label: string): number {
  if (typeof value !== "number" || !Number.isFinite(value))
    throw new Error(`EXPORT_SVG_${label}_INVALID`);
  return value;
}
function asMatrix(value: unknown): MatrixLike {
  if (!value || typeof value !== "object")
    throw new Error("EXPORT_SVG_MATRIX_INVALID");
...
function asBounds(value: unknown): BoundsLike {
  if (!value || typeof value !== "object")
    throw new Error("EXPORT_SVG_BOUNDS_INVALID");
  const row = value as Record<string, unknown>;
  return {
...
function asMetrics(value: unknown): TextMetricsLike | undefined {
  ...
  if (!value || typeof value !== "object")
    throw new Error("EXPORT_TEXT_METRICS_INVALID");
  const row = value as Record<string, unknown>;
  return {
    width: finiteNumber(row.width, "TEXT_METRICS_WIDTH"),
    height: finiteNumber(row.height, "TEXT_METRICS_HEIGHT"),
    baseline: finiteNumber(row.baseline, "TEXT_METRICS_BASELINE"),
  };
}
function parseRenderPlan(value: unknown): RenderPlanLike {
  ...
  const row = value as Record<string, unknown>;
  if (
    typeof row.compiler_version !== "string" ||
  ...
  const items: RenderItemLike[] = row.items.map((raw) => {
    ...
    if (item.resolved_text !== undefined) {
      if (!item.resolved_text || typeof item.resolved_text !== "object")
        throw new Error("EXPORT_RENDER_TEXT_INVALID");
      const text = item.resolved_text as Record<string, unknown>;
      if (typeof text.content !== "string")
        throw new Error("EXPORT_RENDER_TEXT_INVALID");
      const metrics = asMetrics(text.metrics);
      const font =
        text.font && typeof text.font === "object"
          ? (text.font as Record<string, unknown>)
          : undefined;
      resolvedText = {
    ...
    const resolvedResource =
      item.resolved_resource && typeof item.resolved_resource === "object"
    ...
    return {
      ...
      ...(typeof item.clip_id === "string" ? { clip_id: item.clip_id } : {}),
      ...(typeof item.mask_id === "string" ? { mask_id: item.mask_id } : {}),
...
function parseDesignDocument(value: unknown): DesignDocumentLike {
  ...
  return {
    document_id: row.document_id,
    nodes: row.nodes as Readonly<Record<string, DesignNodeLike>>,
  };
}
...
function isDescendant(
  item: RenderItemLike,
  frameId: string,
  byId: ReadonlyMap<string, RenderItemLike>,
): boolean {
...
function vectorPath(document: DesignDocumentLike, id: string): string {
  ...
  if (typeof raw !== "string" || !raw.trim())
    throw new Error("EXPORT_VECTOR_PATH_GEOMETRY_REQUIRED");
  if (/[<>]/.test(raw)) throw new Error("EXPORT_VECTOR_PATH_UNSAFE");
  return raw;
}
function assertVisualSemantics(item: RenderItemLike): void {
  if (item.clip_id || item.mask_id)
    throw new Error("EXPORT_SVG_MASK_CLIP_NOT_IMPLEMENTED_V1");
  const blend = item.resolved_style.blend_mode;
  if (typeof blend === "string" && blend.toLowerCase() !== "normal")
    throw new Error("EXPORT_SVG_BLEND_MODE_NOT_IMPLEMENTED_V1");
}
function itemStyle(item: RenderItemLike): string {
  ...
  const strokeWidth = Math.max(
    0,
    styleNumber(item.resolved_style, "stroke_width", 0),
  );
  const opacity = Math.min(
    1,
    Math.max(0, styleNumber(item.resolved_style, "opacity", 1)),
  );
  return `fill="${escapeXml(fill)}" stroke="${escapeXml(stroke)}" stroke-width="${strokeWidth}" opacity="${opacity}"`;
}
function explicitPixels(
  value: number | undefined,
  unit: ExportVariant["unit"],
  dpi: number,
): number | undefined {
  if (value === undefined) return undefined;
  if (!Number.isFinite(value) || value <= 0)
    throw new Error("EXPORT_TARGET_DIMENSIONS_INVALID");
  const pixels =
    unit === "MM" ? (value / 25.4) * dpi : unit === "IN" ? value * dpi : value;
  const rounded = Math.round(pixels);
  if (!Number.isSafeInteger(rounded) || rounded <= 0 || rounded > 32768)
    throw new Error("EXPORT_TARGET_DIMENSIONS_INVALID");
  return rounded;
}
function textMarkup(item: RenderItemLike, transform: string): string {
  ...
  const lineHeight = Math.max(
    size,
    styleNumber(item.resolved_style, "line_height", size * 1.2),
  );
  const weight = item.resolved_text?.font?.weight ?? 400;
  const fontStyle = item.resolved_text?.font?.style ?? "normal";
  const fill = safePaint(item.resolved_style.fill, "#000000");
  const align =
    typeof item.resolved_style.align === "string"
      ? item.resolved_style.align.toLowerCase()
      : "left";
  const x =
    align === "center"
      ? item.local_bounds.width / 2
      : align === "right"
        ? item.local_bounds.width
        : 0;
  const anchor =
  ...
  if (
    metrics &&
    Math.abs(metrics.height - Math.max(1, lines.length) * lineHeight) > 1
  )
    throw new Error("EXPORT_TEXT_METRICS_LAYOUT_UNSUPPORTED_V1");
  const tspans = lines
    .map(
      (line, index) =>
        `<tspan x="${x}" dy="${index === 0 ? size : lineHeight}">${escapeXml(line)}</tspan>`,
    )
    .join("");
...
export class SafeSvgRenderPlanSerializer {
  ...
  async renderPages(
    ...
    if (
      ...
      plan.document_id !== source.compiler_provenance.document_id
    )
      throw new Error("EXPORT_SOURCE_DOCUMENT_IDENTITY_MISMATCH");
    if (!/^[0-9a-f]{64}$/.test(source.compiler_provenance.compile_hash))
      throw new Error("EXPORT_COMPILER_HASH_REQUIRED");
    ...
    for (const frameId of variant.frame_ids) {
      ...
      if (!frame || frame.kind !== "FRAME" || !frame.visible)
        throw new Error(`EXPORT_FRAME_NOT_FOUND:${frameId}`);
      if (frame.placeholder)
        throw new Error(`EXPORT_FRAME_PLACEHOLDER:${frameId}`);
      const sourceWidth = frame.world_bounds.width;
      const sourceHeight = frame.world_bounds.height;
      if (sourceWidth <= 0 || sourceHeight <= 0)
        throw new Error("EXPORT_FRAME_DIMENSIONS_INVALID");
      const scale = variant.scale ?? 1;
      if (!Number.isFinite(scale) || scale <= 0 || scale > 16)
        throw new Error("EXPORT_SCALE_INVALID");
      const width =
        explicitPixels(variant.width, unit, dpi) ??
        Math.max(1, Math.round(sourceWidth * scale));
      const height =
        explicitPixels(variant.height, unit, dpi) ??
      ...
      if (this.#resources.embeddedFontCss)
        for (const family of [...fonts].sort()) {
          const css = await this.#resources.embeddedFontCss({
            family,
            organization_id: source.organization_id,
            project_id: source.project_id,
          });
          if (css) {
            if (/@import|url\((?!['"]?data:)/i.test(css))
              throw new Error("EXPORT_SVG_EXTERNAL_FONT_FORBIDDEN");
            fontCss.push(css);
          }
        }
      const body: string[] = [];
      for (const item of selected) {
        if (
          ["DOCUMENT_ROOT", "GROUP", "COMPONENT", "INSTANCE", "GUIDE"].includes(
            item.kind,
          )
        )
          continue;
        if (item.placeholder)
          throw new Error(`EXPORT_RENDER_PLACEHOLDER:${item.id}`);
        const transform = matrixAttr(item.world_matrix);
        const bounds = item.local_bounds;
        if (["FRAME", "SHAPE", "MASK"].includes(item.kind)) {
          body.push(
            `<rect data-node-id="${escapeXml(item.id)}" x="0" y="0" width="${bounds.width}" height="${bounds.height}" transform="${transform}" ${itemStyle(item)}/>`,
          );
          continue;
        }
        if (item.kind === "VECTOR_PATH") {
          body.push(
            `<path data-node-id="${escapeXml(item.id)}" d="${escapeXml(vectorPath(document, item.id))}" transform="${transform}" ${itemStyle(item)}/>`,
          );
          continue;
        }
        if (item.kind === "TEXT") {
          body.push(textMarkup(item, transform));
          continue;
        }
        if (item.kind === "IMAGE") {
          const resource = item.resolved_resource;
          if (!resource || resource.status !== "READY")
            throw new Error("EXPORT_IMAGE_RESOURCE_NOT_READY");
          const href = dataImageUri(
            await this.#resources.imageDataUri({
          ...
          body.push(
            `<image data-node-id="${escapeXml(item.id)}" x="0" y="0" width="${bounds.width}" height="${bounds.height}" transform="${transform}" href="${href}" preserveAspectRatio="xMidYMid meet"/>`,
          );
          continue;
        }
        if (item.kind === "VIDEO")
          throw new Error("EXPORT_VIDEO_NODE_REQUIRES_NODE_48");
        throw new Error(`EXPORT_NODE_KIND_UNSUPPORTED:${item.kind}`);
      }
      const background = variant.transparent_background
        ? ""
        : `<rect x="${frame.world_bounds.x}" y="${frame.world_bounds.y}" width="${sourceWidth}" height="${sourceHeight}" fill="${escapeXml(safePaint(variant.background, "#ffffff"))}"/>`;
      const style = fontCss.length
        ? `<style>${fontCss.join("\n")}</style>`
        : "";
      const preserveAspectRatio =
        (variant.resize_mode ?? "SCALE") === "CROP"
          ? "xMidYMid slice"
          : "xMidYMid meet";
      ...
      if (/\b(?:href|src)=["']https?:/i.test(svg) || /<script\b/i.test(svg))
        throw new Error("EXPORT_SVG_SANITIZE_FAILED");
