...
export class ArtifactEngineExportAdapter implements ExportArtifactPort {
  ...
  constructor(args: {
    ...
    if (!GIT_SHA.test(args.code_git_sha))
      throw new Error("EXPORT_CODE_GIT_SHA_INVALID");
  ...
  async persistExport(
    ...
    if (
      ...
      source.artifact_id !== job.source.artifact_id
    )
      throw new Error("EXPORT_SOURCE_ARTIFACT_VERSION_NOT_IN_HISTORY");
    if (
      source.content_hash !== job.source.content_hash ||
      source.constraint_snapshot_hash !== job.source.constraint_snapshot_hash
    )
    ...
    for (const output of outputs) {
      ...
      if (
        ...
        throw new Error("EXPORT_ARTIFACT_UUID_INVALID");
      ...
      this.#engine.addProvenance({
        ...
        ...(job.source.brand_rule_set_version
          ? { brand_rule_set_version: job.source.brand_rule_set_version }
          : {}),
