...
import {
  CompositeExportRenderer,
  type ExportRasterCodecPort,
...
function snapshot(
  overrides: Partial<ExportSourceSnapshot> = {},
): ExportSourceSnapshot {
  return {
    ...
    design_document: {
      ...
      nodes: {
        ...
        "path-1": {
          id: "path-1",
          kind: "VECTOR_PATH",
          metadata: { svg_path: "M0 0 L20 0 L20 20 Z" },
        },
      ...
      items: [
        {
          ...
          z_order: 0,
          visible: true,
          world_matrix: { a: 1, b: 0, c: 0, d: 1, tx: 0, ty: 0 },
          local_bounds: { x: 0, y: 0, width: 100, height: 100 },
          world_bounds: { x: 0, y: 0, width: 100, height: 100 },
        ...
        },
        {
          ...
          z_order: 1,
          visible: true,
          world_matrix: { a: 1, b: 0, c: 0, d: 1, tx: 10, ty: 10 },
          local_bounds: { x: 0, y: 0, width: 20, height: 20 },
          world_bounds: { x: 10, y: 10, width: 20, height: 20 },
          resolved_style: { fill: "#111111" },
          placeholder: false,
        },
        {
          id: "text-1",
          ...
          z_order: 2,
          visible: true,
          world_matrix: { a: 1, b: 0, c: 0, d: 1, tx: 10, ty: 40 },
          local_bounds: { x: 0, y: 0, width: 60, height: 20 },
          world_bounds: { x: 10, y: 40, width: 60, height: 20 },
          resolved_style: { fill: "#111111", font_size: 14 },
          resolved_text: { content: "Hello" },
          placeholder: false,
        },
        {
          id: "image-1",
          ...
          z_order: 3,
          visible: true,
          world_matrix: { a: 1, b: 0, c: 0, d: 1, tx: 70, ty: 70 },
          local_bounds: { x: 0, y: 0, width: 20, height: 20 },
          world_bounds: { x: 70, y: 70, width: 20, height: 20 },
          resolved_style: {},
          resolved_resource: {
            asset_id: "asset-1",
            version: "v1",
...
function serializer(): SafeSvgRenderPlanSerializer {
  return new SafeSvgRenderPlanSerializer({
    async imageDataUri() {
...
describe("NODE-49 exact Export Engine", () => {
  it("pins the exact ArtifactVersion and DesignVersion before worker execution", async () => {
    ...
    const ready = await h.engine.execute(
  ...
  it("builds safe batch ZIP, deterministic manifest and unicode filenames", async () => {
    ...
    const ready = await h.engine.execute(
      "org-1",
      (await h.engine.start(spec())).export_job_id,
    );
    expect(ready.status).toBe("READY");
    ...
    const zip = ready.files.find(
      (file) => file.mime_type === "application/zip",
    )!;
    expect(inspectZipEntries(await h.store.get(zip.storage_key))).toEqual(
      expect.arrayContaining(["files/设计稿.svg", "manifest.json"]),
    );
  });
  ...
  it("authorizes each re-download and only re-signs the existing object", async () => {
    ...
    const denied = new ExportDownloadService({
      jobs: h.jobs,
      authorization: new StaticExportAuthorization(false),
      signer,
      audit,
    ...
    const allowed = new ExportDownloadService({
      jobs: h.jobs,
      authorization: new StaticExportAuthorization(true),
      signer,
      audit,
      now: () => "2026-08-14T00:10:00.000Z",
    });
    await allowed.download({
      organization_id: "org-1",
      actor_id: "u",
      export_job_id: ready.export_job_id,
      file_id: file.file_id,
  ...
  it("rejects floating versions, false color claims, PSD and hidden metadata", async () => {
    const h = harness();
    await expect(
      h.engine.start(spec({ artifact_version_id: "latest" })),
    ).rejects.toThrow("EXPORT_FLOATING_VERSION_FORBIDDEN");
    expect(() => assertExportProfile("CMYK")).toThrow(
      "EXPORT_CMYK_NOT_SUPPORTED_V1",
    );
    expect(() => assertExportProfile("DISPLAY_P3")).toThrow(
      "EXPORT_DISPLAY_P3_NOT_VERIFIED_V1",
    );
    ...
    await expect(hidden.engine.start(spec())).rejects.toThrow(
      "EXPORT_SENSITIVE_METADATA_FORBIDDEN",
    );
  });
    ...
    const h = harness(
      snapshot({
        render_plan: {
          ...
          document_id: "doc-1",
          items: [],
          uri: "https://signed.invalid/x",
        },
      }),
    );
    await expect(h.engine.start(spec())).rejects.toThrow(
      "EXPORT_EPHEMERAL_RUNTIME_REF_FORBIDDEN",
    );
  });
});
...
describe("NODE-49 format and package security", () => {
  it("sanitizes filenames and blocks zip-slip", () => {
    ...
    expect(() => safeZipEntryName("../../etc/passwd")).toThrow(
      "EXPORT_ZIP_TRAVERSAL_FORBIDDEN",
    );
    expect(() => safeZipEntryName("C:/Windows/system.ini")).toThrow(
      "EXPORT_ZIP_ABSOLUTE_PATH_FORBIDDEN",
    );
    const zip = writeStoreZip([
      { name: "文件/海报.txt", bytes: new TextEncoder().encode("ok") },
    ]);
  ...
  it("writes and independently parses multi-page PDF boxes", () => {
    ...
    expect(inspection.media_boxes).toEqual([
      { width_pt: 720, height_pt: 720 },
      { width_pt: 432, height_pt: 216 },
    ]);
  });

  it("serializes actual Canvas tx/ty matrices, vector path, image and font fallback", async () => {
    const [page] = await serializer().renderPages(renderableSource(), {
      variant_id: "svg",
      frame_ids: ["frame-1"],
      format: "SVG",
    });
  ...
  it("converts mm/in at DPI and distinguishes SCALE from CROP", async () => {
    const scale = (
      await serializer().renderPages(renderableSource(), {
        variant_id: "scale",
        frame_ids: ["frame-1"],
        format: "SVG",
        width: 25.4,
  ...
  it("fails closed for print marks not implemented in V1", async () => {
    const codec: ExportRasterCodecPort = {
      async encodeSvg() {
        return { bytes: new Uint8Array([0x89]), mime_type: "image/png" };
      },
    };
    const renderer = new CompositeExportRenderer({
      svg: serializer(),
      raster: codec,
    });
    await expect(
      renderer.render(renderableSource(), {
        variant_id: "print",
        frame_ids: ["frame-1"],
        format: "PNG",
        bleed: 3,
      }),
    ).rejects.toThrow("EXPORT_PRINT_MARKS_NOT_IMPLEMENTED_V1");
  });
  ...
  it("builds a valid PDF from rasterized SVG pages", async () => {
    ...
    const output = await renderer.render(renderableSource(), {
      variant_id: "pdf",
      frame_ids: ["frame-1"],
      format: "PDF",
      dpi: 72,
  ...
  it("validates typed raster worker MIME and decoded dimensions", async () => {
    const codec = new WorkerBackedRasterCodec({
      async invoke(request) {
        return {
          bytes: new Uint8Array([1, 2, 3]),
          mime_type: request.format === "WEBP" ? "image/webp" : "image/png",
          width: request.width,
    ...
    const invalid = new WorkerBackedRasterCodec({
      async invoke() {
        return {
    ...
    ).rejects.toThrow("EXPORT_RASTER_WORKER_DIMENSIONS_MISMATCH");
  });
});
