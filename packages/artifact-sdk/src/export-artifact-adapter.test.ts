...
describe("NODE-49 Artifact integration", () => {
  it("creates READY export ArtifactVersion with verified file, provenance and EXPORTED_FROM lineage", async () => {
    ...
    const renderer: ExportRendererPort = {
      async render() {
        return {
          bytes: Uint8Array.from(PNG_SIGNATURE),
          mime_type: "image/png",
          width: 100,
          height: 80,
        };
    ...
    const artifacts = new ArtifactEngineExportAdapter({
    ...
    const outputVersions = [...artifactEngine.versions.values()].filter(
      (version) => version.id !== "artifact-version-0001",
    );
    expect(outputVersions).toHaveLength(2);
    expect(outputVersions.every((version) => version.status === "READY")).toBe(
      true,
    );
    expect(
      [...artifactEngine.edges.values()].filter(
        (edge) => edge.type === "EXPORTED_FROM",
      ),
    ).toHaveLength(2);
    expect(
      [...artifactEngine.edges.values()].every(
        (edge) => edge.from_version_id === "artifact-version-0001",
      ),
    ).toBe(true);
    expect(
      [...artifactEngine.provenance.values()].every(
        (record) => record.compiler?.compile_hash === HASH_C,
      ),
    ).toBe(true);
    expect(
      [...artifactEngine.files.values()].some(
        (file) =>
  ...
  it("rejects source Artifact history drift before attaching output", async () => {
    ...
    artifactEngine.versions.set("artifact-version-0001", {
      ...artifactEngine.versions.get("artifact-version-0001")!,
