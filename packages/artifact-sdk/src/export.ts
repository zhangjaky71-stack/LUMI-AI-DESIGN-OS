...
export class ArtifactExportRegistry {
  ...
  register(adapter: ArtifactExportAdapter): void {
    if (this.#adapters.has(adapter.format))
      throw new Error(`export adapter already registered: ${adapter.format}`);
  ...
  async render(request: ArtifactExportRequest): Promise<ArtifactExportPayload> {
    ...
    if (!adapter)
      throw new Error(`export adapter unavailable: ${request.format}`);
...
export async function buildArtifactExportManifest(
  ...
  if (version.id !== provenance.artifact_version_id)
    throw new Error("provenance/version mismatch");
  if (version.organization_id !== provenance.organization_id)
    throw new Error("provenance tenant mismatch");
  if (
