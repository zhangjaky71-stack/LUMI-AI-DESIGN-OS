...
function largeSubject(nodeCount = 2000): CriticSubject {
  const nodes: Record<string, DesignDocument["nodes"][string]> = {
    ...
    frame: {
      ...
      parent_id: "root",
      children: [],
      transform: { x: 0, y: 0, width: 1200, height: 1200 },
    },
  };
  ...
  const design_document: DesignDocument = {
    schema_version: "1.0",
...
describe("NODE-50 deterministic critic scale harness", () => {
  it("evaluates a 2k-node document deterministically without model access", () => {
    ...
    expect(
      first.some((item) => item.dimension === "RESOLUTION_EXPORT_READINESS"),
    ).toBe(true);
    expect(first.flatMap((item) => item.repair_operations ?? [])).toHaveLength(
      0,
    );
  });
});
