...
function seeded(): ArtifactEngine {
  ...
  const version: ArtifactVersion = {
    ...
    status: "READY",
    content_hash: HASH,
    constraint_snapshot_hash: HASH,
    created_by_type: "SYSTEM",
    created_by_id: "fixture",
...
describe("NODE-50 Artifact QualityResult integration", () => {
  it("persists full result and appends only normalized quality metadata to exact ArtifactVersion", async () => {
    ...
    const critic = new QualityEngine({
      ports: { artifact: adapter },
    ...
    expect(
      (await results.get(ORG, result.quality_result_id))?.overall_score,
    ).toBe(100);
