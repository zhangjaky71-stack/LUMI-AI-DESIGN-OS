...
export class ArtifactEngineQualityAdapter implements QualityArtifactPort {
  ...
  async record(result: QualityResult): Promise<void> {
    ...
    if (
      artifact.organization_id !== result.organization_id ||
      version.organization_id !== result.organization_id ||
      artifact.project_id !== result.project_id
    ) {
      throw new Error("QUALITY_ARTIFACT_SCOPE_MISMATCH");
    }
    if (
      version.design_document_version_id !== result.design_document_version_id
    ) {
    ...
    const existing = await this.#results.get(
    ...
    if (existing) {
      if (
        ...
        existing.profile_id !== result.profile_id ||
        existing.profile_version !== result.profile_version
      ) {
    ...
    this.#artifacts.versions.set(version.id, {
      ...version,
      quality_score: result.overall_score / 100,
    });
  }
}

export class InMemoryQualityResultRepository
  implements QualityResultRepository
{
  ...
  async save(result: QualityResult): Promise<void> {
    ...
    if (existing && existing.artifact_version_id !== result.artifact_version_id)
      throw new Error("QUALITY_RESULT_ID_CONFLICT");
