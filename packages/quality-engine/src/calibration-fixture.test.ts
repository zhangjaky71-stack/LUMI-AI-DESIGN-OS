...
describe("NODE-50 calibration corpus", () => {
  it("recomputes the pinned FP/FN metrics instead of trusting a narrative report", () => {
    ...
    expect(actual.precision).toBeCloseTo(
      fixture.expected_metrics.precision,
      12,
    );
    ...
    expect(actual.false_positive_rate).toBeCloseTo(
      fixture.expected_metrics.false_positive_rate,
      12,
    );
    expect(actual.false_negative_rate).toBeCloseTo(
      fixture.expected_metrics.false_negative_rate,
      12,
    );
    expect(
      validateCalibration({
