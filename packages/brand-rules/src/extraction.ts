...
export function createExtractionProposal(
  input: Omit<BrandGuideExtractionProposal, "status" | "candidates"> & {
    readonly candidates: readonly BrandGuideExtractionCandidate[];
  },
): BrandGuideExtractionProposal {
  const candidates = input.candidates.map((candidate) => {
    if (candidate.confidence < 0 || candidate.confidence > 1)
      throw new BrandRuleError("extraction confidence must be in [0,1]");
    if (!candidate.citations.length)
      throw new BrandRuleError(
        `candidate ${candidate.candidate_id} requires source citations`,
      );
    if (candidate.rule.severity === "HARD")
      throw new BrandRuleError(
        "unreviewed extraction cannot propose a HARD rule",
      );
    return {
      ...candidate,
      rule: {
        ...candidate.rule,
        source: "INFERRED_PROPOSAL" as const,
...
export function approveExtractionProposal(
  ...
  if (proposal.status !== "PROPOSED")
    throw new BrandRuleError("only PROPOSED extraction can be approved");
  if (!reviewer)
    throw new BrandRuleError("extraction approval requires reviewer identity");
  const approvalMap = new Map(
    approvals.map((item) => [item.candidate_id, item]),
  );
  ...
  for (const candidate of proposal.candidates) {
    ...
    if (!candidate.citations.length)
      throw new BrandRuleError(
        `candidate ${candidate.candidate_id} cannot be approved without citations`,
      );
    approvedRules.push({
      ...candidate.rule,
  ...
  if (!approvedRules.length)
    throw new BrandRuleError(
      "approval must select at least one extraction candidate",
    );
  return {
    proposal: {
      ...proposal,
    ...
    approved_rules: approvedRules.sort(
...
export function rejectExtractionProposal(
  ...
  if (proposal.status !== "PROPOSED")
    throw new BrandRuleError("only PROPOSED extraction can be rejected");
  if (!reviewer)
    throw new BrandRuleError("extraction rejection requires reviewer identity");
  return {
    ...proposal,
    status: "REJECTED",
    reviewed_by: reviewer,
