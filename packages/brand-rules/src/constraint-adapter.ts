...
export interface BrandComplianceResolver {
  ...
  ): Promise<BrandEvaluationContext>;
}
...
export class BrandConstraintAdapter implements BrandComplianceValidator {
  ...
  async validate(
    ...
    if (
      constraint.type !== "REQUIRE_BRAND_COMPLIANCE" &&
      constraint.type !== "LOCK_BRAND"
    )
      return [];
    try {
      ...
      return report.diagnostics.map(
        (item): ConstraintViolation => ({
          ...
          repair_hint: {
            ...
            operation_ids: (item.repair_operations ?? []).map(
