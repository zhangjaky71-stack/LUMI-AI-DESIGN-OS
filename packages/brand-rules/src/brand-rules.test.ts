...
const draft: BrandRuleSet = {
  ...
  rules,
  voice: {
    tone_attributes: ["precise"],
    preferred_vocabulary: ["crafted"],
...
describe("NODE-43 Brand Rules Engine", () => {
  it("detects deterministic hard/soft brand violations and returns Design IR repair operations", () => {
    ...
    expect(
      report.diagnostics.some(
        (item) => item.reason_code === "BRAND_COLOR_FORBIDDEN",
      ),
    ).toBe(true);
    expect(
      report.diagnostics.some(
        (item) => item.reason_code === "BRAND_LOGO_CLEAR_SPACE_VIOLATION",
      ),
    ).toBe(true);
    expect(
      report.diagnostics.some(
        (item) => item.reason_code === "BRAND_FONT_NOT_ALLOWED",
      ),
    ).toBe(true);
    const colorFix = report.diagnostics.find((item) => item.rule_id === "color")
      ?.repair_operations?.[0];
  ...
  it("keeps approved brand rules pinned in compact BrandContext", () => {
    ...
    expect(context.hard_rules.every((rule) => rule.severity === "HARD")).toBe(
      true,
    );
  });
  ...
  it("requires citations and human approval before extracted rules can become HARD", () => {
    ...
    const approved = approveExtractionProposal(
      proposal,
      [{ candidate_id: "candidate-1", severity: "HARD" }],
      "reviewer-1",
      "2026-08-14T00:02:00Z",
    );
    expect(approved.approved_rules[0]?.source).toBe(
  ...
  it("fails closed on stale token/rule-set versions and artifact approval mismatch", () => {
    expect(() =>
      evaluateBrandCompliance({
        document,
        rule_set: published,
        token_set: { ...tokenSet, version: "2.0.0" },
        asset_set: assetSet,
      }),
    ...
    ).toBe("BRAND_RULE_VERSION_MISMATCH");
  });
});
