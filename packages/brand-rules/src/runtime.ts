...
const normalizedColor = (value: string): string => value.trim().toLowerCase();
const nodeColor = (node: DesignNode): string | undefined => firstString(node, ["fill", "color", "background_color", "style.fill", "metadata.fill"]);
const nodeAssetId = (node: DesignNode): string | undefined => firstString(node, ["asset_id", "resource_id", "metadata.asset_id", "metadata.resource_id"]);
const nodeFontAssetId = (node: DesignNode): string | undefined => firstString(node, ["font_asset_id", "typography.font_asset_id", "metadata.font_asset_id"]);
const nodeText = (node: DesignNode): string | undefined => firstString(node, ["text", "content", "metadata.text"]);
const nodeBrandBinding = (node: DesignNode): string | undefined => firstString(node, ["brand_binding", "metadata.brand_binding"]);

function applies(rule: BrandRule, node: DesignNode, context: BrandEvaluationContext): boolean {
  if (rule.scope.node_ids?.length && !rule.scope.node_ids.includes(node.id)) return false;
  if (rule.scope.roles?.length && (!node.role || !rule.scope.roles.includes(node.role))) return false;
  if (rule.scope.channels?.length && (!context.channel || !rule.scope.channels.includes(context.channel))) return false;
  if (rule.scope.locales?.length && (!context.locale || !rule.scope.locales.includes(context.locale))) return false;
  return true;
}

function repair(context: BrandEvaluationContext, nodeId: string, path: string, value: unknown, ruleId: string): DesignOperation {
  return {
...
function colorMap(tokenSet: BrandTokenSet): Map<string, string> {
...
function evaluateColor(rule: BrandRule, node: DesignNode, context: BrandEvaluationContext): BrandDiagnostic[] {
  ...
  if (rule.type === "ALLOWED_COLOR_TOKENS") {
    ...
    const map = colorMap(context.token_set);
    const allowed = new Set(tokenIds.map((id) => map.get(id)).filter((value): value is string => Boolean(value)));
    if (!allowed.size || allowed.has(normalized)) return [];
    const replacement = tokenIds.map((id) => map.get(id)).find((value): value is string => Boolean(value));
...
function evaluateTypography(rule: BrandRule, node: DesignNode, context: BrandEvaluationContext): BrandDiagnostic[] {
  ...
  if (rule.type === "ALLOWED_FONT_ASSETS") {
    ...
    const rightsDenied = assetId !== undefined && context.font_rights_allowed_asset_ids !== undefined && !context.font_rights_allowed_asset_ids.includes(assetId);
    if (!assetId || !allowed.has(assetId) || rightsDenied) {
      const replacement = [...allowed].find((id) => context.font_rights_allowed_asset_ids?.includes(id) ?? true);
      return [diagnostic(
        rule,
        rightsDenied ? "BRAND_FONT_RIGHTS_UNAVAILABLE" : "BRAND_FONT_NOT_ALLOWED",
        node.id,
        [...allowed],
        assetId ?? null,
        replacement ? [repair(context, node.id, "font_asset_id", replacement, rule.id)] : undefined,
      )];
    }
  }
  if (rule.type === "MIN_TEXT_SIZE") {
    ...
    if (typeof size === "number" && size < minimum) {
      return [diagnostic(rule, "BRAND_TEXT_TOO_SMALL", node.id, minimum, size, [repair(context, node.id, "font_size", minimum, rule.id)])];
    }
  }
  return [];
}
...
function overlapsWithMargin(a: { x: number; y: number; width: number; height: number }, b: { x: number; y: number; width: number; height: number }, margin: number): boolean {
  return !(b.x >= a.x + a.width + margin || b.x + b.width <= a.x - margin || b.y >= a.y + a.height + margin || b.y + b.height <= a.y - margin);
}
...
function evaluateLogo(rule: BrandRule, node: DesignNode, context: BrandEvaluationContext): BrandDiagnostic[] {
  ...
  if (rule.type === "ALLOWED_LOGO_ASSETS") {
    const allowed = new Set(stringArray(rule.parameters.asset_ids));
    if (!assetId || !allowed.has(assetId)) diagnostics.push(diagnostic(rule, "BRAND_LOGO_ASSET_NOT_ALLOWED", node.id, [...allowed], assetId ?? null));
    else if (context.verified_asset_ids && !context.verified_asset_ids.includes(assetId)) diagnostics.push(diagnostic(rule, "BRAND_LOGO_ASSET_NOT_VERIFIED", node.id, true, assetId));
  }
  if (rule.type === "LOGO_MIN_SIZE") {
    ...
    if (bounds) {
      ...
      if (bounds.width < minWidth || bounds.height < minHeight) diagnostics.push(diagnostic(rule, "BRAND_LOGO_TOO_SMALL", node.id, { min_width: minWidth, min_height: minHeight }, { width: bounds.width, height: bounds.height }));
    }
  }
  if (rule.type === "LOGO_FORBID_ROTATION") {
    ...
    if (Math.abs(rotation) > tolerance) diagnostics.push(diagnostic(rule, "BRAND_LOGO_ROTATED", node.id, 0, rotation, [repair(context, node.id, "transform.rotation_deg", 0, rule.id)]));
  }
  if (rule.type === "LOGO_FORBID_STRETCH") {
    const sx = node.transform?.scale_x ?? 1;
    const sy = node.transform?.scale_y ?? 1;
    if (Math.abs(sx - sy) > numberValue(rule.parameters.tolerance, 0.01)) diagnostics.push(diagnostic(rule, "BRAND_LOGO_STRETCHED", node.id, "uniform-scale", { scale_x: sx, scale_y: sy }));
  }
  if (rule.type === "LOGO_FORBID_RECOLOR" && nested(node, "metadata.logo_recolored") === true) diagnostics.push(diagnostic(rule, "BRAND_LOGO_RECOLORED", node.id, false, true));
...
function evaluateBinding(rule: BrandRule, node: DesignNode, context: BrandEvaluationContext): BrandDiagnostic[] {
  ...
  return [diagnostic(rule, "BRAND_TOKEN_BINDING_REQUIRED", node.id, prefixes, binding ?? null, suggested ? [repair(context, node.id, "brand_binding", suggested, rule.id)] : undefined)];
}

function evaluateSpacing(rule: BrandRule, node: DesignNode, context: BrandEvaluationContext): BrandDiagnostic[] {
  ...
  return [diagnostic(rule, "BRAND_SPACING_OFF_SCALE", node.id, scale, spacing, replacement !== undefined ? [repair(context, node.id, "metadata.spacing", replacement, rule.id)] : undefined)];
}
...
function evaluateVoice(rule: BrandRule, node: DesignNode): BrandDiagnostic[] {
  ...
  if (rule.type === "VOICE_FORBIDDEN_TERMS") {
    const terms = stringArray(rule.parameters.terms);
    const lowered = text.toLocaleLowerCase();
    const hit = terms.find((term) => lowered.includes(term.toLocaleLowerCase()));
    return hit ? [diagnostic(rule, "BRAND_VOICE_FORBIDDEN_TERM", node.id, terms, hit)] : [];
  }
  return [];
}
...
function evaluateRule(rule: BrandRule, context: BrandEvaluationContext): BrandDiagnostic[] {
  ...
  const diagnostics: BrandDiagnostic[] = [];
  for (const node of Object.values(context.document.nodes).sort((a, b) => a.id.localeCompare(b.id))) {
    if (!applies(rule, node, context)) continue;
    ...
    diagnostics.push(...evaluateSpacing(rule, node, context));
...
export function validateBrandRuleSet(ruleSet: BrandRuleSet): void {
  if (!ruleSet.id || !ruleSet.organization_id || !ruleSet.brand_profile_id || !ruleSet.version) throw new BrandRuleError("brand rule set identity is required");
  ...
  for (const rule of ruleSet.rules) {
    ...
    if (rule.source === "INFERRED_PROPOSAL" && rule.severity === "HARD") throw new BrandRuleError(`inferred proposal ${rule.id} cannot be HARD`);
    if (rule.source === "APPROVED_GUIDE_EXTRACTION" && !rule.citations?.length) throw new BrandRuleError(`approved guide rule ${rule.id} requires source citations`);
  }
  if (ruleSet.status === "PUBLISHED" && ruleSet.rules.some((rule) => rule.source === "INFERRED_PROPOSAL")) throw new BrandRuleError("published rule sets cannot contain unreviewed inferred proposals");
}
...
export function evaluateBrandCompliance(context: BrandEvaluationContext): BrandComplianceReport {
  ...
  if (context.token_set.brand_profile_id !== context.rule_set.brand_profile_id || context.asset_set.brand_profile_id !== context.rule_set.brand_profile_id) throw new BrandRuleError("brand profile mismatch");
  ...
  return {
    ...
    decision: hard ? "FAIL" : diagnostics.length ? "PASS_WITH_WARNINGS" : "PASS",
    score: Math.max(0, Math.min(1, 1 - hard * 0.25 - soft * 0.08 - advisory * 0.02)),
    diagnostics,
