...
const nodeColor = (node: DesignNode): string | undefined =>
  firstString(node, [
    "fill",
    "color",
    "background_color",
...
const nodeAssetId = (node: DesignNode): string | undefined =>
  firstString(node, [
    "asset_id",
    "resource_id",
    "metadata.asset_id",
...
const nodeFontAssetId = (node: DesignNode): string | undefined =>
  firstString(node, [
    "font_asset_id",
    "typography.font_asset_id",
    "metadata.font_asset_id",
  ]);
const nodeText = (node: DesignNode): string | undefined =>
  firstString(node, ["text", "content", "metadata.text"]);
const nodeBrandBinding = (node: DesignNode): string | undefined =>
  firstString(node, ["brand_binding", "metadata.brand_binding"]);
...
function applies(
  ...
  if (rule.scope.node_ids?.length && !rule.scope.node_ids.includes(node.id))
    return false;
  if (
    rule.scope.roles?.length &&
    (!node.role || !rule.scope.roles.includes(node.role))
  )
    return false;
  if (
    rule.scope.channels?.length &&
    (!context.channel || !rule.scope.channels.includes(context.channel))
  )
    return false;
  if (
    rule.scope.locales?.length &&
    (!context.locale || !rule.scope.locales.includes(context.locale))
  )
    return false;
  return true;
}
...
function colorMap(tokenSet: BrandTokenSet): Map<string, string> {
  return new Map(
    tokenSet.colors.map((token) => [token.id, normalizedColor(token.value)]),
  );
}
  ...
  if (rule.type === "ALLOWED_COLOR_TOKENS") {
    ...
    return [
      diagnostic(
        ...
        color,
        replacement
          ? [repair(context, node.id, "fill", replacement, rule.id)]
          : undefined,
      ),
    ];
  }
...
function evaluateTypography(
  ...
  if (rule.type === "ALLOWED_FONT_ASSETS") {
    ...
    const rightsDenied =
      assetId !== undefined &&
      context.font_rights_allowed_asset_ids !== undefined &&
      !context.font_rights_allowed_asset_ids.includes(assetId);
    if (!assetId || !allowed.has(assetId) || rightsDenied) {
      const replacement = [...allowed].find(
        (id) => context.font_rights_allowed_asset_ids?.includes(id) ?? true,
      );
      return [
        diagnostic(
          rule,
          ...
          assetId ?? null,
          replacement
            ? [repair(context, node.id, "font_asset_id", replacement, rule.id)]
            : undefined,
        ),
      ];
    }
  ...
  if (rule.type === "MIN_TEXT_SIZE") {
    const size =
      nested(node, "font_size") ??
      nested(node, "typography.font_size") ??
      nested(node, "metadata.font_size");
    const minimum = numberValue(rule.parameters.px, 0);
    if (typeof size === "number" && size < minimum) {
      return [
        diagnostic(rule, "BRAND_TEXT_TOO_SMALL", node.id, minimum, size, [
          repair(context, node.id, "font_size", minimum, rule.id),
        ]),
      ];
    }
  }
...
function evaluateLogo(
  ...
  if (rule.type === "ALLOWED_LOGO_ASSETS") {
    ...
    if (!assetId || !allowed.has(assetId))
      ...
      context.verified_asset_ids &&
      !context.verified_asset_ids.includes(assetId)
    )
      diagnostics.push(
        diagnostic(
          rule,
          "BRAND_LOGO_ASSET_NOT_VERIFIED",
          node.id,
          true,
          assetId,
        ),
  ...
  if (rule.type === "LOGO_FORBID_ROTATION") {
    ...
    if (Math.abs(rotation) > tolerance)
      diagnostics.push(
        diagnostic(rule, "BRAND_LOGO_ROTATED", node.id, 0, rotation, [
          repair(context, node.id, "transform.rotation_deg", 0, rule.id),
        ]),
      );
  }
  if (rule.type === "LOGO_FORBID_STRETCH") {
    ...
    if (Math.abs(sx - sy) > numberValue(rule.parameters.tolerance, 0.01))
      diagnostics.push(
        diagnostic(rule, "BRAND_LOGO_STRETCHED", node.id, "uniform-scale", {
          scale_x: sx,
          scale_y: sy,
        }),
      );
  ...
  if (
    rule.type === "LOGO_FORBID_RECOLOR" &&
    nested(node, "metadata.logo_recolored") === true
  )
    diagnostics.push(
      diagnostic(rule, "BRAND_LOGO_RECOLORED", node.id, false, true),
    );
...
function evaluateBinding(
  ...
  if (
    binding &&
    (!prefixes.length || prefixes.some((prefix) => binding.startsWith(prefix)))
  )
    return [];
  ...
  return [
    diagnostic(
      ...
      binding ?? null,
      suggested
        ? [repair(context, node.id, "brand_binding", suggested, rule.id)]
        : undefined,
    ),
  ];
}
...
function evaluateSpacing(
  ...
  if (scale.some((allowed) => Math.abs(allowed - spacing) <= tolerance))
    return [];
  const replacement = [...scale].sort(
    (a, b) => Math.abs(a - spacing) - Math.abs(b - spacing),
  )[0];
  return [
    diagnostic(
      rule,
      "BRAND_SPACING_OFF_SCALE",
      node.id,
      scale,
      spacing,
      replacement !== undefined
        ? [repair(context, node.id, "metadata.spacing", replacement, rule.id)]
        : undefined,
    ),
  ];
}
...
function evaluateAsset(
  ...
  if (
    ...
    !["IMAGE", "VIDEO", "VECTOR_PATH"].includes(node.kind)
  )
    return [];
  const assetId = nodeAssetId(node);
  const allowed = new Set(stringArray(rule.parameters.asset_ids));
  ...
  if (
    ...
    return [
      diagnostic(rule, "BRAND_ASSET_NOT_VERIFIED", node.id, true, assetId),
    ];
  return [];
}
...
function evaluateVoice(rule: BrandRule, node: DesignNode): BrandDiagnostic[] {
  ...
  if (rule.type === "VOICE_FORBIDDEN_TERMS") {
    ...
    return hit
      ? [diagnostic(rule, "BRAND_VOICE_FORBIDDEN_TERM", node.id, terms, hit)]
      : [];
  }
  return [];
}
...
function evaluateRule(
  ...
  const diagnostics: BrandDiagnostic[] = [];
  for (const node of Object.values(context.document.nodes).sort((a, b) =>
...
export function validateBrandRuleSet(ruleSet: BrandRuleSet): void {
  if (
    ...
    !ruleSet.brand_profile_id ||
    !ruleSet.version
  )
    throw new BrandRuleError("brand rule set identity is required");
  if (
    new Set(ruleSet.rules.map((rule) => rule.id)).size !== ruleSet.rules.length
  )
    throw new BrandRuleError("brand rule ids must be unique");
  for (const rule of ruleSet.rules) {
    if (!Number.isFinite(rule.priority))
      throw new BrandRuleError(`rule ${rule.id} priority must be finite`);
    ...
    if (rule.source === "APPROVED_GUIDE_EXTRACTION" && !rule.citations?.length)
      throw new BrandRuleError(
        `approved guide rule ${rule.id} requires source citations`,
      );
  }
  if (
    ruleSet.status === "PUBLISHED" &&
    ruleSet.rules.some((rule) => rule.source === "INFERRED_PROPOSAL")
  )
    throw new BrandRuleError(
      "published rule sets cannot contain unreviewed inferred proposals",
    );
}

export function publishBrandRuleSet(
  ruleSet: BrandRuleSet,
  publishedAt: string,
): BrandRuleSet {
  const candidate: BrandRuleSet = {
    ...ruleSet,
    status: "PUBLISHED",
...
export function evaluateBrandCompliance(
  ...
  if (context.rule_set.status !== "PUBLISHED")
    throw new BrandRuleError("compliance requires a PUBLISHED BrandRuleSet");
  if (context.token_set.version !== context.rule_set.token_set_version)
    throw new BrandRuleError("token set version mismatch");
  if (context.asset_set.version !== context.rule_set.asset_set_version)
    throw new BrandRuleError("asset set version mismatch");
  if (
    context.token_set.brand_profile_id !== context.rule_set.brand_profile_id ||
    context.asset_set.brand_profile_id !== context.rule_set.brand_profile_id
  )
    throw new BrandRuleError("brand profile mismatch");
  ...
  return {
    ...
    score: Math.max(
      0,
      Math.min(1, 1 - hard * 0.25 - soft * 0.08 - advisory * 0.02),
    ),
    diagnostics,
