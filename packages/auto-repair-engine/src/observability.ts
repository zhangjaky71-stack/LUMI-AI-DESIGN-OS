...
export function autoRepairTelemetry(
  ...
  return {
    ...
    promoted_ready_count: result.attempts.filter(
      (item) => item.disposition === "PROMOTED_READY",
    ).length,
    promoted_draft_count: result.attempts.filter(
      (item) => item.disposition === "PROMOTED_DRAFT",
    ).length,
    rejected_count: result.attempts.filter(
      (item) => item.disposition === "REJECTED",
    ).length,
    action_kinds: [
      ...new Set(result.attempts.map((item) => item.action_kind)),
    ].sort(),
    reason_codes: [
      ...new Set([
        ...result.reason_codes,
        ...result.attempts.flatMap(
          (item: RepairAttemptRecord) => item.reason_codes,
        ),
      ]),
    ].sort(),
  };
