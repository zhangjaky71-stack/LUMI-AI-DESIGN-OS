...
function candidateSubject(
  ...
  return {
    ...
    ...(source.subject.expected_text
      ? { expected_text: source.subject.expected_text }
      : {}),
...
export class AutoRepairLoop {
  ...
  constructor(ports: AutoRepairPorts, options: AutoRepairRunOptions) {
    ...
    if (
      options.policy.minimum_expected_gain < 0 ||
      options.policy.max_score_regression < 0
    ) {
  ...
  async run(initial: RepairSource): Promise<RepairLoopResult> {
    const loopId = await loopIdentity(
      initial,
      this.#options.policy.policy_id,
      this.#options.policy.version,
    );
    if (
      !(await this.#ports.artifacts.isCurrentHead(
        initial.branch_id,
        initial.expected_branch_head,
      ))
    ) {
    ...
    for (
      let iteration = 1;
      iteration <= this.#options.policy.max_auto_repair_iterations;
      iteration += 1
    ) {
      if (
        current.quality.status === "PASS" ||
        current.quality.status === "PASS_WITH_WARNINGS"
      ) {
        return this.#result(
          loopId,
          "SUCCEEDED",
      ...
      const plan = await buildRepairPlan({
        ...
        ...(this.#ports.cost_estimator
          ? { estimator: this.#ports.cost_estimator }
          : {}),
      });
      ...
      try {
        ...
        materialization =
          item.kind === "STRUCTURAL_DESIGN_OP"
            ? await executeStructuralRepair(
                item,
                current,
                this.#ports.structural_materializer,
              )
            : await this.#executeGenerative(item, current, reservation);
      } catch (error) {
        if (reservation)
          await this.#ports.budget!.release(
            reservation,
            "REPAIR_EXECUTION_FAILED",
          );
        const reasons = [
          error instanceof Error ? error.message : "REPAIR_EXECUTION_FAILED",
        ];
        const record = this.#attemptRecord(
          loopId,
          iteration,
          item,
          current,
      ...
      const quality = await this.#ports.quality.evaluate(
        candidateSubject(current, candidate),
        {
      ...
      if (comparison.disposition === "REVIEW") {
        await this.#ports.artifacts.rejectCandidate(
          candidate,
          comparison.reason_codes,
        );
        return this.#result(
      ...
      try {
        await this.#ports.artifacts.promoteCandidate({
          ...
          target_status:
            comparison.disposition === "PROMOTED_READY" ? "READY" : "DRAFT",
          quality,
        });
      } catch (error) {
        await this.#ports.artifacts.rejectCandidate(candidate, [
          "BRANCH_HEAD_CHANGED",
        ]);
        return this.#result(
          loopId,
  ...
  async #selectExecutable(
    ...
    source: RepairSource,
    spent: string,
  ): Promise<RepairPlanItem | null> {
    const loopRemaining = remainingUsd(
      this.#options.policy.max_repair_cost_usd,
      spent,
    );
    for (const item of items) {
      ...
      if (
        !this.#ports.budget ||
        !canAfford(loopRemaining, item.estimated_cost_usd)
      )
        continue;
  ...
  async #executeGenerative(
    ...
    if (!this.#ports.generative)
      throw new AutoRepairError("AUTO_REPAIR_GENERATIVE_PORT_UNAVAILABLE");
    if (!reservation)
      throw new AutoRepairError("AUTO_REPAIR_PAID_REPAIR_WITHOUT_RESERVATION");
    return this.#ports.generative.execute(
      item,
      source,
