...
function largeDocument(): DesignDocument {
  ...
  for (let index = 0; index < 2_000; index += 1) {
    ...
    nodes[id] = {
      ...
      parent_id: "frame",
      children: [],
      content: index === 0 ? "Wrong" : `Label ${index}`,
      transform: {
        x: index % 100,
...
function quality(
  ...
  score: number,
  repair_actions: readonly DesignOperation[] = [],
  artifactVersion = "00000000-0000-4000-8000-000000000004",
  designVersion = "00000000-0000-4000-8000-000000000005",
): QualityResult {
  return {
    ...
    artifact_version_id: artifactVersion,
    design_document_version_id: designVersion,
...
describe("NODE-51 2k-node repair benchmark contract", () => {
  it("repairs one target without dropping any of 2,000 sibling nodes", async () => {
    ...
    const op: DesignOperation = {
      operation_id: "repair-benchmark-op",
      type: "SET_TEXT",
      target_ids: ["text-0"],
    ...
    const sourceQuality = quality(
      "quality-result:benchmark-source",
      "FAIL_REPAIRABLE",
      60,
      [op],
    );
    const source: RepairSource = {
      ...
      expected_branch_head: sourceQuality.artifact_version_id,
      subject: {
        organization_id: sourceQuality.organization_id,
        project_id: sourceQuality.project_id,
        artifact_id: sourceQuality.artifact_id,
        artifact_version_id: sourceQuality.artifact_version_id,
        design_document_version_id: sourceQuality.design_document_version_id,
        design_document: doc,
        rendered_asset_ref: "benchmark:source",
      },
      quality: sourceQuality,
      constraints: [],
    };
    const repo = new MemoryRepairArtifactRepository(
      source.branch_id,
      source.expected_branch_head,
    );
    ...
    const result = await new AutoRepairLoop(
      {
        ...
        structural_materializer: {
          async materialize(candidate) {
            expect(Object.keys(candidate.nodes)).toHaveLength(2_002);
            expect(candidate.nodes["text-0"]?.content).toBe("Correct");
            return {
              rendered_asset_ref: "benchmark:candidate",
              content_hash: "e".repeat(64),
              constraint_snapshot_hash: "f".repeat(64),
            };
          },
        },
        quality: {
          async evaluate(candidate) {
            return quality(
              "quality-result:benchmark-pass",
              "PASS",
              96,
              [],
              candidate.artifact_version_id,
              candidate.design_document_version_id,
            );
          },
        },
      },
      {
        policy: {
          policy_id: "repair-policy:benchmark",
          version: "1",
          max_auto_repair_iterations: 2,
          max_repair_cost_usd: "0",
