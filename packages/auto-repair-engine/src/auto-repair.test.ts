  ...
  return {
    ...
    nodes: {
      ...
      title: {
        ...
        children: [],
        content: text,
        transform: { x: 20, y: 20, width: 200, height: 50 },
      },
    },
    resources: {},
...
function profile(
  dimension: QualityDimension = "TEXT_ACCURACY",
): QualityProfile {
  return {
...
function violation(
  ...
  dimension: QualityDimension = "TEXT_ACCURACY",
  severity: QualityViolation["severity"] = "MAJOR",
): QualityViolation {
  return {
    violation_id: `v:${reason}`,
    dimension,
    severity,
    reason_code: reason,
    message: reason,
...
function structuralMaterializer(events: string[] = []) {
  return {
    async materialize(doc: DesignDocument) {
      ...
      return {
        rendered_asset_ref: `artifact-file:rendered:${version}`,
        content_hash: H,
...
function loopPorts(
  repo: MemoryRepairArtifactRepository,
  qport: RepairQualityPort,
  events: string[] = [],
): AutoRepairPorts {
  return {
    artifacts: repo,
...
async function realQuality(doc = document()) {
  ...
  return {
    p,
    engine,
    result: await engine.evaluate({ subject: subject(doc), profile: p }),
  };
}
...
describe("NODE-51 bounded Auto Repair Loop", () => {
  it("executes NODE-50 DesignOperation through NODE-39 and promotes only after re-evaluation", async () => {
    ...
    const ports = loopPorts(
      repo,
      {
        async evaluate(candidate) {
          events.push("quality");
          expect(
            repo.recordByArtifactVersion(candidate.artifact_version_id)?.status,
          ).toBe("DRAFT");
    ...
    const output = await new AutoRepairLoop(ports, {
      policy,
      now: () => NOW,
    }).run(source(result));
    ...
    expect(
      repo.recordByArtifactVersion(output.final_artifact_version_id)?.status,
    ).toBe("READY");
    expect(events.indexOf("quality")).toBeGreaterThan(
      events.indexOf("materialize"),
    );
    expect(repo.events.findIndex((e) => e.startsWith("persist:"))).toBeLessThan(
      repo.events.findIndex((e) => e.startsWith("promote:")),
    );
  });
  ...
  it("rejects a worse candidate and keeps the source branch head", async () => {
      ...
      loopPorts(repo, {
        async evaluate(candidate) {
          return quality({
            ...
            status: "FAIL_REPAIRABLE",
            score: 60,
            artifactVersion: candidate.artifact_version_id,
            designVersion: candidate.design_document_version_id,
            violations: [violation()],
            repairs: [],
          });
        },
      }),
      { policy: { ...policy, max_auto_repair_iterations: 1 }, now: () => NOW },
    ).run(source(before));
    ...
    expect(candidate && repo.recordByArtifactVersion(candidate)?.status).toBe(
      "REJECTED",
    );
  });
  ...
  it("allows cumulative bounded DRAFT improvement before a later READY repair", async () => {
    const first = quality({
      ...
      status: "FAIL_REPAIRABLE",
      score: 50,
      violations: [violation("COPY_WRONG")],
      repairs: [operation("op:1", "Step One")],
    });
    const repo = new MemoryRepairArtifactRepository(BRANCH, VERSION);
    let round = 0;
      ...
      async evaluate(candidate) {
        ...
        if (round === 1)
          return quality({
            ...
            artifactVersion: candidate.artifact_version_id,
            designVersion: candidate.design_document_version_id,
            violations: [violation("COPY_STILL_LONG")],
            repairs: [operation("op:2", "Correct", 8)],
          });
        return quality({
          id: "q:2",
          status: "PASS",
    ...
    const output = await new AutoRepairLoop(loopPorts(repo, qport), {
      policy,
      now: () => NOW,
    }).run(source(first));
    expect(output.status).toBe("SUCCEEDED");
    expect(output.iterations).toBe(2);
    expect(output.attempts.map((row) => row.disposition)).toEqual([
      "PROMOTED_DRAFT",
      "PROMOTED_READY",
    ]);
    expect(repo.heads.get(BRANCH)).toBe(output.final_artifact_version_id);
  ...
  it("fails closed when a paid repair cannot be reserved within loop/external budget", async () => {
    const before = quality({
      ...
      status: "FAIL_REPAIRABLE",
      score: 55,
      violations: [violation("BACKGROUND_NOISE", "IMAGE_DEFECTS")],
      repairs: [],
    });
    const repo = new MemoryRepairArtifactRepository(BRANCH, VERSION);
    let invoked = false;
    const output = await new AutoRepairLoop(
      ...
      { policy: { ...policy, max_repair_cost_usd: "0.5" }, now: () => NOW },
    ).run(source(before));
  ...
  it("reserves budget before paid generative execution and settles actual cost", async () => {
    ...
    const budget: BudgetReservationPort = {
      ...
      async settle(_reservation, actual) {
        events.push(`settle:${actual}`);
      },
      async release() {
        events.push("release");
      },
    ...
    const output = await new AutoRepairLoop(
      {
        ...
        budget,
        generative: {
          async execute(_item, current, reservationId) {
            events.push(`generate:${reservationId}`);
            return {
              design_document: current.subject.design_document,
              rendered_asset_ref: "artifact-file:edited",
              content_hash: "c".repeat(64),
              constraint_snapshot_hash: "d".repeat(64),
    ...
    expect(events.indexOf("reserve")).toBeLessThan(
      events.indexOf("generate:reservation:1"),
    );
    expect(events.indexOf("generate:reservation:1")).toBeLessThan(
      events.indexOf("settle:0.35"),
    );
    expect(events.indexOf("settle:0.35")).toBeLessThan(
      events.indexOf("quality"),
    );
  });
  ...
  it("returns STALE_SOURCE when a concurrent user edit wins before candidate promotion", async () => {
    const before = quality({
      ...
      score: 60,
      violations: [violation()],
      repairs: [operation("op:1", "Correct")],
    });
    const repo = new MemoryRepairArtifactRepository(BRANCH, VERSION);
    const output = await new AutoRepairLoop(
      loopPorts(repo, {
        async evaluate(candidate) {
          repo.simulateExternalHead(
            BRANCH,
  ...
  it("never persists a structural candidate denied by NODE-39 preflight", async () => {
    const before = quality({
      ...
      status: "FAIL_REPAIRABLE",
      score: 50,
      violations: [violation("LOCKED_COPY")],
      repairs: [operation("op:locked", "Correct")],
    });
    const constraint: DesignConstraint = {
      id: "lock-title",
      type: "LOCK_TEXT",
      scope: { node_ids: ["title"] },
      severity: "HARD",
    ...
    const output = await new AutoRepairLoop(
      ...
      { policy: { ...policy, max_auto_repair_iterations: 2 }, now: () => NOW },
    ).run(source(before, document(), [constraint]));
