...
export function aggregateViolations(
  ...
  for (const violation of violations) {
    ...
    const stronger =
      SEVERITY_RANK[violation.severity] > SEVERITY_RANK[current.severity];
    const higherScore =
      (violation.score ?? Number.NEGATIVE_INFINITY) >
      (current.score ?? Number.NEGATIVE_INFINITY);
    if (stronger || (!stronger && higherScore))
      byKey.set(key(violation), violation);
  }
...
export function postflightDecision(
  violations: readonly ConstraintViolation[],
): PostflightDecision {
  if (violations.some((violation) => violation.severity === "HARD"))
    return "FAIL";
  if (violations.some((violation) => violation.severity === "SOFT"))
    return "REPAIR";
  return "PASS";
}
