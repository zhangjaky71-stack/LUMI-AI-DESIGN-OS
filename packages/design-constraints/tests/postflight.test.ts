...
describe("NODE-39 postflight", () => {
  it("fails closed when a hard QR validator is unavailable", async () => {
    const report = await new ConstraintPostflightRuntime([]).validate(
      context([constraint()]),
    );
  ...
  it("detects a changed QR payload", async () => {
    const decoder: QrDecoder = {
      async decode() {
        return {
          detected: true,
          payload: "https://wrong.example",
          quiet_zone_modules: 4,
          readable_at_target_size: true,
        };
      },
    };
    const report = await new ConstraintPostflightRuntime([
      new QrScannabilityEvaluator(decoder),
    ]).validate(context([constraint()]));
    expect(report.decision).toBe("FAIL");
    expect(
      report.violations.some(
        (item) => item.reason_code === "QR_PAYLOAD_CHANGED",
      ),
    ).toBe(true);
  });
  ...
  it("turns a hard evaluator crash into VALIDATION_UNAVAILABLE", async () => {
    ...
    const report = await new ConstraintPostflightRuntime([broken]).validate(
      context([constraint()]),
    );
    expect(report.decision).toBe("FAIL");
