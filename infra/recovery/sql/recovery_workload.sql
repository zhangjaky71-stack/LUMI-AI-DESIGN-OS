...
SELECT 'COUNT', 'idempotency_ambiguous', count(*)::text, NULL, NULL
  FROM idempotency_operations WHERE status = 'ambiguous'
UNION ALL
SELECT 'COUNT', 'idempotency_in_progress', count(*)::text, NULL, NULL
  FROM idempotency_operations WHERE status = 'in_progress'
UNION ALL
SELECT 'COUNT', 'tasks_running_or_waiting', count(*)::text, NULL, NULL
  FROM tasks WHERE status IN ('pending','ready','running','retry','failed_retryable','waiting_user','waiting_external')
UNION ALL
SELECT 'COUNT', 'agent_runs_non_terminal', count(*)::text, NULL, NULL
  FROM agent_run_control WHERE control_status IN ('pending','running','interrupted')
ORDER BY category;
...
SELECT 'ITEM', 'idempotency_operation', id::text, status,
 ...
 WHERE status IN ('new','in_progress','failed_retryable','ambiguous')
...
SELECT 'ITEM', 'task', id::text, status,
  ...
  FROM tasks
 WHERE status IN ('pending','ready','running','retry','failed_retryable','waiting_user','waiting_external')
 ORDER BY updated_at, id
 LIMIT 500;
